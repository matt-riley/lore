...
5. [OTHER] When running Fallow on Lore, configure `tests/**/*.test.mjs` as entry points, mark browser-served assets as dynamically loaded, and ignore runtime-provided modules like `@github/copilot-sdk` and `node:sqlite` before trusting unused-code fixes - the unconfigured audit stripped required test-helper exports and misclassified real browser/runtime surfaces as dead code.
6. [OTHER] When refactoring Lore prompt-context or capsule assembly, carry derived decision state like style suppression, day-summary reasons, and temporal-verifier reasons explicitly between helpers instead of re-deriving it from partially mutated trace objects - those values drive both rendered context and omission traces, so recomputing them across phases is easy to break.
7. [OTHER] When Lore prompt-context code needs a default `promptNeed`, derive it from `detectPromptContextNeed(prompt)` rather than inferring temporal intent from lexical terms alone - generic prompts like `continue auth migration` can otherwise fall into temporal fallback paths and emit wrong day-summary/temporal trace state.
8. [OTHER] When `tests/unit/diagnostics.test.mjs` rewrites `lib/diagnostics.mjs` into a `data:` import for hotspot exports, add any new relative `diagnostics.mjs` imports to that rewrite list or the test loader will fail with unresolved module specifiers during validation.
