...
- In `website/`, use the Node.js and pnpm versions pinned in `website/package.json`: `pnpm install --frozen-lockfile`, then `pnpm dev`. Validate with `pnpm check`, `pnpm test`, `pnpm build`, and `pnpm check:links`.
