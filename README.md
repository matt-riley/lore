...
When standing directives are enabled, explicit directives and extracted standing preferences and prohibitions can appear without matching the prompt's keywords. Cross-project directives require explicit global wording or an explicit manual global write; inferred directives without repository attribution stay out of ambient recall. The section is capped at six eligible memories and respects repository scope, expiry, suppression, and the final output budget. Ordinary complaints and rejected task proposals do not qualify as standing policies.
