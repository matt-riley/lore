...
When standing directives are enabled, explicit global or manually written directives can appear without matching the prompt's keywords. Auto-extracted repository directives must match the current prompt, so a rule from one task does not steer an unrelated task in the same repository. Extracted preferences and rejected approaches remain contextual memories rather than entering the always-on directive block. Cross-project directives require explicit global wording or an explicit manual global write. The section is capped at six eligible memories and respects repository scope, expiry, suppression, and the final output budget.

All five coding agents (GitHub Copilot CLI, Pi, Codex CLI, Claude Code, and Google Antigravity CLI) share the same unified memory engine, deterministic recall, and transcript extraction. Lore defines nine canonical model tools (`lore_recall`, `lore_retain`, `lore_onboard`, `lore_search`, `lore_forget`, `lore_status`, `lore_explain`, `lore_validate`, and `lore_correct`). Copilot registers that set, and legacy names remain available as input aliases through `/lore <verb>` or `lore tool <name>`; Pi registers the canonical set plus the `lore_save` alias. Extra capabilities and native CLI workflows run through `/lore <verb>` slash commands, `lore <verb>` in the shell, or the `$LORE_HOME/bin/lore` PATH shim (e.g. `export PATH="$LORE_HOME/bin:$PATH"`).
...
- **Relevance and scope.** Explicit global or manually written directives are ambient; auto-extracted repository directives must match the current prompt. Extracted preferences and rejected approaches stay on the normal contextual recall path rather than entering `Standing Directives`.
- **Durability.** Relevant auto-extracted directives are graded "durable policy" versus "one-off instruction" before the six-row cap, and statements below `minDurability` are dropped from the rendered section. This keeps session-specific requests such as *"there should be a live env key"* out of the directive list. Scores are persisted in the row's metadata, so each memory is judged once and reused; unscored rows and scoring failures are kept, exactly as before the feature existed.
- **Specificity.** When more than six relevant directives qualify, the specific ones take the slots. Memory hygiene also scores open loops and assistant goals in the background (bounded per run, never on the prompt path) and reports those below `minSpecificity` as **vague** review candidates. A vagueness score never supersedes a memory, in any mode — a model judgment is a review signal, not a deletion.
...
`maintenanceScheduler` is the opt-in maintenance loop for automated memory hygiene, deferred extraction processing, validation and replay corpus runs, backlog review, and index upkeep. On session start, Lore evaluates the maintenance plan and runs the bounded `memoryHygiene` and `deferredExtraction` tasks when they are enabled and due — on every supported client, not just Copilot. Copilot and Pi run this in-process, since both host Lore as a long-lived process; Codex, Claude Code, and Antigravity hooks are short-lived, so they instead spawn a detached, unref'd `scripts/run-maintenance.mjs --background` process and return immediately without waiting on it. A cross-process lock keeps concurrent sessions from double-running the sweep. The same scheduler also powers manual or scripted sweeps through `maintenance_schedule_run` and `node scripts/run-maintenance.mjs`.
...
The [documentation website](website/README.md) is a separate Astro site with setup guides for all five agents and an interactive memory walkthrough. It uses the Node.js and pnpm versions pinned by [`website/package.json`](website/package.json), unlike Lore's build-free runtime. Run it locally with `cd website && pnpm install --frozen-lockfile && pnpm dev`; its own README covers checks and Cloudflare Workers static-asset hosting.
