...
| `scripts/run-maintenance.mjs` | 🟡 Experimental | The supported out-of-session entry point for maintenance tasks (`memoryHygiene`, `validationCorpus`, `replayCorpus`, `backlogReview`, `traceCompaction`, `indexUpkeep`, `doctorSnapshot`, `extractionRevalidation`). Designed for cron, launchd, or any external scheduler. `--background` is the quiet, cross-process-locked session-start variant that Codex, Claude Code, and Antigravity spawn automatically — not intended for manual/interactive use. It has a 60-second time budget and exits 1 if that budget expires, after in-flight work settles so the database can close safely. The script exits 1 on unknown task names or DB errors too. Operates only on the configured Lore database — never on test fixtures or other users' databases. See [`docs/maintenance-scheduling.md`](maintenance-scheduling.md) for the full guide. Use `maintenance_schedule_run` tool for in-session triggering. |
| `scripts/run-browser.mjs` | 🟡 Experimental | Starts the local browser dashboard. Loopback hosts only (`127.0.0.1`, `localhost`, or `::1`). |
| `scripts/recover.mjs` | 🟢 Supported | Reports recovery status, creates an explicit database snapshot, or previews/restores one with `--from`; a write restore requires `--write --clients-stopped` and `lsof` for open-handle detection, and fails closed if detection is unavailable. |
