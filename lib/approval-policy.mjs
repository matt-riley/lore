...
const APPROVAL_FAMILY = Object.freeze({
...
function normalizeRecordApprovalInput({
  actionFamily,
  repository = null,
  targetIdentity = null,
  decision,
...
function findReusableApproval({ db, actionFamily, repository, targetIdentity }) {
...
function updateApprovalRecord(db, {
  id,
  normalizedDecision,
  durable,
  reason,
...
function insertApprovalRecord(db, {
  id,
  actionFamily,
  repository,
  targetIdentity,
 ...
 */
function resolveApproval(db, { actionFamily, repository = null, targetIdentity = null } = {}) {
  if (typeof actionFamily !== "string" || actionFamily.trim().length === 0) {
    throw new Error("actionFamily must be a non-empty string");
  }
...
function recordApproval(db, {
  ...
  const input = normalizeRecordApprovalInput({
    actionFamily,
    repository,
    targetIdentity,
    decision,
  ...
  const existing = findReusableApproval({
    db,
  ...
  if (existing) {
    return updateApprovalRecord(db, {
      id: existing.id,
  ...
  return insertApprovalRecord(db, {
    id,
    actionFamily: input.actionFamily,
    repository: input.repository,
    targetIdentity: input.targetIdentity,
 ...
 */
function listApprovals(db, { actionFamily = null, repository = null } = {}) {
 ...
 */
function purgeExpiredApprovals(db) {
  const now = nowIso();
