...
function backfillRecentSessions({
  db,
  sessionStore,
  repository,
  limit = 25,
...
export async function buildSessionStartBackfillPreview({
  ...
  while (collected.length < candidateLimit && inspected < inspectedLimit) {
    ...
    const lastWindowRow = window[window.length - 1];
    cursor = lastWindowRow
      ? {
          updatedAt: "sessionStoreUpdatedAt" in lastWindowRow
            ? (lastWindowRow.sessionStoreUpdatedAt ?? "")
            : (lastWindowRow.updated_at ?? ""),
          id: lastWindowRow.id,
        }
      : cursor;
