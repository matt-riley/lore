 ...
 *
 * Every surface (human CLI, protocol lore tool, /lore slash, Pi worker, host
 * adapters) dispatches through lib/runtime/operation-dispatch.mjs, which owns
 * alias resolution, the single registered-tool lookup, and the structured
 * { ok, text | error } outcome that adapters map to their transport.
 *
...
const CAPABILITY_SPECS = [
    ...
    parameters: {
      ...
      properties: {
        ...
        tasks: {
          ...
          items: {
            type: "string",
            enum: ["memoryHygiene", "deferredExtraction", "validationCorpus", "replayCorpus", "backlogReview", "traceCompaction", "indexUpkeep", "doctorSnapshot", "extractionRevalidation"],
          },
  ...
  },
  {
    ...
    aliases: [],
    description: "Replay the current standing-directive grammar against rule-extracted memories and report, or reversibly apply, rows an older grammar version would no longer produce.",
    routeKindHints: [],
    surfaces: {
      model: false,
      cli: false,
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: "string",
          enum: ["run", "rollback"],
          description: "Report/apply a revalidation pass, or roll back one exact extractor-revalidation:* marker",
        },
        repository: {
          type: "string",
          description: "Optional repository to scope the candidate rows to (global rows are still included unless includeGlobal is false)",
        },
        includeGlobal: {
          type: "boolean",
          description: "When true (default), include global-scoped rows alongside the given repository's rows",
        },
        apply: {
          type: "boolean",
          description: "When true, apply verdicts under a reversible extractor-revalidation:<runId> marker instead of only reporting them",
        },
        maxItems: {
          type: "number",
          description: "Maximum candidate rows to inspect in this run",
        },
        marker: {
          type: "string",
          description: "Exact extractor-revalidation:* marker required for rollback",
        },
        actor: {
          type: "string",
          description: "Operator identity recorded in an apply or rollback audit",
        },
        reason: {
          type: "string",
          description: "Human-readable reason recorded in an apply or rollback audit",
        },
      },
    },
    support: {
      status: "experimental",
      category: "Maintenance",
      notes: "Report-only by default. Never creates memory_suppression rows: a rejected row means the current grammar would not produce it, not that the user asked to forget it. Apply is reversible via rollback with the run's exact marker.",
      rolloutFlags: [],
    },
  },
  {
  ...
  },
  {
    ...
    support: {
      ...
      notes: "Primary recall verb. Returns matched memories with provenance. Optional local query expansion changes retrieval terms only and retries deterministic retrieval when expansion finds no evidence. When `localInference.embeddings` is configured, fuses embedding-ranked hits into one lexical∪vector list with reciprocal rank fusion; fails open to lexical-only on endpoint errors. When `typesafe.enabled` + `typesafe.rerank.enabled` are set, reorders the fused shortlist by TypeSafe Jev usefulness scores; fails open to the fused order on provider errors.",
      rolloutFlags: [],
    },
  },
  {
  ...
  },
  {
    ...
    support: {
      status: "supported",
      category: "Status and diagnostics",
      notes: "Explains what context would be injected for a given prompt and why. CLI / `lore tool` aliases for one deprecation cycle: `memory_explain`.",
      rolloutFlags: [],
    },
  },
  {
    ...
    support: {
      status: "supported",
      category: "Status and diagnostics",
      notes: "Validates DB integrity and schema parity. CLI / `lore tool` aliases for one deprecation cycle: `memory_validate`.",
      rolloutFlags: [],
    },
  },
  {
  ...
  },
  {
    ...
    description: "Run the observe-only Lore Doctor: classify incidents from maintenance task states, trajectory artifacts, latency metrics, improvement backlog signals, install health (missing/pinned Node paths, missing lore-cli.mjs entries, duplicate global/project installs), ingestion checkpoint failures, and maintenance run staleness. Emits an additive doctor-report trajectory artifact (unless dryRun is set). No trusted-source mutation occurs; read-only, including of client hook settings.",
    routeKindHints: [],
    surfaces: {
      model: false,
      cli: false,
    ...
    support: {
      status: "experimental",
      category: "Self-diagnostics and proposals",
      notes: "Generates a structured health report, including install health (missing/pinned Node paths, missing `lore-cli.mjs`, duplicate global/project installs), ingestion checkpoint failures, and maintenance run staleness. Requires `loreDoctor` and `evolutionLedger` rollout flags. Alias: `memory_doctor_report`.",
