 ...
 * (lib/memory-tools.mjs) attaches parameters and handlers here; the capability
 * inventory barrel and scanner modules read route hints directly from this
...
// Native CLI adapters invoke the same core tools without a protocol server.
export const LORE_CLI_TOOL_NAMES = Object.freeze([
  "lore_recall", "lore_retain", "lore_onboard", "memory_search",
  "memory_save", "memory_forget", "memory_status", "memory_correct",
  "memory_repair", "memory_purge",
]);
...
const CAPABILITY_SPECS = [
  ...
  },
  {
    ...
    support: {
      ...
      notes: "Triggers a maintenance sweep (dry-run or live), reports automated memory hygiene, or rolls back one exact `auto-hygiene:*` marker with an audit artifact.",
      rolloutFlags: [],
    },
  },
  {
  ...
  },
  {
    ...
    support: {
      status: "supported",
      category: "Core memory verbs",
      notes: "Primary retain verb. Persists a memory with scope, category, and optional domain association.",
  ...
  },
  {
    ...
    support: {
      status: "supported",
      category: "Core memory verbs",
      notes: "Keyword search over the derived semantic-memory store. Meaning-based (vector) search is available via `lore_recall` when embeddings are configured.",
      rolloutFlags: [],
    },
  },
  {
  ...
  },
  {
    ...
    support: {
      status: "supported",
      category: "Core memory verbs",
      notes: "Soft-deletes a memory by ID. Superseded rows and related residual data may remain for provenance and recovery; this is not secure erasure.",
      rolloutFlags: [],
    },
  },
  {
    ...
    support: {
      ...
      notes: "Defaults to a read-only preview. Repository selects the manual replacement destination. Apply requires planFingerprint, preserves expiry unless changed, and creates a validated snapshot.",
      rolloutFlags: [],
    },
  },
  {
    ...
    support: {
      status: "experimental",
      category: "Memory administration",
      notes: "Deterministic complete-source repair, bounded to 32 MiB per source and the preview candidate limit. Missing, incomplete, or ambiguous sources remain unresolved. Apply requires planFingerprint and actionable selectedCandidateIds.",
      rolloutFlags: [],
    },
  },
  {
    ...
    support: {
      status: "experimental",
      category: "Memory administration",
      notes: "Requires explicit memoryIds, repository, or global scope selection. Shared derived copies require includeDependentAggregates and all typed preview candidate IDs. Apply validates planFingerprint and a snapshot; raw sources, backups, and suppression remain.",
      rolloutFlags: [],
    },
  },
  {
