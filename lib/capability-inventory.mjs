...
const TOOL_ROUTE_HINTS = Object.freeze({
  lore_recall: ["retrieval"],
  lore_reflect: ["retrieval"],
...
function buildToolRouteHints(name, extensionName) {
  ...
  if (extensionName === "lore" && /^memory_(status|validate|replay)$/.test(name)) {
    return ["direct"];
  }
  return [];
}
  ...
  switch (routeCandidate.route) {
    case "retrieval":
      ...
      if (promptProfile.reflectIntent) {
        heuristicScore += 5;
        reasons.push("Reflection phrasing maps well to lore_reflect.");
      }
...
async function scanExtensionSurfaces(rootPath) {
  ...
  const loreToolsText = await safeReadFile(loreToolsFile);
  if (loreToolsText) {
    const loreTools = extractToolSpecs(loreToolsText);
    for (const tool of loreTools) {
      tools.push(buildCapabilityBase({
        id: `tool:${tool.name}`,
        name: tool.name,
        description: tool.description || "lore tool",
        sourcePath: relativePath(rootPath, loreToolsFile),
        capabilityType: "tool",
        routeKindHints: buildToolRouteHints(tool.name, "lore"),
        ...
        triggerCapabilities: [
          "lore",
          ...buildToolRouteHints(tool.name, "lore"),
        ],
        metadata: {
    ...
    if (loreEntry) {
      loreEntry.toolNames = [...new Set([...loreEntry.toolNames, ...loreTools.map((tool) => tool.name)])]
        .sort();
