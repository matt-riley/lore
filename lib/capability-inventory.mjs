...
function selectRouteTarget(capabilities, routeId, promptProfile, supportingMatches, matchMap) {
  ...
  if (routeId === "skill") {
    ...
    return selectMatchedCapabilityTarget(capabilities, routeId, filteredMatches);
  }
  if (routeId === "agent") {
...
function applySkillRouteHeuristics({
  ...
  if (promptProfile.promptRewriteIntent && selectedTarget?.targetName === "reverse-prompt") {
    addHeuristicReason(state, 18, "Explicit prompt-sharpening requests should prefer the reverse-prompt skill.");
  }
  if (explicitIntentScore > 0) {
    addHeuristicReason(state, 8 + explicitIntentScore, "Prompt explicitly asks for a reusable workflow or skill-like playbook.");
  }
  if (targetMatchScore > 0) {
    addHeuristicReason(state, Math.min(8, targetMatchScore), `Matched skill target ${selectedTarget.targetName}.`);
  }
  if (explicitIntentScore === 0 && targetMatchScore < 7) {
    addHeuristicReason(state, -8, "Skill routing stays conservative without a clear workflow or skill signal.");
  }
  if (promptProfile.promptNeed.requiresLookup && explicitIntentScore === 0) {
    addHeuristicReason(state, -6, "Prompt looks more like recall/explanation than a skill workflow.");
  }
  if (
    ...
    addHeuristicReason(state, -28, "Generic reference questions should stay direct unless they clearly ask for a local workflow.");
  }
  if (promptProfile.planBeforeExecution && promptProfile.ciMigrationIntent) {
    addHeuristicReason(state, -16, "Prompt asks for migration planning before editing, so orchestration should outrank a direct migration skill.");
  }
}
...
function applyAgentRouteHeuristics({
  ...
  if (promptProfile.promptRewriteIntent && promptProfile.hasReversePromptSkill) {
    addHeuristicReason(state, -18, "Explicit prompt-sharpening requests should stay on the skill workflow path instead of agent delegation.");
  }
  if (explicitIntentScore > 0) {
    addHeuristicReason(state, 8 + explicitIntentScore, "Prompt asks for planning, research, delegation, or orchestration.");
  }
  if (targetMatchScore > 0) {
    addHeuristicReason(state, Math.min(8, targetMatchScore), `Matched agent target ${selectedTarget.targetName}.`);
  }
  if (selectedTarget?.manualOnly && explicitIntentScore === 0 && !selectedTarget.nameMatched) {
    addHeuristicReason(state, -4, "Manual-only agents need clearer delegation intent than this prompt provides.");
  }
  if (explicitIntentScore === 0 && targetMatchScore < 7) {
    addHeuristicReason(state, -8, "Agent routing stays conservative without clear delegation intent.");
  }
  if (promptProfile.promptNeed.requiresLookup && explicitIntentScore === 0) {
    addHeuristicReason(state, -8, "Prompt looks like local recall/explanation instead of delegated work.");
  }
  if (promptProfile.planBeforeExecution && promptProfile.ciMigrationIntent) {
    addHeuristicReason(state, 28, "Plan-first CI migration prompts should prefer the migration orchestrator before execution.");
  }
}
...
function applyDirectRouteHeuristics({ promptProfile, selectedTarget, state }) {
  if (promptProfile.promptNeed.identityOnly) {
    addHeuristicReason(state, 18, "Identity-only or direct-address prompt does not need another local surface.");
  }
  if (promptProfile.greeting) {
    addHeuristicReason(state, 6, "Greeting-style prompt can be answered directly.");
  }
  if (
    promptProfile.referenceQuestion
    && promptProfile.promptNeed.requiresLookup !== true
    ...
    addHeuristicReason(state, 24, "Generic reference question does not need a local workflow recommendation.");
  }
  if (!promptProfile.promptNeed.requiresLookup && promptProfile.simplePrompt) {
    addHeuristicReason(state, 4, "Short prompt with no recall/delegation signal fits a direct response.");
  }
  if (promptProfile.promptNeed.requiresLookup) {
    addHeuristicReason(state, -8, "Prompt needs local context, so direct/no-op is less appropriate.");
  }
  if (
    promptProfile.skillIntentScore > 0
    || promptProfile.agentIntentScore > 0
    ...
    addHeuristicReason(state, -6, "Prompt contains stronger routing signals than a direct/no-op response.");
  }
  addTargetRationale(state, selectedTarget);
}
...
function evaluateRouterAssertions(definition, recommendation) {
  ...
  for (const [label, passed, details] of baseAssertions) {
    assertions.push({ label, passed, details });
  }
  if (definition.expectedRouteKind !== "direct") {
    assertions.push({
      label: "matched capabilities are present",
      passed: Array.isArray(recommendation.capabilityMatches) && recommendation.capabilityMatches.length > 0,
      details: `actual=${recommendation.capabilityMatches?.length ?? 0}`,
    });
  }
