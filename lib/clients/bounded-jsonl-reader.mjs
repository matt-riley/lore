...
export async function readJsonlDelta(file, {
  ...
  for (const [name, value] of Object.entries({ maxBytes, maxRecordBytes, budgetMs, maxRecords })) {
  ...
  try {
    ...
    const prefixLength = Math.min(checkpoint?.sourcePrefixLength ?? 4096, source.size);
    ...
    const reset = Boolean(checkpoint && (
      ...
      (checkpoint.sourcePrefixHash && checkpoint.sourcePrefixHash !== sourcePrefixHash)
    ));
    ...
    while (offset < source.size && bytesRead < maxBytes && performance.now() - started < budgetMs) {
      ...
      let start = 0;
      while (start < read.bytesRead && records.length < maxRecords) {
        ...
        if (complete) {
          if (!discarding && partial.toString("utf8").trim()) {
            try {
              ...
              if (!acceptRecord || acceptRecord(value)) records.push({ value, offset: recordOffset });
            } catch {
