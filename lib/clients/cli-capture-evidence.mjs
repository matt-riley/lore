...
export function reconcileCaptureEvidence({ db, sessionId, artifacts, extraction }) {
  ...
  for (const memory of extraction.semanticMemories) {
    if (memory.evidence) memory.evidence.metadata = { ...memory.evidence.metadata, captureGeneration: generation };
  }
  const bySource = db.db.prepare("SELECT evidence_key, revision FROM session_evidence WHERE session_id = ? AND source_record_id = ?");
...
function reconcileClaudeGraph({ db, sessionId, artifacts, abandoned }) {
  ...
  const put = db.db.prepare(`INSERT INTO session_evidence (evidence_key, session_id, repository,
    ...
    metadata_json, captured_at) VALUES (?, ?, ?, 'claude', ?, 'capture_node', ?, ?, ?, ?, ?)
  ...
  for (const record of artifacts.claudeRecords ?? []) {
    ...
    put.run(key(record.uuid), sessionId, artifacts.session.repository, record.uuid, record.sourceRevision,
