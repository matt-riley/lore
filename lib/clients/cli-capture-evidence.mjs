  ...
  for (const id of abandoned) {
    for (const old of bySource.all(sessionId, id)) {
      ...
      if (artifacts.activeLeafUuid) {
        ...
        db.db.prepare("UPDATE session_evidence SET metadata_json = json_set(metadata_json, '$.captureBranchLinks', json(?)) WHERE evidence_key = ?")
          .run(JSON.stringify(links), old.evidence_key);
      }
  ...
  for (let count = 0; work && count < 128; count += 1) {
    ...
    if (work.phase === "mark") {
      ...
      for (const evidence of evidenceRows) {
        ...
        if (metadata.captureGeneration !== artifacts.captureGeneration || !Array.isArray(metadata.captureBranchLinks)) continue;
