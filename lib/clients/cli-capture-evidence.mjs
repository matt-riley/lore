...
export function reconcileCaptureEvidence({ db, sessionId, artifacts, extraction }) {
  ...
  const bySource = db.db.prepare("SELECT evidence_key, revision, metadata_json FROM session_evidence WHERE session_id = ? AND source_record_id = ? AND retired_at IS NULL");
