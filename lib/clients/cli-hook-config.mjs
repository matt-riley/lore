import { LORE_CLIENT_HOOKS } from "../capabilities/capability-manifest.mjs";

export const shellQuote = (value, platform = process.platform) => {
  const str = String(value);
  if (platform === "win32") {
    return `"${str.replaceAll('"', '""')}"`;
  }
  return `'${str.replaceAll("'", "'\\''")}'`;
};

export function buildCliHookConfig(client, { nodePath, entryPath }) {
  if (!Object.hasOwn(LORE_CLIENT_HOOKS, client)) throw new Error("Client must be codex, claude, or antigravity");
  const events = Object.fromEntries(LORE_CLIENT_HOOKS[client].map((event) => {
    const handler = {
      type: "command",
      command: `${shellQuote(nodePath)} ${shellQuote(entryPath)} hook ${client} ${event}`,
      // Codex SessionEnd used to be 3s (observed host max). Try 10s; if a host
      // rejects the hook config, keep 3s and finish capture with `lore capture --resume`.
      timeout: 10,
    };
    return [event, client === "antigravity" && event !== "PostToolUse"
      ? [handler] : [{ hooks: [handler] }]];
  }));
  return client === "antigravity" ? { lore: events } : { hooks: events };
}

export function mergeCliHookConfig(existing, fragment, client, { remove = false, ownedCommands = null } = {}) {
  if (!existing || typeof existing !== "object" || Array.isArray(existing)) throw new Error("Existing hook config must be an object");
  const result = structuredClone(existing);
  if (client === "antigravity") {
    // The host keeps Lore in a single top-level `lore` group, so replacement is
    // only safe when every existing command is one Lore owns (or the fragment's
    // own commands on a first install). Otherwise refuse instead of silently
    // overwriting an unrelated or modified group.
    const commands = new Set(ownedCommands ?? Object.values(fragment.lore).flatMap((value) => value.flatMap((hook) => hook.command || hook.hooks?.[0]?.command)));
    if (result.lore) {
      const currentCommands = Object.values(result.lore).flatMap((value) => value.flatMap((hook) => hook.command || hook.hooks?.[0]?.command));
      if (currentCommands.some((command) => !commands.has(command))) throw new Error("An unrelated or modified 'lore' hook group already exists; review it before replacing");
    }
    if (remove) delete result.lore;
    else result.lore = fragment.lore;
    return result;
  }
  if (result.hooks !== undefined && (!result.hooks || typeof result.hooks !== "object" || Array.isArray(result.hooks))) {
    throw new Error("Existing hooks must be an object");
  }
  result.hooks ??= {};
  for (const [event, groups] of Object.entries(fragment.hooks)) {
    const command = groups[0].hooks[0].command;
    const commands = new Set(ownedCommands ?? [command]);
    const current = result.hooks[event] ?? [];
    if (!Array.isArray(current) || current.some((group) => !Array.isArray(group.hooks))) throw new Error(`Invalid hook groups for ${event}`);
    result.hooks[event] = current.map((group) => ({ ...group, hooks: group.hooks.filter((hook) => !commands.has(hook.command)) }))
      .filter((group) => group.hooks.length);
    if (!remove) result.hooks[event].push(...groups);
    if (!result.hooks[event].length) delete result.hooks[event];
  }
  return result;
}
