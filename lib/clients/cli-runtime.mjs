...
import { onSessionCapture, recordErrorTelemetry, recordPostToolUseObservation } from "../lifecycle/session-lifecycle.mjs";
...
/**
 * Create a stateless error telemetry compactor for CLI hooks.
 * CLI hooks are short-lived processes (one per event), so a persistent counter
 * doesn't work. This uses a random draw with injectable source for testing.
 * Approximately 2% of error telemetry writes will trigger pruning.
 *
 * @param {object} options
 * @param {object} options.db - LoreDb instance
 * @param {() => number} [options.randomSource] - For testing: injectable random
 *   source. Defaults to Math.random(). Should return a number [0, 1).
 * @returns {() => void} A callback suitable for recordErrorTelemetry's onCompact.
 */
export function createCliErrorTelemetryCompactor({ db, randomSource = Math.random } = {}) {
  return () => {
    // Probabilistic compaction: ~2% of writes trigger pruning.
    if (randomSource() >= 1 / 50) {
      return;
    }
...
function capture(runtime, client, sessionId, artifacts, { revision } = {}) {
  ...
  const { captureState } = onSessionCapture({
    db,
    sessionId,
    repository,
    config,
    artifacts,
    source: `rule:${client}:${revision ?? ""}`,
    skipApplyWhenNoTurns: true,
  });
  return captureState;
}
...
export async function runCliHook(client, event, payload) {
  ...
  try {
    ...
    if (["PostToolUse", "PostToolUseFailure"].includes(event)) {
      ...
      recordPostToolUseObservation({ db, config, repository, payload: observationPayload, contextFields: () => ({ client, sessionId }) });
      if (failed) recordErrorTelemetry({
        db,
        config,
        payload: { context: "tool_use" },
        sessionId,
        onCompact: createCliErrorTelemetryCompactor({ db }),
      });
    }
    return neutral;
