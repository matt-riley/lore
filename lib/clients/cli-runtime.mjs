...
import { ingestCliTranscript, validateCliTranscriptIdentity } from "./cli-transcript-ingestion.mjs";
...
export async function runCliTool(name, args) {
  ...
  const administration = ["memory_correct", "memory_repair", "memory_purge"].includes(name);
  const request = administration
    ? normalizeAdministrationRequest({ ...args, operation: name.slice("memory_".length) })
    : null;
...
export async function runCliCapture(client, nativeId, payload) {
  ...
  await validateCliTranscriptIdentity(transcriptPath, { client, nativeId });
  const runtime = await openCliRuntime({ cwd, explicit: process.env.LORE_REPOSITORY });
  if (!runtime) throw new Error("Lore is disabled; enable it in lore.json");
  try {
