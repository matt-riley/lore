...
export async function runCliHook(client, event, payload) {
  ...
  const runtime = await openCliRuntime({ cwd, explicit: process.env.LORE_REPOSITORY, client });
...
// Classify an invocation before storage is opened: administration previews
// must open read-only, and invalid administration requests must fail before a
// store can be created or migrated. Throws on invalid administration args.
export function resolveCliToolAccess(name, args = {}) {
  const resolved = resolveLoreToolName(name) ?? name;
  const operation = ADMINISTRATION_OPERATIONS[resolved];
  if (!operation) {
    return { administration: false, readOnly: false };
  }
  const request = normalizeAdministrationRequest({ ...args, operation });
  return { administration: true, readOnly: request.action === "preview" };
}
...
export async function runCliTool(name, args) {
  ...
  const access = resolveCliToolAccess(resolved, args);
  ...
  try {
    ...
    return await tool.handler(access.administration ? args : { ...args, repository }, { sessionId: `cli:${process.pid}` });
  } finally { runtime.db.close(); }
}
