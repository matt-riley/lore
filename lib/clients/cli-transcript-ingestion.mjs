...
function mergeTurns(state, entries, client) {
  ...
  while (next.turns.length > 1 && textBytes > MAX_TEXT_BYTES) {
    const evicted = next.turns.shift();
    textBytes -= Buffer.byteLength(evicted.user_message) + Buffer.byteLength(evicted.assistant_response);
  }
  const retainedIds = new Set(sourceRecords(next.turns).map((record) => record.source_record_id));
...
export async function ingestCliTranscript({
  ...
  let result;
  try {
    result = prior?.adapterState?.branchWork && sourcePath === prior.adapterState.sourcePath
