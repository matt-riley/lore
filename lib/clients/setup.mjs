...
import { resolveStableNodePath } from "./stable-node-path.mjs";

export const LORE_PATH_SHIM_MARKER = "# lore-path-shim";

const SETUP_CLIENTS = Object.freeze([
...
// Shared by planSetup/setupTargets (which also install/remove) and the
// install-health doctor check (which only needs to know where to look).
export function hookClientTargets(env, home) {
  return {
...
function findWritablePathDirectory(env = process.env, {
  home = os.homedir(),
  skip = [],
  destName = "lore",
  allowDest = null,
...
export function selectClients(value, clients, { allowedExtra = [] } = {}) {
  const available = clients.filter((client) => client.executablePath);
  const allowed = new Set([...available.map((client) => client.id), ...allowedExtra]);
  const tokens = value.trim().toLowerCase().split(/[\s,]+/u).filter(Boolean);
  if (!tokens.length) return [];
  const ids = tokens.length === 1 && tokens[0] === "all" ? [...allowed]
    : tokens.map((token) => /^\d+$/u.test(token) ? available[Number(token) - 1]?.id : token);
  if (ids.some((id) => !allowed.has(id))) throw new Error("Select only detected or previously installed clients by number or name (or all).");
  return [...new Set(ids)];
}

// Removal must work after a host CLI is uninstalled: recorded installs are
// valid choices even when no executable is on PATH.
export function listInstalledClients({ env = process.env, home = os.homedir() } = {}) {
  const paths = resolveLorePaths({ env, home });
  const manifest = readManifest(path.join(paths.loreHome, INSTALL_MANIFEST));
  return Object.keys(manifest.installs).filter((id) => SETUP_CLIENTS.some((client) => client.id === id));
}
...
export function planSetup(ids, { env = process.env, home = os.homedir(), source = SOURCE, shim = true, node = null } = {}) {
  ...
  // Prefer a stable, version-manager-independent node path for hooks/shim so
  // pruning an old pinned version (mise, asdf, fnm, volta, nvm) does not
  // silently break every installed hook. An explicit --node flag wins
  // outright; otherwise LORE_NODE, then a detected stable alias/shim.
  const nodeResolution = node
    ? { path: node, source: "explicit --node option", versionManager: null, pinned: false }
    : resolveStableNodePath({ env });
  const targets = {
  ...
  for (const id of ids) {
    ...
    if (clientKind(id) === "directory") {
      ...
      const fragment = buildCliHookConfig(id, { nodePath: nodeResolution.path, entryPath: path.join(source, "lore-cli.mjs") });
      const installed = manifest.installs?.[id];
      const ownedCommands = installed?.commands;
      const previousTarget = installed?.target;
      const targetMoved = typeof previousTarget === "string" && path.resolve(previousTarget) !== path.resolve(target);
      ...
      // A rerun under a new CODEX_HOME/CLAUDE_CONFIG_DIR must not orphan the
      // old hook file: remove exactly the commands this install owned there
      // before the ownership record moves to the new target. Modified or
      // unrelated content is preserved (or the plan refuses, for hosts such as
      // Antigravity where partial replacement is not safe).
      if (targetMoved && existsSync(previousTarget)) {
        addJson(previousTarget, (value) => mergeCliHookConfig(value, fragment, id, { remove: true, ownedCommands }));
      }
  ...
  return {
    ...
    pathExport: manifest.shim?.pathCopy ? null : (shim ? formatLorePathExport(paths.loreHome) : null),
    node: nodeResolution,
  };
}
...
export function setupTargets(paths, env, home) {
  return {
    copilot: path.join(paths.copilotHome, "extensions", "lore"),
    pi: path.join(env.PI_CODING_AGENT_DIR || path.join(home, ".pi", "agent"), "extensions", "lore"),
    ...hookClientTargets(env, home),
...
// Recorded manifest.commands is the primary way removal recognizes a prior
// Lore install; this is only a fallback for manifests written before that
// field existed (or edited by hand). Match against every node path Lore
// could plausibly have baked in — the raw running node (pre-fix behavior)
// and today's resolved stable path — since either could have been used.
function legacyHookCommands(value, client, { env = process.env } = {}) {
  const found = [];
  const nodePaths = new Set([process.execPath, resolveStableNodePath({ env }).path]);
  const expected = new Set();
  for (const nodePath of nodePaths) {
    for (const command of hookCommands(buildCliHookConfig(client, { nodePath, entryPath: path.join(SOURCE, "lore-cli.mjs") }), client)) expected.add(command);
  }
  const visit = (item) => {
...
export function planRemove(ids, { env = process.env, home = os.homedir() } = {}) {
  ...
  for (const id of ids) {
    ...
    if (clientKind(id) === "directory") {
      ...
      const fragment = buildCliHookConfig(id, { nodePath: resolveStableNodePath({ env }).path, entryPath: path.join(SOURCE, "lore-cli.mjs") });
      const commands = record?.commands?.length ? record.commands : legacyHookCommands(value, id, { env });
      if (!commands.length) {
