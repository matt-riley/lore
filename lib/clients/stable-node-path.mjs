...
const DEFAULT_FS = { accessSync, constants: fsConstants };
...
const DETECTORS = [
  ...
  // nvm's `alias/default` is a version selector, not an executable alias.
  // Resolving it to an installed version directory still bakes in a path
  // that can disappear when nvm prunes that version, so keep process.execPath.
  { id: "nvm", pattern: /[/\\]versions[/\\]node[/\\][^/\\]+[/\\]bin[/\\]node$/u, detect: () => [] },
];
 ...
 * @param {object} [options.fs] - Injectable { accessSync, constants }.
