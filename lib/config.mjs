...
// COHERENCE_COPILOT_HOME overrides the default ~/.copilot home directory.
// Useful for test fixtures, CI, or non-standard installs.
// COHERENCE_CONFIG overrides the config file path directly (takes priority).
const COPILOT_HOME =
  process.env.COHERENCE_COPILOT_HOME?.trim() || path.join(os.homedir(), ".copilot");
const CONFIG_PATH =
  process.env.COHERENCE_CONFIG?.trim() || path.join(COPILOT_HOME, "coherence.json");
