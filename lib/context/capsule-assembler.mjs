...
import { detectPromptContextNeed, extractQueryTerms } from "./prompt-need.mjs";
...
export { detectPromptContextNeed };
...
function fetchStyleContextRows({ db, query: _query, repository, identityName, includeStyleAddressingContext, identityOnly }) {
...
function resolveHistoryHintLines({
  ...
  // Only Copilot's raw SessionStore reader implements searchIndex; native CLI
  // adapters pass an EpisodeSessionSource, which does not. Fail open (no
  // history hints) rather than crash session-start assembly for those hosts.
  const historyLookupEnabled = !!(sessionStore && typeof sessionStore.searchIndex === "function" && query);
...
function resolveRawSessionLines({
  ...
  // Only Copilot's raw SessionStore reader implements getRecentSessions;
  // native CLI adapters pass an EpisodeSessionSource, which does not. Fail
  // open (no raw-session fallback) rather than crash session-start assembly.
  const rawSessionsAvailable = !!sessionStore && typeof sessionStore.getRecentSessions === "function";
  if (!allowRepoLocalTaskContext || episodeLines.length > 0 || !rawSessionsAvailable) {
    recordCapsuleLookup(
      state,
      "rawSessions",
      buildEmptyCapsuleLookup(
        rawSessionsAvailable,
        !allowRepoLocalTaskContext
...
function enforceCapsuleTotalBudget(state, config) {
  const budget = Number(config?.budgets?.total);
  if (!Number.isFinite(budget)) return;
  const bounded = enforceSectionBudget({
    sections: state.sectionDetails,
    totalBudget: budget,
    requiredTitles: REQUIRED_COMPRESSION_TITLES,
  });
  state.sectionDetails = bounded.sections;
  state.sections = bounded.sections.map((section) => section.text);
  state.totalTokens = bounded.estimatedTokens;
  if (state.trace) {
    state.trace = filterTraceIncludedRows(state.trace, bounded.text);
    state.trace.output.sectionTitles = bounded.sections.map((section) => section.title);
    state.trace.output.sectionDetails = bounded.sections.map((section) => ({
      title: section.title,
...
function assembleMemoryCapsuleForQuery({
  ...
  enforceCapsuleTotalBudget(state, config);
...
function enforceFinalCapsuleBudget(result, config) {
  const budget = Number(config?.budgets?.total);
  if (!Number.isFinite(budget) || !result?.text) return result;
  const bounded = enforceSectionBudget({
    sections: sectionsFromRenderedText(result.text, result.trace?.output?.sectionTitles, result.sectionDetails),
    totalBudget: budget,
    requiredTitles: REQUIRED_COMPRESSION_TITLES,
  });
  const trace = result.trace
    ...
    : null;
  if (trace) {
    trace.output.sectionTitles = bounded.sections.map((section) => section.title);
    trace.output.sectionDetails = bounded.sections.map((section) => ({
      title: section.title,
      source: section.source,
      budget: section.budget ?? null,
      usedTokens: estimateTokens(section.text),
    ...
    trace.output.estimatedTokens = bounded.estimatedTokens;
  }
  return {
    ...result,
    text: bounded.text,
...
export async function assembleMemoryCapsule({
  ...
  result = enforceFinalCapsuleBudget(compressed.result, config);
