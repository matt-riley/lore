...
export function enforceSectionBudget({ sections = [], totalBudget = Infinity, requiredTitles = [] } = {}) {
  ...
  // Reserve one entry for every required section as a single allocation. A
  // locally fitting entry can consume the space needed by a later section, so
  // backtrack before falling back to the best partial allocation.
  let searchSteps = 0;
  const reserveRequired = (sectionIndex) => {
    if (sectionIndex >= required.length) return true;
    if (searchSteps >= 10_000) return false;
    const section = required[sectionIndex];
    for (let index = 0; index < section.entries.length; index += 1) {
      searchSteps += 1;
      section.selected.push(index);
      if (estimateTokens(rendered()) <= limit && reserveRequired(sectionIndex + 1)) return true;
      section.selected.pop();
      if (searchSteps >= 10_000) return false;
    }
    return false;
...
function renderedEntrySegments(entry) {
  let value = normalizedText(entry).replace(/^[-*]\s+/u, "");
  value = value.replace(/^\d+[.)]\s+/u, "");
  value = value.replace(/^\d{4}-\d{2}-\d{2}:\s+/u, "");
  value = value.replace(/^\[[^\]]+\]\s+/u, "");
  value = value.replace(/\s+\[(?:example from|from)\s+[^\]]+\]/u, "");
  value = value.replace(/\s+\(`[^`]*`\)$/u, "");
  value = value.replace(/\s+\(-?(?:\d+(?:\.\d+)?|\.\d+)\)$/u, "");
  value = value.replace(/\s+\((?:local:[^()\s]+|[a-z0-9._-]+(?:\/[a-z0-9._-]+)+)\)$/iu, "");
  const segments = value.split(/\s+[—-]\s+|\s+\|\s+/u).map((part) => part.trim()).filter(Boolean);
  return segments.flatMap((part) => {
    const withoutLabel = part.replace(/^(?:decision|open|actions?|themes?|mission|objective|constraints?|blockers?|next actions?|key decisions?):\s+/u, "");
    return withoutLabel === part ? [part] : [part, withoutLabel];
  });
}
...
export function filterTraceIncludedRows(trace, renderedText) {
  ...
  const entries = String(renderedText).replace(/^##[^\n]*$/gmu, "").split(/\n(?=\s*(?:[-*]|\d+[.)])\s+)/u)
    .map((entry) => renderedEntrySegments(entry)).flat();
  const present = (value) => {
    const normalized = normalizedText(value);
    const wanted = [...new Set([normalized, ...renderedEntrySegments(value)])].filter(Boolean);
    return wanted.some((candidate) => {
      const truncated = candidate.length > 180 ? `${candidate.slice(0, 179).trimEnd()}…` : null;
      return entries.some((entry) => entry === candidate || entry === `${candidate}…` || entry === truncated);
    });
  };
