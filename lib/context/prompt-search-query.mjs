...
export function extractMeaningfulPromptTerms(prompt, { maxTerms = 12 } = {}) {
  const terms = [...new Set(String(prompt ?? "").match(/[a-z0-9][a-z0-9_/-]*/gi) ?? [])]
    ...
    .filter(({ raw, term }) => (term.length > 2 || (/^[A-Z][A-Z0-9-]{1,}$/u.test(raw) && !ACRONYM_STOPWORDS.has(raw))) && !SCAFFOLD.has(term))
...
function promptTermForms(term) {
  ...
  const exact = String(term ?? "").toLowerCase();
  const forms = [exact, base];
  if (/[-_/]/u.test(base)) {
