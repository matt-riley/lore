...
import { neutralizeContextMarkup } from "./context-escape.mjs";
import { applyGlobalRelevanceGate } from "./global-relevance-gate.mjs";
import {
...
async function fusePromptContext({
  ...
  try {
    ...
    // Embedding similarity cannot tell a stale global row that merely shares
    // the prompt's task shape ("review this code") from a relevant one, so
    // vector hits face the same global relevance gate as lexical hits.
    const { kept: vectorRows, filtered: vectorFiltered } = applyGlobalRelevanceGate(semantic.rows, prompt);
    const fused = fuseLexicalAndVector(lexical, vectorRows, { limit });
    applyFusedMemories(collected, fused);
    const localLookup = collected.trace?.lookups?.localMemories;
    if (localLookup && vectorFiltered.length > 0) {
      localLookup.filtered = [
        ...(localLookup.filtered ?? []),
        ...vectorFiltered.map((row) => ({ stage: "vector_fusion", reason: "global_relevance_gate", row })),
      ];
    }
    if (collected.trace) {
      collected.trace.lookups = {
        ...
        fusion: {
          ...
          vectorGatedCount: vectorFiltered.length,
...
// Redaction and prompt-injection hardening happen at the single point where
// recall text becomes the payload the agent receives, so every section
// (directives, memories, episodes, overlays) is covered without touching each
// renderer. Rows, traces and the database keep the real content: this is about
// what reaches a model.
function redactRecallPayload(result) {
  if (!result) {
    return result;
  }
  const text = neutralizeContextMarkup(redactSensitiveContent(result.text ?? ""));
  const sections = Array.isArray(result.sections)
    ? result.sections.map((section) => (typeof section === "string"
      ? neutralizeContextMarkup(redactSensitiveContent(section))
      : section))
    : result.sections;
  // sectionDetails is a programmatic field today, but it carries the same text:
  // redact and neutralize it rather than rely on every future consumer not rendering it.
  const sectionDetails = Array.isArray(result.sectionDetails)
    ? result.sectionDetails.map((detail) => (detail && typeof detail.text === "string"
      ? { ...detail, text: neutralizeContextMarkup(redactSensitiveContent(detail.text)) }
      : detail))
