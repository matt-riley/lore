...
const QUERY_EVIDENCE_FIELDS = ["includedRows", "rows", "rankedRows"];

function recallHasQueryEvidence(result) {
...
function collectRenderedPromptContext(db, args) {
  if (typeof db.collectPromptContext === "function") {
    return renderPromptContext(db.collectPromptContext(args));
  }
...
async function assemblePromptRecall({
  ...
  let base = collectRenderedPromptContext(db, collectArgs);
  if (queryExpansion.used && !recallHasQueryEvidence({ trace: base.trace })) {
    base = collectRenderedPromptContext(db, {
      ...collectArgs,
