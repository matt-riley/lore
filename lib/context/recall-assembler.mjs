...
import { FEATURE_PROMPT_VERSION, minDurability, scoreMemoryFeatures, typesafeFeaturesEnabled } from "../inference/typesafe-features.mjs";
...
function isHistoricalExtractorRejection(row) {
  const metadata = row?.metadata && typeof row.metadata === "object" ? row.metadata : {};
  return metadata.source === "rule_extractor"
    && metadata.confidenceBasis === "explicit_rejection_sentence";
}
...
function isAmbientDirectiveEligible(row) {
  if (row?.type !== "directive" || row.scope !== "global") {
    return true;
  }
  const metadata = row.metadata && typeof row.metadata === "object" ? row.metadata : {};
  if (row.scope_source === "manual" || MANUAL_MEMORY_SOURCES.has(metadata.source)) {
    return true;
  }
...
function storedDurability(row) {
  ...
  const value = Number(typesafe.durability);
...
async function filterDirectiveCandidates({ db, rows, config, fetchImpl }) {
  ...
  const unscored = rows.filter((row) => isFilterableDirective(row) && storedDurability(row) === null);
  if (unscored.length > 0 && typeof db.setSemanticMemoryMetadata === "function") {
    const result = await scoreMemoryFeatures({ memories: unscored, config, fetchImpl });
    ...
    for (const feature of result.features) {
      ...
      // Persisting is an optimisation, never a precondition: a locked or
      // read-only store must not turn a scored recall into a failed one.
      let saved = false;
      try {
        saved = db.setSemanticMemoryMetadata(feature.id, { typesafe: annotation }) === true;
      } catch {
        saved = false;
      }
      if (!saved) {
        trace.persistFailures = (trace.persistFailures ?? 0) + 1;
      }
...
async function fetchDirectives({ db, repository, includeOtherRepositories, config, fetchImpl }) {
  ...
  const rejectionRows = db.searchSemantic({
    query: "",
    repository,
    includeOtherRepositories,
    types: ["rejected_approach"],
    limit: 6,
    standingExtractorRejection: true,
  }).filter(isHistoricalExtractorRejection);
  const seen = new Set();
  const rows = [];
  for (const row of [...directiveRows, ...historicalRows, ...rejectionRows]) {
  ...
  const selected = filtered.rows.slice(0, DIRECTIVE_LIMIT);
...
async function assemblePromptRecall({
  ...
  const directives = activePhases.directives === true && need.identityOnly !== true
    ? await fetchDirectives({ db, repository, includeOtherRepositories, config, fetchImpl })
