...
function fetchDirectives({ db, repository, includeOtherRepositories, config }) {
  ...
  if (rows.length === 0) {
    return { rows: [], text: "", trace: { enabled: true, reason: "no_directives_found", rows: [], includedRows: [] } };
  }
  ...
  return {
    rows,
    text: lines.join("\n"),
    trace: { enabled: true, reason: "directives_included", count: rows.length, rows, includedRows: rows },
  };
}
