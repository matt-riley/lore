import { readWorkstreamOverlaysEnabled } from "../rollout/rollout-flags.mjs";
import { sanitizeRetainedList, sanitizeRetainedText } from "../memory/retention-sanitizer.mjs";
...
function shouldRecallWorkstreamOverlay(prompt, promptNeed = null) {
...
export function buildWorkstreamOverlayMemory({
  ...
  return {
    ...
    repository,
    scope,
    domainKey: sanitizeRetainedText(domainKey).toLowerCase() || null,
    confidence,
    sourceSessionId,
...
export function parseWorkstreamOverlayMemory(row) {
  ...
  return {
    ...
    domainKey: sanitizeRetainedText(metadata.domainKey),
...
function buildDisabledWorkstreamResult({ prompt, reason }) {
  return {
    overlays: [],
    text: "",
    trace: {
...
function findWorkstreamCandidateRows({
  db,
  prompt,
  repository,
  ...
  if (rankedRows.length > 0) {
    return {
      matchedRows,
      selectedRows: rankedRows.slice(0, limit),
    };
  }

  const fallbackRows = db.searchSemantic({
    query: "",
    repository,
  ...
  return {
    matchedRows,
    selectedRows: fallbackRows.slice(0, limit),
  };
}

function deriveWorkstreamTraceReason({ overlays, matchedRows }) {
  if (overlays.length === 0) {
    return "no_matching_workstream_overlay";
  }
...
export function findRelevantWorkstreamOverlays({
  ...
  if (!readWorkstreamOverlaysEnabled(config)) {
    return buildDisabledWorkstreamResult({
      prompt,
  ...
  const { matchedRows, selectedRows } = findWorkstreamCandidateRows({
    db,
    prompt,
    repository,
    includeOtherRepositories,
  ...
  return {
    ...
    trace: {
      ...
      reason: deriveWorkstreamTraceReason({ overlays, matchedRows }),
    },
  };
}

function formatOverlayEntry(overlay) {
  const parts = [`- ${overlay.title} [${overlay.status}]`];
  if (overlay.mission) parts.push(`  mission: ${overlay.mission}`);
  if (overlay.objective) parts.push(`  objective: ${overlay.objective}`);
  if (overlay.constraints.length > 0) parts.push(`  constraints: ${overlay.constraints.join(" | ")}`);
  if (overlay.blockers.length > 0) parts.push(`  blockers: ${overlay.blockers.join(" | ")}`);
  if (overlay.nextActions.length > 0) parts.push(`  nextActions: ${overlay.nextActions.join(" | ")}`);
  if (overlay.decisions.length > 0) parts.push(`  decisions: ${overlay.decisions.join(" | ")}`);
  if (overlay.retainPriorities.length > 0) parts.push(`  retainPriorities: ${overlay.retainPriorities.join(" | ")}`);
  if (overlay.reflectPriorities.length > 0) parts.push(`  reflectPriorities: ${overlay.reflectPriorities.join(" | ")}`);
  return parts;
}
...
export function formatWorkstreamOverlaySection(overlays) {
  ...
  for (const overlay of overlays) {
    lines.push(...formatOverlayEntry(overlay));
  }
  return lines.join("\n");
}
