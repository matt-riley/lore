...
export function resolveLoreHomeFromConfig(config, fallback = resolveLorePaths().loreHome) {
  const explicit = config?.paths?.loreHome;
  if (typeof explicit === "string" && explicit.trim()) {
    return path.resolve(explicit);
  }
  const derived = config?.paths?.derivedStorePath;
  if (typeof derived === "string" && derived.trim()) {
    return path.dirname(path.resolve(derived));
  }
  return fallback;
}

export function resolveProposalRoot(config) {
  return path.join(resolveLoreHomeFromConfig(config), "proposals");
}
