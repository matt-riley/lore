...
export function normalizeTraceScalarFields({ id, repository, scopeType, normalizedHook, route, routeReason, contextInjected, latencyMs, promptPreview }) {
  return {
...
export function normalizeTraceJsonFields({ promptNeed, eligibility, lookups, omissions, output, trace }) {
  return {
...
export function listRetrievalTraceSamplesImpl(db, {
  ...
  return rows.map((row) => ({
    ...
    sectionTitles: (() => {
      try {
        const parsed = JSON.parse(row.section_titles_json);
    ...
    promptNeed: (() => {
      try {
        const parsed = JSON.parse(row.prompt_need_json);
    ...
    eligibility: (() => {
      try {
        const parsed = JSON.parse(row.eligibility_json);
        return typeof parsed === "object" && parsed !== null ? parsed : {};
      } catch {
        return {};
      }
    ...
    lookups: (() => {
      try {
        const parsed = JSON.parse(row.lookups_json);
        return typeof parsed === "object" && parsed !== null ? parsed : {};
      } catch {
        return {};
      }
    ...
    omissions: (() => {
      try {
        const parsed = JSON.parse(row.omissions_json);
        return Array.isArray(parsed) ? parsed : [];
      } catch {
        return [];
      }
    ...
    trace: (() => {
      try {
        const parsed = JSON.parse(row.trace_json);
        return typeof parsed === "object" && parsed !== null ? parsed : {};
      } catch {
        return {};
      }
