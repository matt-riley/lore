...
export class LoreDb {
  ...
  getCurrentVersion() {
    const hasVersionTable = this.db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'lore_schema_version'`,
      )
      .get();
    if (!hasVersionTable) {
      return 0;
    }
    const row = this.db.prepare(`SELECT MAX(version) AS version FROM lore_schema_version`).get();
  ...
  runMigrations(currentVersion) {
    ...
    try {
      ...
      this.db.exec(`DELETE FROM lore_schema_version;`);
      this.db.prepare(`INSERT INTO lore_schema_version (version) VALUES (?)`).run(SCHEMA_VERSION);
      this.db.exec("COMMIT");
    } catch (error) {
