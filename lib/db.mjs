...
function addStringFilter(where, params, col, value, transform = (v) => v) {
...
export class LoreDb {
  ...
  getActivityState({ repository = null, includeGlobal = true } = {}) {
    ...
    const repo = normalizeRepository(repository);
    if (repo) {
      const repoRow = queryActivityStateRow(this.db, `repo:${repo}`);
      if (repoRow) {
        rows.push(repoRow);
      }
    }
    if (includeGlobal) {
      const globalRow = queryActivityStateRow(this.db, "global");
      if (globalRow) {
        rows.push(globalRow);
      }
    }
  ...
  buildRetrievalTraceSampleRecord({
    ...
    return {
      ...normalizeTraceScalarFields({
        id, repository, scopeType: this.normalizeRetrievalTraceScopeType(scopeType),
  ...
  findRelevantEpisodesDetailed({ prompt, repository, includeOtherRepositories = false, scopes = [], limit = 5 }) {
    const { primaryTerms, terms, entityTerms, hybridEnabled, effectivePrimaryTerms, effectiveTerms, lexicalQuery } =
