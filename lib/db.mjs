import { promptSearchQuery } from "./prompt-search-query.mjs";
...
import { jsonText } from "./json-text-utils.mjs";
import { clampInteger } from "./numeric-utils.mjs";
import { normalizeRepository } from "./repository-utils.mjs";
import { addStringFilter } from "./sql-filter-utils.mjs";
import { validateRequiredStringField } from "./required-field-utils.mjs";
import {
...
export class LoreDb {
    ...
    try {
      ...
      for (const tableName of tables) {
        const exists = readOnlyDb
          ...
          .get(tableName);
        if (!exists) continue;
        const row = readOnlyDb.prepare(`SELECT MAX(version) AS version, COUNT(*) AS count FROM ${tableName}`).get();
   ...
   */
  listSemanticMemoriesForEmbedding({ types = [], repository = null } = {}) {
    this.ensureOpen();
    const params = [];
    const hasEmbeddingTable = this.tableExists("memory_embedding");
    let sql = `
      SELECT sm.id, sm.type, sm.content, sm.repository, sm.updated_at
      FROM semantic_memory sm
    ...
    if (hasEmbeddingTable) {
      sql = `
        SELECT sm.id, sm.type, sm.content, sm.repository, sm.updated_at,
          me.updated_at AS embedding_updated_at
        FROM semantic_memory sm
        LEFT JOIN memory_embedding me ON me.memory_id = sm.id
        WHERE sm.superseded_by IS NULL
      `;
    }
    if (types.length > 0) {
  ...
  buildPromptSemanticContext({ prompt, repository, limit, identityName, identityOnly }) {
    ...
    if (!identityOnly && memories.length === 0 && contentQuery) {
      memories = this.searchPromptSemanticRows({
        query: contentQuery,
        expandAliases: false,
        repository,
        includeOtherRepositories: false,
