...
function normalizeFtsToken(term) {
...
function extractFtsTerms(query) {
  const directTerms = String(query || "")
    ...
    .toLowerCase()
    .split(/\s+/)
    .map(normalizeFtsToken)
    .filter((term) => term.length > 2)
    .filter((term) => !GENERIC_QUERY_TERMS.has(normalizeMatchTerm(term)));

  const expandedTerms = [];
  for (const term of directTerms) {
    expandedTerms.push(term);
    for (const alias of QUERY_ALIASES[term] ?? []) {
      const normalizedAlias = normalizeFtsToken(alias);
      if (normalizedAlias.length > 2 && !GENERIC_QUERY_TERMS.has(normalizeMatchTerm(normalizedAlias))) {
...
function appendPromptTemporalRecallIntro(lines, trace, {
  ...
  let temporalSource;
  let temporalConfidence;
  let temporalVerifierReason = null;

  if (temporalDate === null) {
    temporalSource = "none";
    temporalConfidence = "none";
    temporalVerifierReason = daySummaryReason;
  } else if (hasIncludedDaySummary) {
    temporalSource = "day_summary";
    temporalConfidence = "high";
  } else if (hasIncludedEpisodes) {
    temporalSource = "episode_fallback";
    temporalConfidence = "medium";
  } else if (temporalVerifierRows.length > 0) {
    temporalSource = "session_store_verifier";
    temporalConfidence = "low";
    temporalVerifierReason = daySummaryReason;
  } else {
    temporalSource = "none";
    temporalConfidence = "none";
    temporalVerifierReason = daySummaryReason === "missing_day_summary" || daySummaryReason === "summary_did_not_match_prompt_terms"
      ? daySummaryReason
      : null;
  }
...
export class LoreDb {
  ...
  adoptLegacyActivityStateTable() {
    if (this.tableExists(PRIMARY_ACTIVITY_STATE_TABLE) || !this.tableExists(LEGACY_ACTIVITY_STATE_TABLE)) {
      return;
    }
    ...
    return [
      {
        ...
        run: () => this.applyScopeMigration(),
      },
      {
        label: "scope-governance",
        shouldRun: (currentVersion) => currentVersion < 5,
        run: () => this.applyScopeGovernanceMigration(),
      },
      {
        label: "growth-memory",
        shouldRun: (currentVersion) => currentVersion < 7,
        run: () => this.applyGrowthMemoryMigration(),
      },
      {
        label: "improvement-backlog",
        shouldRun: (currentVersion) => currentVersion < 8,
        run: () => this.applyImprovementBacklogMigration(),
      },
      {
        label: "lore-visibility-substrate",
        shouldRun: (currentVersion) => currentVersion < 13,
        run: () => this.applyLoreVisibilitySubstrateMigration(),
      },
      {
        label: "memory-domain-observation",
        shouldRun: (currentVersion) => currentVersion < 14,
        run: () => this.applyMemoryDomainObservationMigration(),
      },
      {
        label: "approval-substrate",
        shouldRun: (currentVersion) => currentVersion < 15,
        run: () => this.applyApprovalSubstrateMigration(),
      },
    ];
  }
  ...
  buildMigrationPlan(currentVersion) {
    return [
      ...this.getPreSchemaMigrationSteps(),
      {
        label: "schema-statements",
        shouldRun: () => true,
        run: () => this.applySchemaStatementsMigration(),
      },
      ...this.getPostSchemaMigrationSteps(),
  ...
  runMigrations(currentVersion) {
    ...
    try {
      for (const step of this.buildMigrationPlan(currentVersion)) {
        step.run();
      }
      this.adoptSchemaVersion(SCHEMA_VERSION);
      this.db.exec("COMMIT");
    } catch (error) {
  ...
  applyGrowthMemoryMigration() {
    this.prepareGrowthMemoryMigration();
    this.backfillGrowthMemoryCanonicalKeys();
    this.mergeGrowthMemoryDuplicateUserIdentityRows();
  }
  ...
  backfillGrowthMemoryCanonicalKeys() {
    const semanticRows = this.listGrowthMemoryRowsForCanonicalBackfill();
  ...
  mergeGrowthMemoryDuplicateUserIdentityRows() {
    ...
    for (const { canonical_key: canonicalKey } of duplicateKeys) {
      const candidates = this.listGrowthMigrationCandidates(canonicalKey);
      if (candidates.length <= 1) {
        continue;
      }

      const [winner, ...losers] = candidates;
      const mergedState = this.mergeGrowthMigrationCandidates(candidates, winner);
      this.updateGrowthMigrationWinner(winner.id, mergedState);
      this.supersedeGrowthMigrationLosers(winner.id, losers);
    }
  }
  ...
  mergeGrowthMigrationCandidates(candidates, winner) {
    const initialLastSeen = this.getGrowthMigrationLastSeen(winner) || nowIso();
    const initialConfidence = Number.isFinite(winner?.confidence) ? winner.confidence : 1.0;
    return candidates.reduce((state, candidate) => {
      const nextLastSeen = this.getGrowthMigrationLastSeen(candidate);
      return {
        ...
        latestSeen: nextLastSeen && nextLastSeen > state.latestSeen
          ? nextLastSeen
          : state.latestSeen,
        mergedTags: mergeTagText(state.mergedTags, candidate?.tags),
        mergedMetadata: {
  ...
  applyApprovalSubstrateMigration() {
    this.db.exec(`
    ...
    `);
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_action_approval_family_repo
        ON action_approval(action_family, repository, decision, updated_at DESC);
    `);
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_action_approval_family_target
        ON action_approval(action_family, target_identity, decision, updated_at DESC);
    `);
  }

  applyMemoryDomainObservationMigration() {
    if (this.tableExists("semantic_memory")) {
      this.ensureColumn("semantic_memory", "domain_key", "TEXT");
      this.db.exec(`
  ...
  getStats() {
    this.ensureOpen();
    const semanticCount = readTableCount(this.db, "semantic_memory");
    const episodeCount = readTableCount(this.db, "episode_digest");
  ...
  serializeActivityFallbackContext(latestContextRow) {
    return {
      ...
      last_context_injection_sections_json: JSON.stringify(parseJsonArray(latestContextRow?.section_titles_json)),
  ...
  buildRetrievalTraceSampleRecord({
    ...
    const normalizedHook = this.normalizeRetrievalTraceHook(hook);
    if (!normalizedHook) {
    ...
    return {
      ...
      scopeType: this.normalizeRetrievalTraceScopeType(scopeType),
      hook: normalizedHook,
      ...
      sectionTitles: this.normalizeRetrievalTraceSectionTitles(sectionTitles),
  ...
  buildBackfillRunSummaryUpdate(runId, { lastError = null } = {}) {
    const counts = this.getBackfillRunCounts(runId);
    const status = this.deriveBackfillRunStatus(counts);
    return {
      status,
      ...
      updatedAt: nowIso(),
      lastError: this.deriveBackfillRunLastError(runId, lastError),
    };
  }
  ...
  explainPromptContext({
    ...
    const {
      ...
      allowCrossRepoFallback,
      identityOnly,
    } = this.buildPromptContextFlags(need);
    const {
      memories,
      identityMemories,
      localMemories,
      assistantPersonaRows,
      relationshipPreferenceRows,
    } = this.buildPromptSemanticContext({
      prompt,
      repository,
      limit,
      identityName,
    ...
    const {
      ...
      episodes,
      effectiveStyleSection,
    } = this.buildPromptTemporalContext({
      prompt,
      repository,
      need,
      limit,
    ...
    const {
      ...
      crossRepoEpisodes,
      crossRepoHints,
    } = this.buildPromptCrossRepoContext({
      lexicalPrompt,
      repository,
      allowCrossRepoFallback,
      pureTemporalRecall,
    ...
    const lines = [];
    const trace = createPromptContextTrace({
      prompt,
      repository,
      allowCrossRepoFallback,
      allowGenericCrossRepoFallback,
    ...
    appendPromptDaySummarySection(lines, trace, {
      repository,
      includedDaySummaryRows,
      daySummaryReason,
      temporalDate,
    });
    appendPromptEpisodesSection(lines, trace, {
      episodes,
      renderTerms,
      allowRepoLocalTaskContext,
      episodeDetails,
    ...
    appendPromptLocalMemoriesSection(lines, trace, localMemories, identityOnly);
    ...
    appendPromptCrossRepoPreferencesSection(lines, trace, {
      crossRepoPreferences,
      allowGenericCrossRepoFallback,
      pureTemporalRecall,
    });
