...
export class LoreDb {
  ...
  runMigrations(currentVersion) {
    ...
    try {
      ...
      if (currentVersion < 13) {
        this.applyLoreVisibilitySubstrateMigration();
      }
      if (currentVersion < 14) {
        this.applyMemoryDomainObservationMigration();
      }
      if (currentVersion < 15) {
        this.applyApprovalSubstrateMigration();
      }
  ...
  getStats() {
    ...
    const domainCount = this.db.prepare(`SELECT COUNT(*) AS count FROM memory_domain`).get().count;
    const observationCount = this.db.prepare(`SELECT COUNT(*) AS count FROM refreshable_observation`).get().count;
  ...
  insertSemanticMemory(memory) {
    ...
    const domainKey = normalizeText(memory.domainKey).toLowerCase() || null;
  ...
  explainPromptContext({
    ...
    const lexicalPrompt = promptTerms.length > 0 ? promptTerms.join(" ") : prompt;
    ...
    const trace = {
      ...
      lookups: {
        ...
        temporalVerifier: {
          enabled: !!sessionStore && temporalDate !== null && pureTemporalRecall,
          date: temporalDate,
          rows: [],
          includedRows: [],
          reason: null,
    ...
    let daySummaryReason = null;
    if (!need.hasTemporalSignal) {
      daySummaryReason = "no_temporal_signal";
    } else if (temporalDate === null) {
      daySummaryReason = "unresolved_temporal_date";
    } else if (!hasIncludedDaySummary && daySummaryRows.length === 0) {
      daySummaryReason = "missing_day_summary";
    } else if (!hasIncludedDaySummary) {
      daySummaryReason = "summary_did_not_match_prompt_terms";
    }
    ...
    } else if (daySummaryReason === "no_temporal_signal") {
      trace.lookups.daySummary.reason = "no_temporal_signal";
      trace.omissions.push({ stage: "day_summary", reason: "no_temporal_signal" });
    } else if (daySummaryReason === "unresolved_temporal_date") {
      trace.lookups.daySummary.reason = "unresolved_temporal_date";
      trace.omissions.push({ stage: "day_summary", reason: "unresolved_temporal_date" });
    } else if (daySummaryReason === "missing_day_summary") {
    ...
    if (temporalVerifierRows.length > 0) {
      ...
      lines.push("## Verified Session History", "");
