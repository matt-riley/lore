 ...
 * rows whose stored extractorVersion is missing or older than the current
 * one -- exactly the rows extraction-revalidation.mjs's pure classifier
 * should replay the grammar against. Manual/explicit writes never match:
 * scope_source = 'manual' or a non rule_extractor metadata.source excludes
 * them, mirroring isExplicitLifecycleWrite (db-memory-lifecycle.mjs).
 */
export function listExtractionRevalidationCandidates(owner, {
  ...
  let scopeSql = "1 = 1";
  if (repo) {
    scopeSql = "(scope = 'global' OR repository = ?)";
    params.push(repo);
  ...
  const rows = owner.db.prepare(`
    ...
    WHERE superseded_by IS NULL
      ...
      AND COALESCE(json_extract(metadata_json, '$.source'), '') = 'rule_extractor'
      AND (
...
export function reclassifyExtractionRevalidationMemory(owner, {
  ...
  const timestamp = nowIso();
  const result = owner.db.prepare(`
    UPDATE semantic_memory SET type = ?, updated_at = ? WHERE id = ? AND superseded_by IS NULL
  `).run(nextType, timestamp, id);
  if (result.changes === 0) {
    return false;
  }
...
export function rollbackExtractionRevalidation(owner, { marker, actor, reason } = {}) {
  ...
  const auditRows = owner.db.prepare(`
    SELECT id, target_id, action, previous_scope, next_scope, previous_repository
  ...
  for (const row of auditRows) {
    if (row.action === "extractor_revalidation_reclassify") {
      owner.db.prepare("UPDATE semantic_memory SET type = ?, updated_at = ? WHERE id = ?")
        .run(row.previous_scope, timestamp, row.target_id);
      owner.insertScopeOverrideAudit({
        targetType: "semantic",
      ...
      owner.db.prepare(`
        ...
        WHERE id = ?
      `).run(row.previous_scope, row.previous_repository, timestamp, row.target_id);
      owner.insertScopeOverrideAudit({
        targetType: "semantic",
        targetId: row.target_id,
        action: "extractor_revalidation_rollback",
