...
export function reclassifyExtractionRevalidationMemory(owner, {
  ...
  const result = owner.db.prepare(`
    ...
    SET type = ?,
        ...
        updated_at = ?
    WHERE id = ? AND superseded_by IS NULL
  `).run(nextType, extractorVersion, timestamp, id);
  if (result.changes === 0) {
    return false;
  }
  ...
  for (const row of auditRows) {
    if (row.action === "extractor_revalidation_reclassify") {
      const result = owner.db.prepare(`
        UPDATE semantic_memory
        SET type = ?,
            metadata_json = json_remove(COALESCE(metadata_json, '{}'), '$.extractorVersion'),
            updated_at = ?
        WHERE id = ? AND type = ? AND superseded_by IS NULL
      `).run(row.previous_scope, timestamp, row.target_id, row.next_scope);
