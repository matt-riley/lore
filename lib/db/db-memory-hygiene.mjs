...
export function restoreMemoriesBySupersessionMarker(owner, marker) {
  ...
  const restore = () => {
    ...
    // Only suppressions written by this exact hygiene run are reversed; a
    // manual forget (different actor/reason, or an earlier legacy row) keeps
    // blocking retrieval.
    ...
    for (const row of rows) {
      deleteSuppression.run(row.id, "memory_hygiene", normalizedMarker);
    }
  };
