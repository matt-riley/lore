...
export class MigrationRunner {
  ...
  getCurrentVersionInfo() {
    for (const tableName of [PRIMARY_SCHEMA_VERSION_TABLE, ...LEGACY_SCHEMA_VERSION_TABLES]) {
      ...
      if (version > SCHEMA_VERSION) {
        const dbPath = this.config?.paths?.derivedStorePath ?? "<unknown path>";
        throw new Error(
          `unsupported future Lore schema version ${version} (supported through ${SCHEMA_VERSION}) at ${dbPath}`,
        );
      }
  ...
  getPreSchemaMigrationSteps(api = this) {
    return [
      {
        label: "memory-domain-column",
        shouldRun: (currentVersion) => currentVersion > 0 && currentVersion < 14,
        run: () => {
  ...
  getPostSchemaMigrationSteps(api = this) {
    return [
      ...
      {
        label: "session-evidence-record-index",
        shouldRun: (currentVersion) => currentVersion < 20,
        run: () => api.applySessionEvidenceRecordIndexMigration(),
      },
      {
        label: "scope-override-audit-metadata",
        shouldRun: (currentVersion) => currentVersion < 21,
        run: () => api.ensureColumn("scope_override_audit", "previous_metadata_json", "TEXT"),
      },
    ];
  }
   ...
   * @returns {void}
   */
  runMigrations(currentVersion, api = this, options = {}) {
    if (currentVersion > SCHEMA_VERSION) {
      const dbPath = this.config?.paths?.derivedStorePath ?? "<unknown path>";
      throw new Error(
        `unsupported future Lore schema version ${currentVersion} (supported through ${SCHEMA_VERSION}) at ${dbPath}`,
      );
    }
    if (currentVersion >= SCHEMA_VERSION) {
      return;
  ...
  /**
   * Rebuilds the session evidence record index so existing databases pick up
   * the retired_at column added to the schema definition.
   *
   * @param {MigrationApi} [api=this]
   * @returns {void}
   */
  applySessionEvidenceRecordIndexMigration(api = this) {
    if (!api.tableExists("session_evidence")) {
      return;
    }
    this.db.exec(`
      DROP INDEX IF EXISTS idx_session_evidence_record;
      CREATE INDEX idx_session_evidence_record
        ON session_evidence(session_id, source_record_id, retired_at);
    `);
  }
