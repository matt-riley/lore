...
import { extractFtsTerms as extractNormalizedFtsTerms, extractDirectTerms as extractNormalizedDirectTerms, extractTemporalContentTerms as extractNormalizedTemporalContentTerms, GENERIC_TASK_REQUEST_TERMS, inferDateFromPrompt as inferNormalizedDateFromPrompt, normalizeFtsToken, QUERY_ALIASES, STOPWORDS, tokenizeText } from "../utils/query-normalizer.mjs";
  ...
  return {
    ...
    lookups: {
      localMemories: {
        ...
        filtered: globalRelevanceFiltered.map((row) => ({
          stage: "local_memories",
          reason: "global_relevance_gate",
          row: serializeSemanticTraceRow(row, repository),
        })),
      },
      identityMemories: {
        query: identityName ?? "",
 ...
 * applied independently during RRF fusion in recall-assembler.mjs, so a row
 * this gate drops can still be re-admitted there on vector similarity.
 */
...
function applyGlobalRelevanceGate(rows, prompt) {
...
function buildExplainPromptTrace({ prompt, repository, allowCrossRepoFallback, promptTerms, identityName, semanticCtx, temporalCtx, crossRepoCtx, sessionStore }) {
  return createPromptContextTrace({
    ...
    globalRelevanceFiltered: semanticCtx.globalRelevanceFiltered ?? [],
...
export function buildPromptSemanticContext(owner, { prompt, repository, limit, identityName, identityOnly }) {
  ...
  const mergedMemories = dedupeSemanticContextRows([
    ...identityMemories,
    ...memories,
  ]);
  const { kept: localMemories, filtered: globalRelevanceFiltered } = applyGlobalRelevanceGate(mergedMemories, prompt);
