// Read-only database statistics. Inputs are explicit; this module owns no connection or lifecycle.
...
function buildLoreStatsPayload({
  ...
  return {
    ...
    lastSuccessActivity: latestActivity ?? null,
  };
}
...
export function readLoreStats({ db, config, lastBackupPath, schemaVersion, latestActivity }) {
  const { semanticCount, semanticGrowth, semanticScopes } = querySemanticMemoryStats(db);
  const { episodeCount, episodeScopes, daySummaryCount } = queryEpisodeDigestStats(db);
  const domainCount = readTableCount(db, "memory_domain");
  const observationCount = readTableCount(db, "refreshable_observation");
  const { deferredCounts, backfillCounts } = queryExtractionJobStats(db);
  const overrideAuditCount = readTableCount(db, "scope_override_audit");
  const improvementCounts = queryImprovementBacklogStats(db);
  const { maintenanceCounts, maintenanceLatest, maintenanceTaskCount } = queryMaintenanceRunStats(db);
  const { trajectoryCounts, intentJournalCounts, retrievalTraceSampleCounts } = querySignalStats(db);
