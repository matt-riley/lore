...
export function buildSemanticEligibilitySql({
  ...
  if (includeOtherRepositories && !transferableFallback) {
    ...
    scopeSql = `((${column("scope")} = '${MEMORY_SCOPE.GLOBAL}' AND (${column("repository")} IS NULL OR ${column("repository")} = '')) OR ${column("repository")} = ?${transferableFallback ? ` OR ${column("scope")} = '${MEMORY_SCOPE.TRANSFERABLE}'` : ""})`;
  } else {
