...
// Connection-local pure functions allow SQLite to filter suppressed evidence
// before pagination without materializing the complete suppression ledger.
export function registerRetrievalPolicyFunctions(connection) {
  if (registeredConnections.has(connection)) return;
  connection.function("lore_fingerprint", { deterministic: true }, (type, value) =>
    value == null ? null : hash([type ?? "", value].join("\0")),
  );
  connection.function("lore_evidence_fingerprint", { deterministic: true }, (value) =>
    value == null ? null : isSha256(value) ? value : hash(value),
  );
  registeredConnections.add(connection);
}

const MANUAL_SOURCES = new Set(["memory_save", "lore_retain", "onboarding", "pi", "pi:command"]);
...
export function buildSemanticEligibilitySql({
  ...
  let scopeSql;
  if (includeOtherRepositories && !transferableFallback) {
    ...
    scopeSql = `((${column("scope")} = '${MEMORY_SCOPE.GLOBAL}' AND (${column("repository")} IS NULL OR ${column("repository")} = '')) OR ${column("repository")} = ? OR ${column("repository")} IN (SELECT legacy FROM repository_identity_mapping WHERE canonical = ?)${transferableFallback ? ` OR ${column("scope")} = '${MEMORY_SCOPE.TRANSFERABLE}'` : ""})`;
  } else {
  ...
  return {
    sql: `${column("superseded_by")} IS NULL
      ...
      AND (
        ...
        OR COALESCE(json_extract(${column("metadata_json")}, '$.source'), '') IN ('memory_save', 'lore_retain', 'onboarding', 'pi', 'pi:command')
        OR NOT EXISTS (
          ...
          WHERE policy_ms.superseded_at IS NULL
            ...
            AND policy_ms.scope = ${column("scope")}
            AND IFNULL(policy_ms.repository, '') = IFNULL(${column("repository")}, '')
      ...
      AND (
        ...
        OR (
          NOT EXISTS (
            SELECT 1 FROM memory_suppression policy_ms
            WHERE policy_ms.superseded_at IS NULL AND COALESCE(policy_ms.repair_candidate, 0) = 0
              AND policy_ms.scope = ${column("scope")}
              AND IFNULL(policy_ms.repository, '') = IFNULL(${column("repository")}, '')
              AND policy_ms.canonical_fingerprint = lore_fingerprint(${column("type")}, ${column("canonical_key")})
          )
          AND NOT EXISTS (
            SELECT 1 FROM memory_suppression policy_ms
            WHERE policy_ms.superseded_at IS NULL AND COALESCE(policy_ms.repair_candidate, 0) = 0
              AND policy_ms.scope = ${column("scope")}
              AND IFNULL(policy_ms.repository, '') = IFNULL(${column("repository")}, '')
              AND policy_ms.evidence_fingerprint IN (
                CASE WHEN ${column("canonical_key")} IS NULL THEN lore_fingerprint(${column("type")}, ${column("content")}) END,
                lore_evidence_fingerprint(json_extract(${column("metadata_json")}, '$.evidence.key')),
                lore_evidence_fingerprint(json_extract(${column("metadata_json")}, '$.evidence.contentHash')),
                lore_evidence_fingerprint(json_extract(${column("metadata_json")}, '$.evidence.propositionFingerprint'))
              )
          )
        )
      )
...
export function buildScopeEligibilitySql({ alias = "row", repository = null, includeOtherRepositories = false } = {}) {
  ...
  if (repository) return {
    sql: `((${col("scope")} = '${MEMORY_SCOPE.GLOBAL}' AND (${col("repository")} IS NULL OR ${col("repository")} = '')) OR ${col("repository")} = ? OR ${col("repository")} IN (SELECT legacy FROM repository_identity_mapping WHERE canonical = ?))`,
