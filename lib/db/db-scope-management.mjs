...
export function ensureScopeOverrideAuditMetadataColumn(owner) {
  try {
    owner.ensureColumn("scope_override_audit", "previous_metadata_json", "TEXT");
  } catch (error) {
    // Another process may add this lazy, backwards-compatible column at the
    // same time. Treat that race as success once the column is present.
    if (!owner.tableHasColumn("scope_override_audit", "previous_metadata_json")) {
      throw error;
    }
  }
}
...
export function insertScopeOverrideAudit(owner, {
  ...
  const normalizedPreviousRepository = normalizeRepository(previousRepository);
  const normalizedNextRepository = normalizeRepository(nextRepository);
  const createdAt = nowIso();
  if (previousMetadataJson !== undefined) {
    ...
    return owner.db.prepare(`
      INSERT INTO scope_override_audit (
        ...
        previous_repository, next_repository, previous_metadata_json,
    ...
    `).run(
      ...
      previousScope ?? null,
      nextScope ?? null,
      normalizedPreviousRepository,
      normalizedNextRepository,
      previousMetadataJson,
      actor,
      reason,
      source,
