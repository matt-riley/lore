...
/**
 * Merge a patch into a memory's metadata without touching updated_at: derived
 * annotations (evidence features) must not re-rank a row's recency.
 * Returns false when the memory is missing or the patch is unusable.
 */
export function setSemanticMemoryMetadata(owner, memoryId, patch) {
  owner.ensureOpen();
  const id = typeof memoryId === "string" ? memoryId.trim() : "";
  if (!id || !patch || typeof patch !== "object" || Array.isArray(patch)) {
    return false;
  }
  const row = owner.db.prepare("SELECT metadata_json FROM semantic_memory WHERE id = ?").get(id);
  if (!row) {
    return false;
  }
