...
export class LoreDb {
   ...
   */
  listSemanticMemoriesForEmbedding({ types = [], scopes = [], repository = null, includeOtherRepositories = false, transferableFallback = false, limit = 256, offset = null, after = null, stableOrder = false, now = new Date() } = {}) {
    this.ensureOpen();
    const params = [];
    ...
    if (after && typeof after === "object") {
      ...
      } else if (hasEmbeddingTable) {
        sql += ` AND (
          (CASE WHEN me.memory_id IS NULL THEN 0 ELSE 1 END) > ?
          OR ((CASE WHEN me.memory_id IS NULL THEN 0 ELSE 1 END) = ? AND (
            COALESCE(me.updated_at, '') > ?
            OR (COALESCE(me.updated_at, '') = ? AND (
              sm.updated_at > ?
              OR (sm.updated_at = ? AND sm.id > ?)
            ))
          ))
        )`;
        params.push(
          Number(after.cacheRank) || 0,
          Number(after.cacheRank) || 0,
          after.embeddingUpdatedAt ?? "",
          after.embeddingUpdatedAt ?? "",
          after.updatedAt ?? "",
          after.updatedAt ?? "",
          after.id ?? "",
    ...
    sql += stableOrder
      ...
      : hasEmbeddingTable
      ? " ORDER BY CASE WHEN me.memory_id IS NULL THEN 0 ELSE 1 END, COALESCE(me.updated_at, ''), sm.updated_at ASC, sm.id ASC"
      : " ORDER BY sm.updated_at ASC, sm.id ASC";
    sql += " LIMIT ?";
