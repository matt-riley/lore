import { readLoreStats } from "./db-reporting.mjs";
...
export class LoreDb {
  ...
  getStats() {
    this.ensureOpen();
    const activity = queryActivityStateRow(this.db, "global");
    return readLoreStats({
      db: this.db,
      config: this.config,
      lastBackupPath: this.lastBackupPath,
      schemaVersion: this.getCurrentVersion(),
      latestActivity: activity ? mapActivityStateRow(activity) : null,
    });
  }
  ...
  searchSemantic({
    ...
    standingExtractorRejection = false,
    now = new Date(),
  }) {
    this.ensureOpen();
    ...
    const runSearch = (ftsQuery, effectiveLimit = limit, excludedIds = []) => {
      ...
      for (let offset = 0; ; offset += pageSize) {
        ...
        if (standingExtractorRejection) {
        sql += ` AND COALESCE(json_extract(sm.metadata_json, '$.source'), '') = 'rule_extractor'
          AND COALESCE(json_extract(sm.metadata_json, '$.confidenceBasis'), '') = 'explicit_rejection_sentence' `;
        }
