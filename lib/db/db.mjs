import { promptSearchQuery, extractMeaningfulPromptTerms, scorePromptFallbackRows } from "../context/prompt-search-query.mjs";
...
export class LoreDb {
  ...
  searchSemantic({
    ...
    const runSearch = (ftsQuery, effectiveLimit = limit, excludedIds = []) => {
      ...
      for (let offset = 0; ; offset += pageSize) {
        ...
        const page = this.db.prepare(`${sql} LIMIT ? OFFSET ?`).all(...params);
  ...
  listSemanticMemoriesForEmbedding({ types = [], scopes = [], repository = null, includeOtherRepositories = false, transferableFallback = false, embeddingState = "all", limit = 256, offset = null, after = null, stableOrder = false, now = new Date() } = {}) {
    ...
    if (repository && !includeOtherRepositories) {
      sql += ` AND (sm.repository IS NULL OR sm.repository = ?)`;
  ...
  searchPromptSemanticFallback({
    ...
    const terms = extractMeaningfulPromptTerms(prompt);
    if (terms.length === 0) return [];
    // The primary prompt query intentionally stays strict FTS. Once it has
    // missed, scan a bounded eligible slice and score meaningful terms so
    // natural questions can match singular/plural variants and paraphrased
    // wording without weakening explicit memory_search.
    let rows = this.searchSemantic({
      query: "",
      repository,
      includeOtherRepositories,
      types,
      scopes,
      limit: Math.max(limit * 16, 128),
      expandAliases: false,
      now,
    });
    let scored = scorePromptFallbackRows(rows, terms, { limit });
    const matchedSourceSessions = new Set(scored.map((row) => row.source_session_id).filter(Boolean));
    if (matchedSourceSessions.size > 0) {
      const selectedIds = new Set(scored.map((row) => row.id));
      const related = rows
        .filter((row) => row.source_session_id && matchedSourceSessions.has(row.source_session_id) && !selectedIds.has(row.id))
        .slice(0, Math.max(0, limit - scored.length))
        .map((row) => ({ ...row, promptScore: 0, matchedTerms: [] }));
      scored = [...scored, ...related];
    }
    if (scored.length === 0 && includeGlobalFallback) {
      rows = this.searchSemantic({
        query: "",
        repository,
        includeOtherRepositories: false,
        types,
      ...
      scored = rows.slice(0, limit).map((row) => ({ ...row, promptScore: 0, matchedTerms: [] }));
    }
    return scored;
  }

  buildPromptSemanticContext({ prompt, repository, limit, identityName, identityOnly, allowGlobalFallback = false }) {
    ...
    if (!identityOnly && memories.length === 0 && contentQuery) {
      memories = this.searchPromptSemanticFallback({
        prompt,
        repository,
        includeOtherRepositories: false,
        types: ["commitment", "open_loop", "rejected_approach", "blocker", "user_preference", "assistant_identity", "user_identity", "assistant_goal", "recurring_mistake"],
        limit,
        includeGlobalFallback: allowGlobalFallback,
      });
    }
  ...
  explainPromptContext({
    ...
    const semanticCtx = this.buildPromptSemanticContext({ prompt, repository, limit, identityName, identityOnly, allowGlobalFallback: need.allowCrossRepoFallback === true });
