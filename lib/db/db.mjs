...
import { buildSemanticEligibilitySql, buildScopeEligibilitySql, isSemanticMemoryRowEligible } from "./db-retrieval-policy.mjs";
...
export class LoreDb {
  ...
  searchSemantic({
    ...
    const repo = normalizeRepository(repository);
    const suppressionRows = this.listActiveMemorySuppressions();
   ...
   */
  listSemanticMemoriesForEmbedding({ types = [], scopes = [], repository = null, includeOtherRepositories = false, limit = 256, offset = 0, stableOrder = false, now = new Date() } = {}) {
    this.ensureOpen();
    const params = [];
    const hasEmbeddingTable = this.tableExists("memory_embedding");
    const columns = `sm.id, sm.type, sm.content, sm.repository, sm.scope, sm.scope_source, sm.updated_at,
      sm.expires_at${hasEmbeddingTable ? ", me.updated_at AS embedding_updated_at, me.vector, me.content_hash, me.provider, me.model, me.dimensions" : ""}`;
    let sql = `SELECT ${columns} FROM semantic_memory sm ${hasEmbeddingTable ? "LEFT JOIN memory_embedding me ON me.memory_id = sm.id" : ""}`;
    const eligibility = buildSemanticEligibilitySql({ alias: "sm", repository: normalizeRepository(repository), includeOtherRepositories, now });
    sql += ` WHERE ${eligibility.sql}`;
    params.push(...eligibility.params);
    if (types.length > 0) {
      sql += ` AND sm.type IN (${types.map(() => "?").join(", ")})`;
    ...
    sql += stableOrder
      ...
      : hasEmbeddingTable
      ? " ORDER BY CASE WHEN me.memory_id IS NULL THEN 0 ELSE 1 END, me.updated_at ASC, sm.updated_at ASC, sm.id ASC"
      : " ORDER BY sm.updated_at ASC, sm.id ASC";
    sql += " LIMIT ? OFFSET ?";
    params.push(Math.max(1, Math.min(256, Math.trunc(Number(limit) || 256))), Math.max(0, Math.trunc(Number(offset) || 0)));
    return this.db.prepare(sql).all(...params);
  }
  ...
  searchPromptSemanticFallback({
    ...
    return scorePromptFallbackRows([...byId.values()], terms, { limit });
  }
