...
import { acquireRestoreGuard, waitForRestoreLockClear } from "./db-restore-guard.mjs";
...
import {
  ensureMaintenanceLockTable,
  acquireMaintenanceLock,
  releaseMaintenanceLock,
} from "./db-maintenance-lock.mjs";
import {
...
import {
  demoteExtractionRevalidationMemory,
  listExtractionRevalidationCandidates,
  reclassifyExtractionRevalidationMemory,
  rollbackExtractionRevalidation,
  supersedeExtractionRevalidationMemory,
} from "./db-extraction-revalidation.mjs";
import {
  hasEpisodeDigest,
  upsertEpisodeDigest,
  refreshDaySummary,
...
const WAL_SWITCH_DEADLINE_MS = 5000;

// Switching a rollback-journal store (fresh or legacy) to WAL escalates a
// shared lock straight to exclusive. When another process is escalating at
// the same time SQLite reports BUSY immediately (deadlock avoidance) without
// consulting the busy handler, so retry briefly instead of failing the hook.
// Once the file is in WAL mode this pragma is a cheap no-op.
function enableWalMode(db) {
  const deadline = Date.now() + WAL_SWITCH_DEADLINE_MS;
  ...
  for (;;) {
    try {
      ...
      const busy = error?.errcode === 5 || (error?.errcode & 0xff) === 5 || /database is locked/i.test(String(error?.message));
      if (!busy || Date.now() >= deadline) throw error;
      Atomics.wait(pause, 0, 0, 10 + Math.floor(Math.random() * 20));
    }
  }
}
...
export class LoreDb {
  ...
  openReadOnly() {
    ...
    if (!existsSync(dbPath)) {
      const error = new Error(`Lore database is unavailable at ${dbPath}; previews cannot create a store. Run a write command (for example \`lore status\`) to initialize it, then retry.`);
      error.code = "DATABASE_UNAVAILABLE";
      throw error;
    }
    waitForRestoreLockClear(dbPath);
    this.preflightSchemaVersion(dbPath);
    this.db = new DatabaseSync(dbPath, { readOnly: true, timeout: 5000 });
    this.readOnly = true;
    this.migrations = new MigrationRunner(this.db, this.config);
    try {
      ...
      if (currentVersion !== SCHEMA_VERSION) {
        const error = new Error(`Lore schema upgrade required before this preview at ${dbPath} (found v${currentVersion}, expected v${SCHEMA_VERSION}). Run a write command (for example \`lore status\`) to migrate it, then retry.`);
  ...
  openDatabase() {
    ...
    // The busy timeout must apply before the journal_mode pragma below:
    // converting a fresh or legacy store to WAL takes an exclusive lock, and
    // a concurrent opener without a timeout would fail with BUSY at once.
  ...
  initialize() {
    ...
    this.openDatabase();
    try {
      // Fast path: every native hook call from every concurrent session runs
      // initialize(), and almost all of the time the schema is already
      // current, so there is nothing to protect with a writer lock. Take a
      // plain (deferred) read of the same three conditions the locked path
      // re-checks below — no BEGIN IMMEDIATE, so this never contends with a
      // concurrent RESERVED/EXCLUSIVE holder. The read can only be stale in
      // the safe direction: schema versions move forward and legacy-table
      // adoption is one-directional, so a concurrent migration landing
      // between this read and now can make the locked path see MORE work
      // than we do here, never less. If we see no work, there genuinely is
      // none to do, and it is safe to return without ever taking the lock.
      const fastVersionInfo = this.getCurrentVersionInfo();
      const fastAdoptsSchemaVersion = fastVersionInfo.version > 0
        && fastVersionInfo.tableName
        && fastVersionInfo.tableName !== PRIMARY_SCHEMA_VERSION_TABLE;
      const fastAdoptsActivityState = !this.tableExists("lore_activity_state")
        && this.tableExists("coherence_activity_state");
      const fastNeedsWork = fastAdoptsSchemaVersion || fastAdoptsActivityState || fastVersionInfo.version < SCHEMA_VERSION;
      if (!fastNeedsWork) {
        return { backupPath: this.lastBackupPath };
      }
  ...
  preflightSchemaVersion(dbPath) {
    ...
    // Wait out a concurrent migration or WAL conversion (both hold an
    // exclusive lock briefly) instead of failing the whole hook with BUSY.
    const readOnlyDb = new DatabaseSync(dbPath, { readOnly: true, timeout: 5000 });
    try {
  ...
  backupDatabase() {
    ...
    // Concurrent upgrades from several hook processes can land in the same
    // millisecond; a per-process suffix keeps each VACUUM INTO target unique
    // so no process deletes or collides with another's in-progress backup.
    const suffix = `${process.pid}-${crypto.randomUUID().slice(0, 8)}`;
    const backupPath = path.join(backupDir, `lore-${timestamp}-${suffix}.db`);
    this.ensureOpen();
  ...
  restoreFromBackup(backupPath) {
    ...
    try {
      ...
      // Prove no other process has dbPath open (mid-transaction or even just
      // holding a read snapshot) before we start renaming files out from
      // under it. See db-restore-guard.mjs for how this differs from (and
      // improves on) a plain writer-lock probe, and for the residual race
      // window this cannot close.
      let guard;
      try {
        guard = acquireRestoreGuard(dbPath);
      } catch (error) {
  ...
  ensureMaintenanceLockTable(...args) {
    return ensureMaintenanceLockTable(this, ...args);
  }

  acquireMaintenanceLock(...args) {
    return acquireMaintenanceLock(this, ...args);
  }

  releaseMaintenanceLock(...args) {
    return releaseMaintenanceLock(this, ...args);
  }
  ...
  listExtractionRevalidationCandidates(...args) {
    return listExtractionRevalidationCandidates(this, ...args);
  }

  supersedeExtractionRevalidationMemory(...args) {
    return supersedeExtractionRevalidationMemory(this, ...args);
  }

  reclassifyExtractionRevalidationMemory(...args) {
    return reclassifyExtractionRevalidationMemory(this, ...args);
  }

  demoteExtractionRevalidationMemory(...args) {
    return demoteExtractionRevalidationMemory(this, ...args);
  }

  rollbackExtractionRevalidation(...args) {
    return rollbackExtractionRevalidation(this, ...args);
  }
  ...
  setSemanticMemoryMetadata(...args) {
    return setSemanticMemoryMetadata(this, ...args);
  }
