...
export function reflectionEvidenceCandidateLimit(config, requested) {
  if (!requested || config?.enabled !== true || config?.embeddings?.enabled !== true) {
    return null;
  }
  return Math.max(3, Math.min(40, boundedInteger(config.embeddings.maxInputs, 24, 2, 50) - 1));
}
...
export function formatEmbeddingRetrievalInput(model, text, kind) {
  const normalizedModel = String(model || "").toLowerCase();
  if (normalizedModel.includes("embeddinggemma")) {
    return kind === "query"
      ? `task: search result | query: ${text}`
      : `title: Lore session evidence | text: ${text}`;
  }
  if (normalizedModel.includes("nomic-embed")) {
...
async function requestRankedInsights({
  ...
  const vectors = await requestLocalInferenceEmbeddings({
    ...
    input: [
      formatEmbeddingRetrievalInput(config.embeddings?.model, prompt, "query"),
      ...candidates.map((insight) => formatEmbeddingRetrievalInput(
        config.embeddings?.model,
        insight.evidence || insight.text,
        "document",
      )),
    ],
    fetchImpl,
...
async function rankReflectionInsights({
  ...
  const minSimilarity = boundedSimilarity(config.embeddings?.minSimilarity, 0.35);
...
async function groundReflectionAnalysis({
  ...
  const vectors = await requestLocalInferenceEmbeddings({
    config: buildEmbeddingConfig(config),
    input: boundedCandidates.map((item) => formatEmbeddingRetrievalInput(
      config.embeddings?.model,
      item.text,
      "query",
    )),
...
async function groundSynthesizedReflection({
  ...
  const vectors = await requestLocalInferenceEmbeddings({
    config: buildEmbeddingConfig(config),
    input: [summary, ...candidates.map((candidate) => candidate.text)].map((text) => (
      formatEmbeddingRetrievalInput(config.embeddings?.model, text, "query")
    )),
    fetchImpl,
  });
  if (vectors.length !== candidates.length + 1) {
...
export async function enhanceReflectionWithLocalInference({
  ...
  const ranked = await rankReflectionInsights({
    config,
    prompt: reflection.retrievalPrompt || reflection.prompt,
