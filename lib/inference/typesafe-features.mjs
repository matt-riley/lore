...
const DEFAULT_MAX_MEMORIES_PER_RUN = 8;
...
function buildQuestions(count) {
  ...
  for (let index = 0; index < count; index += 1) {
    questions[`durable_${index}`] = {
      type: "noul",
      instructions: `Is the statement at \`memories[${index}].content\` a durable rule that should govern future work, rather than a one-off instruction tied to a single task or moment?`,
      criteria: {
        true: "A lasting policy, preference, or constraint that applies to future work",
        false: "A one-off request, status update, or instruction for a specific task or moment",
      },
    };
