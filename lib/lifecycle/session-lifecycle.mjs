 ...
 *   - recordPostToolUseObservation / recordErrorTelemetry: the rollout-gated
 *     "build a privacy-preserving record, write it" glue around the pure
 *     builders in lib/lifecycle/passive-hooks.mjs.
 */
...
import { buildErrorTelemetryRecord, buildPostToolUseObservation } from "./passive-hooks.mjs";
import { readErrorTelemetryEnabled, readPostToolUseEnabled } from "../rollout/rollout-flags.mjs";
...
/**
 * Rollout-gated, privacy-preserving post-tool-use observation write. Shared
 * decision + persistence glue around buildPostToolUseObservation; hosts
 * supply repository scope and any host-specific context fields (Pi's
 * `hookKind`/`argsShape`, the CLI's `client`/`sessionId`) via `contextFields`.
 *
 * @param {object} options
 * @param {object} options.db
 * @param {object} options.config
 * @param {string|null} [options.repository]
 * @param {unknown} options.payload - Raw onPostToolUse-shaped payload.
 * @param {(config: object) => boolean} [options.isEnabled]
 * @param {(observation: object) => object} [options.contextFields] - Extra
 *   fields merged into the persisted `context` object.
 * @returns {{ recorded: boolean, reason?: string, observation?: object }}
 */
export function recordPostToolUseObservation({
  db,
  config,
  repository = null,
  payload,
  isEnabled = readPostToolUseEnabled,
  contextFields = () => ({}),
}) {
  if (!isEnabled(config)) {
    return { recorded: false, reason: "disabled" };
  }
  const observation = buildPostToolUseObservation(payload);
  if (!observation) {
    return { recorded: false, reason: "no_observation" };
  }
...
/**
 * Rollout-gated, privacy-preserving error telemetry write. Shared decision +
 * persistence glue around buildErrorTelemetryRecord; hosts that periodically
 * compact the table pass `onCompact` (their own counter/closure) so the
 * compaction cadence stays exactly as it was per host.
 *
 * @param {object} options
 * @param {object} options.db
 * @param {object} options.config
 * @param {unknown} options.payload - Raw onErrorOccurred-shaped payload.
 * @param {string|null} [options.sessionId]
 * @param {(config: object) => boolean} [options.isEnabled]
 * @param {(() => void)|null} [options.onCompact]
 * @returns {{ recorded: boolean, reason?: string, record?: object }}
 */
export function recordErrorTelemetry({
  ...
  payload,
  sessionId = null,
  isEnabled = readErrorTelemetryEnabled,
  onCompact = null,
}) {
  if (!isEnabled(config)) {
    return { recorded: false, reason: "disabled" };
  }
  const record = buildErrorTelemetryRecord(payload, sessionId);
  if (!record) {
    return { recorded: false, reason: "no_record" };
  }
