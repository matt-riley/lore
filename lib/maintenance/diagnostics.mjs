...
import { buildProceduralProfile, detectRelevantInstructionFiles } from "../memory/procedural-memory.mjs";
...
function summarizeTraceRow(row) {
  ...
  const suffix = row.repository ? ` (${row.repository})` : "";
  if (nonEmptyString(row.content)) {
    return `[${row.type ?? row.sourceType ?? "row"}] ${row.content}${suffix}`;
  }
  if (nonEmptyString(row.summary)) return `${row.summary}${suffix}`;
  if (nonEmptyString(row.excerpt)) return `${row.excerpt}${suffix}`;
...
function aggregateFilteredReasons(filtered) {
  return collectFilteredReasonSummaries(filtered).join(", ");
}
...
function evaluateExpectedEvidence(definition, explanation) {
  ...
  const items = expectedItems.map((item) => evaluateEvidenceItem(traceRows, item));
  return {
...
function createImprovementLinkedMemory({
  ...
  if (!readAutoWriteImprovementGoalsEnabled(runtime.config)) {
    return null;
  }
...
function cleanupSeedDiagnosticsMemories(runtime, ids) {
  ...
  // insertSemanticMemory returns null instead of inserting when the exact
  // content is already actively suppressed (for example two validation cases
  // sharing identical extraSeedMemories content within the same isolated
  // run); there is nothing to forget in that case.
...
// Diagnostics seed synthetic global memories and read the real corpus. Run
// them against a temporary copy so seeds can never collide with, retire, or
// become visible alongside live memories; only result artifacts are written
// back to the live store by the caller.
function createIsolatedDiagnosticsRuntime(runtime) {
  const directory = mkdtempSync(path.join(os.tmpdir(), "lore-diagnostics-"));
  const isolatedPath = path.join(directory, "lore.db");
  runtime.db.db.exec(`VACUUM INTO '${isolatedPath.replaceAll("'", "''")}'`);
  const config = { ...runtime.config, paths: { ...runtime.config.paths, derivedStorePath: isolatedPath } };
...
export async function explainMemoryRetrieval({
  ...
  if (mode === "session_start") {
    ...
    const result = await assembleRecall({
      ...
      sessionSource: runtime.sessionStore,
      config: runtime.config,
      includeTrace: true,
      phases: SESSION_START_PHASES,
    });
    return {
      mode,
      prompt,
...
export async function runValidationSet({ runtime, caseIds = [] }) {
  ...
  const isolated = createIsolatedDiagnosticsRuntime(runtime);
  const diagnosticsRuntime = isolated.runtime;
  const seededIds = seedDiagnosticsMemories(diagnosticsRuntime);
  try {
    const cases = [];
    const improvementArtifacts = [];
    for (const definition of selected) {
      const extraSeedIds = seedExtraDiagnosticsMemories(diagnosticsRuntime, definition.extraSeedMemories ?? []);
      try {
        const explanation = await explainMemoryRetrieval({
          runtime: diagnosticsRuntime,
          prompt: definition.prompt,
          mode: definition.mode,
          repository: definition.repository ?? diagnosticsRuntime.repository ?? null,
        });
        ...
        cleanupSeedDiagnosticsMemories(diagnosticsRuntime, extraSeedIds);
      }
    }
    ...
    cleanupSeedDiagnosticsMemories(diagnosticsRuntime, seededIds);
    isolated.close();
  }
}
...
export async function runReplayCorpus({ runtime, caseIds = [] }) {
  ...
  const seededIds = seedDiagnosticsMemories(diagnosticsRuntime);
  try {
    const replay = await collectReplayCases(diagnosticsRuntime, selected, runtime);
    return buildReplayCorpusResult(diagnosticsRuntime, replay);
  } finally {
    cleanupSeedDiagnosticsMemories(diagnosticsRuntime, seededIds);
    isolated.close();
  }
...
async function runReplayDefinition(runtime, definition, persistRuntime = runtime) {
...
async function collectReplayCases(runtime, definitions, persistRuntime = runtime) {
  const cases = [];
  const improvementArtifacts = [];
  for (const definition of definitions) {
    const replayResult = await runReplayDefinition(runtime, definition, persistRuntime);
