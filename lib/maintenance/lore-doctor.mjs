...
function classifyIngestionCheckpointFailures(rows) {
  if (!rows.length) return [];
  return [
    {
      kind: INCIDENT_KINDS.INGESTION_CHECKPOINT_FAILURES,
      severity: "warning",
      summary: `${rows.length} ingestion checkpoint(s) recorded a failure code`,
      context: {
        byClientCode: summarizeFailureCodes(rows),
...
export function runDoctorObservation({
  ...
  return {
    ...
    signals: {
      ...
      ingestionCheckpointFailures: ingestionCheckpointFailures.length,
