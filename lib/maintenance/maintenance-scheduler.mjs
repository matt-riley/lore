import crypto from "node:crypto";
import { processDeferredExtractions } from "../sessions/backfill.mjs";
import { normalizeBoolean, clampInteger } from "../core/config.mjs";
import {
  REPLAY_CASES,
  runReplayCorpus,
  runValidationSet,
  VALIDATION_CASES,
} from "./diagnostics.mjs";
import {
  generateProposalArtifacts,
  verifyProposalArtifacts,
} from "../sessions/proposal-generator.mjs";
import { runDoctorObservation } from "./lore-doctor.mjs";
import {
  createGitAncestorChecker,
  runMemoryHygiene,
} from "../memory/memory-hygiene.mjs";
import { runExtractionRevalidation } from "../memory/extraction-revalidation.mjs";
import { indexMemoryEmbeddings } from "../memory/semantic-search.mjs";

export const TASK_ORDER = Object.freeze([
  "memoryHygiene",
  "deferredExtraction",
  "validationCorpus",
  "replayCorpus",
  "backlogReview",
  "traceCompaction",
  "indexUpkeep",
  "doctorSnapshot",
  "extractionRevalidation",
]);

const TASK_LABELS = Object.freeze({
  memoryHygiene: "Memory Hygiene",
  deferredExtraction: "Deferred Extraction",
  validationCorpus: "Validation Corpus",
  replayCorpus: "Replay Corpus",
  backlogReview: "Backlog Review",
  traceCompaction: "Trace Compaction",
  indexUpkeep: "Index Upkeep",
  doctorSnapshot: "Doctor Snapshot",
  extractionRevalidation: "Extraction Revalidation",
});

function ensureArray(value) {
  return Array.isArray(value) ? value : [];
}

function nowIso() {
  return new Date().toISOString();
}

function parseTime(value) {
  const parsed = Date.parse(String(value ?? ""));
  return Number.isFinite(parsed) ? parsed : null;
}

function elapsedMinutesSince(value) {
  const timestamp = parseTime(value);
  if (timestamp == null) {
    return null;
  }
  return Math.max(0, Math.floor((Date.now() - timestamp) / (60 * 1000)));
}

function dedupeTaskNames(taskNames = []) {
  const seen = new Set();
  const normalized = [];
  for (const value of ensureArray(taskNames)) {
    const name = typeof value === "string" ? value.trim() : "";
    if (!TASK_ORDER.includes(name) || seen.has(name)) {
      continue;
    }
    seen.add(name);
    normalized.push(name);
  }
  return normalized;
}

function buildCaseWindow(cases, cursor, limit) {
  const boundedCases = ensureArray(cases);
  if (boundedCases.length === 0) {
    return {
      caseIds: [],
      nextCursor: 0,
      cursor: 0,
    };
  }
  const boundedLimit = clampInteger(limit, 1, { min: 1, max: boundedCases.length });
  const boundedCursor = clampInteger(cursor, 0, { min: 0, max: boundedCases.length - 1 });
  const caseIds = [];
  for (let index = 0; index < boundedLimit; index += 1) {
    const entry = boundedCases[(boundedCursor + index) % boundedCases.length];
    if (entry?.id) {
      caseIds.push(entry.id);
    }
  }
  return {
    caseIds,
    cursor: boundedCursor,
    nextCursor: boundedCases.length > 0
      ? (boundedCursor + caseIds.length) % boundedCases.length
      : 0,
  };
}

function buildMaintenanceOptions(config) {
  const maintenance = config?.maintenanceScheduler ?? config?.maintenance ?? {};
  const tasks = maintenance.tasks ?? {};
  const cadence = maintenance.taskCadenceMinutes ?? {};
  const memoryHygiene = maintenance.memoryHygiene ?? {};
  const memoryHygieneMode = ["off", "shadow", "apply"].includes(memoryHygiene.mode)
    ? memoryHygiene.mode
    : "off";
  const extractionRevalidation = maintenance.extractionRevalidation ?? {};
  return Object.freeze({
    enabled: normalizeBoolean(maintenance.enabled, false),
    autoRunOnSessionStart: normalizeBoolean(maintenance.autoRunOnSessionStart, true),
    staleRunAfterMinutes: clampInteger(maintenance.staleRunAfterMinutes, 30, { min: 1, max: 365 * 24 * 60 }),
    maxTasksPerRun: clampInteger(maintenance.maxTasksPerRun, 4, { min: 1, max: TASK_ORDER.length }),
    validationCaseLimit: clampInteger(maintenance.validationCaseLimit, 3, { min: 1, max: Math.max(1, VALIDATION_CASES.length) }),
    replayCaseLimit: clampInteger(maintenance.replayCaseLimit, 2, { min: 1, max: Math.max(1, REPLAY_CASES.length) }),
    backlogReviewLimit: clampInteger(maintenance.backlogReviewLimit, 10, { min: 1, max: 50 }),
    backlogStaleAfterHours: clampInteger(maintenance.backlogStaleAfterHours, 72, { min: 1, max: 24 * 365 }),
    memoryHygiene: {
      mode: memoryHygieneMode,
      maxItems: clampInteger(memoryHygiene.maxItems, 50, { min: 1, max: 200 }),
      includeGlobal: normalizeBoolean(memoryHygiene.includeGlobal, true),
    },
    extractionRevalidation: {
      mode: ["off", "shadow", "apply"].includes(extractionRevalidation.mode) ? extractionRevalidation.mode : "off",
      maxItems: clampInteger(extractionRevalidation.maxItems, 50, { min: 1, max: 200 }),
      includeGlobal: normalizeBoolean(extractionRevalidation.includeGlobal, true),
    },
    tasks: {
      memoryHygiene: normalizeBoolean(tasks.memoryHygiene, true),
      deferredExtraction: normalizeBoolean(tasks.deferredExtraction, true),
      validationCorpus: normalizeBoolean(tasks.validationCorpus, true),
      replayCorpus: normalizeBoolean(tasks.replayCorpus, true),
      backlogReview: normalizeBoolean(tasks.backlogReview, true),
      traceCompaction: normalizeBoolean(tasks.traceCompaction, false),
      indexUpkeep: normalizeBoolean(tasks.indexUpkeep, false),
      doctorSnapshot: normalizeBoolean(tasks.doctorSnapshot, false),
      extractionRevalidation: normalizeBoolean(tasks.extractionRevalidation, false),
    },
    taskCadenceMinutes: {
      memoryHygiene: clampInteger(cadence.memoryHygiene, 0, { min: 0, max: 30 * 24 * 60 }),
      deferredExtraction: clampInteger(cadence.deferredExtraction, 0, { min: 0, max: 7 * 24 * 60 }),
      validationCorpus: clampInteger(cadence.validationCorpus, 12 * 60, { min: 0, max: 30 * 24 * 60 }),
      replayCorpus: clampInteger(cadence.replayCorpus, 24 * 60, { min: 0, max: 30 * 24 * 60 }),
      backlogReview: clampInteger(cadence.backlogReview, 6 * 60, { min: 0, max: 30 * 24 * 60 }),
      traceCompaction: clampInteger(cadence.traceCompaction, 60, { min: 0, max: 30 * 24 * 60 }),
      indexUpkeep: clampInteger(cadence.indexUpkeep, 12 * 60, { min: 0, max: 30 * 24 * 60 }),
      doctorSnapshot: clampInteger(cadence.doctorSnapshot, 24 * 60, { min: 0, max: 30 * 24 * 60 }),
      extractionRevalidation: clampInteger(cadence.extractionRevalidation, 24 * 60, { min: 0, max: 30 * 24 * 60 }),
    },
  });
}

const TASK_ENABLEMENT_CHECKS = Object.freeze({
  memoryHygiene: ({ options }) => options.memoryHygiene.mode !== "off",
  extractionRevalidation: ({ options }) => options.extractionRevalidation.mode !== "off",
  deferredExtraction: ({ runtime, trigger }) => {
    if (runtime.config?.deferredExtraction?.enabled !== true) return false;
    if (trigger === "session_start" && runtime.config?.deferredExtraction?.autoProcessOnSessionStart !== true) {
      return false;
    }
    return true;
  },
  traceCompaction: ({ runtime }) => runtime.traceRecorder?.isEnabled?.() === true,
  doctorSnapshot: ({ runtime }) => runtime.config?.rollout?.loreDoctor === true,
});

function isTaskEnabled({ taskName, options, runtime, trigger }) {
  if (options.tasks[taskName] !== true) {
    return false;
  }
  const taskCheck = TASK_ENABLEMENT_CHECKS[taskName];
  return taskCheck ? taskCheck({ options, runtime, trigger }) : true;
}

function buildDueResult({
  due,
  reason,
  cadenceMinutes,
  lastRunMinutesAgo,
  nextRunMinutes = null,
}) {
  return {
    due,
    reason,
    cadenceMinutes,
    lastRunMinutesAgo,
    nextRunMinutes,
  };
}

function readTaskReferenceAt(state) {
  return state?.last_completed_at ?? state?.last_started_at ?? null;
}

function buildAgeBasedDueEvaluation(cadenceMinutes, ageMinutes) {
  if (cadenceMinutes === 0) {
    return buildDueResult({
      due: true,
      reason: "always_due",
      cadenceMinutes,
      lastRunMinutesAgo: ageMinutes,
    });
  }
  if (ageMinutes == null || ageMinutes >= cadenceMinutes) {
    return buildDueResult({
      due: true,
      reason: ageMinutes == null ? "due_unknown_age" : "cadence_elapsed",
      cadenceMinutes,
      lastRunMinutesAgo: ageMinutes,
    });
  }
  return buildDueResult({
    due: false,
    reason: "within_cadence",
    cadenceMinutes,
    lastRunMinutesAgo: ageMinutes,
    nextRunMinutes: cadenceMinutes - ageMinutes,
  });
}

function buildDueEvaluation({ taskName, state, options, force = false }) {
  const cadenceMinutes = options.taskCadenceMinutes[taskName] ?? 0;
  const referenceAt = readTaskReferenceAt(state);
  if (force) {
    return buildDueResult({
      due: true,
      reason: "forced",
      cadenceMinutes,
      lastRunMinutesAgo: elapsedMinutesSince(referenceAt),
    });
  }
  if (!referenceAt) {
    return buildDueResult({
      due: true,
      reason: "never_run",
      cadenceMinutes,
      lastRunMinutesAgo: null,
    });
  }
  return buildAgeBasedDueEvaluation(cadenceMinutes, elapsedMinutesSince(referenceAt));
}

function buildTaskPreview({ taskName, state, options }) {
  if (taskName === "validationCorpus") {
    return buildCaseWindow(VALIDATION_CASES, state?.cursor ?? 0, options.validationCaseLimit);
  }
  if (taskName === "replayCorpus") {
    return buildCaseWindow(REPLAY_CASES, state?.cursor ?? 0, options.replayCaseLimit);
  }
  return null;
}

function buildTaskEntry({
  taskName,
  state,
  options,
  runtime,
  trigger,
  selectedTaskNames,
  force,
}) {
  const enabled = isTaskEnabled({ taskName, options, runtime, trigger });
  const cadenceMinutes = options.taskCadenceMinutes[taskName] ?? 0;
  const due = enabled
    ? buildDueEvaluation({ taskName, state, options, force })
    : buildDueResult({
      due: false,
      reason: "disabled",
      cadenceMinutes,
      lastRunMinutesAgo: elapsedMinutesSince(readTaskReferenceAt(state)),
    });
  const preview = buildTaskPreview({ taskName, state, options });
  return {
    taskName,
    label: TASK_LABELS[taskName] ?? taskName,
    enabled,
    selected: selectedTaskNames.includes(taskName),
    due: due.due,
    dueReason: due.reason,
    cadenceMinutes: due.cadenceMinutes,
    lastRunMinutesAgo: due.lastRunMinutesAgo,
    nextRunMinutes: due.nextRunMinutes ?? null,
    state: state ?? null,
    preview,
  };
}

export function buildMaintenancePlan({
  runtime,
  repository,
  trigger = "manual",
  requestedTasks = [],
  force = false,
  recoverStaleWork = false,
} = {}) {
  const options = buildMaintenanceOptions(runtime.config);
  if (recoverStaleWork) {
    runtime.db.reclaimStaleMaintenanceRuns?.({
      staleAfterMs: options.staleRunAfterMinutes * 60 * 1000,
    });
  }
  const taskStates = runtime.db.listMaintenanceTaskStates();
  const taskStateMap = new Map(taskStates.map((row) => [row.task_name, row]));
  const normalizedRequested = dedupeTaskNames(requestedTasks);
  const selectedTaskNames = normalizedRequested.length > 0 ? normalizedRequested : [...TASK_ORDER];

  const tasks = TASK_ORDER.map((taskName) => buildTaskEntry({
    taskName,
    state: taskStateMap.get(taskName),
    options,
    runtime,
    trigger,
    selectedTaskNames,
    force,
  }));

  const dueTasks = tasks.filter((task) => task.selected && task.enabled && task.due);
  const triggerBoundDueTasks = trigger === "session_start"
    ? dueTasks.filter((task) => ["memoryHygiene", "deferredExtraction"].includes(task.taskName))
    : dueTasks;
  const selectedTasks = triggerBoundDueTasks.slice(0, options.maxTasksPerRun);
  const deferredTask = tasks.find((task) => task.taskName === "deferredExtraction") ?? null;

  return {
    generatedAt: nowIso(),
    repository: repository ?? null,
    trigger,
    enabled: options.enabled,
    autoRunOnSessionStart: options.autoRunOnSessionStart,
    maxTasksPerRun: options.maxTasksPerRun,
    staleRunAfterMinutes: options.staleRunAfterMinutes,
    tasks,
    dueTasks: triggerBoundDueTasks,
    selectedTasks,
    deferredTask,
    skippedDueToCap: Math.max(0, triggerBoundDueTasks.length - selectedTasks.length),
    requestedTasks: normalizedRequested,
    force,
    recentRuns: runtime.db.listMaintenanceRuns({ limit: 5 }),
  };
}

function summarizeArtifacts(artifacts) {
  return ensureArray(artifacts).map((artifact) => ({
    id: artifact.id,
    sourceKind: artifact.sourceKind,
    sourceCaseId: artifact.sourceCaseId,
    title: artifact.title,
  }));
}

function currentTaskCursor(entry) {
  return entry.state?.cursor ?? 0;
}

function buildTaskExecution(status, entry, summary) {
  return {
    status,
    cursor: currentTaskCursor(entry),
    summary,
  };
}

async function executeDeferredExtractionTask({ runtime, repository, entry }) {
  const result = await processDeferredExtractions({
    db: runtime.db,
    sessionStore: runtime.sessionStore,
    repository: runtime.config?.deferredExtraction?.processCurrentRepositoryOnly === false
      ? null
      : repository,
    limit: clampInteger(
      runtime.config?.deferredExtraction?.maxJobsPerRun,
      2,
      { min: 1, max: 20 },
    ),
    retryDelayMinutes: clampInteger(
      runtime.config?.deferredExtraction?.retryDelayMinutes,
      15,
      { min: 0, max: 24 * 60 },
    ),
    fetchImpl: runtime.localInferenceFetch,
  });
  const stats = runtime.db.getStats();
  return buildTaskExecution(
    result.failed > 0 || result.inferenceFailed > 0 ? "needs_attention" : "completed",
    entry,
    {
      ...result,
      pendingAfter: stats.deferredPendingCount,
      failedAfter: stats.deferredFailedCount,
      completedAfter: stats.deferredCompletedCount,
    },
  );
}

async function executeMemoryHygieneTask({ runtime, repository, entry, options }) {
  const isCommitAncestor = runtime.memoryHygieneIsCommitAncestor
    ?? createGitAncestorChecker(runtime.checkoutPath);
  const result = await runMemoryHygiene({
    db: runtime.db,
    repository,
    mode: options.memoryHygiene.mode,
    maxItems: options.memoryHygiene.maxItems,
    includeGlobal: options.memoryHygiene.includeGlobal,
    isCommitAncestor,
    config: runtime.config,
    fetchImpl: runtime.fetchImpl,
  });
  return buildTaskExecution(
    "completed",
    entry,
    {
      ...result,
      items: result.items.slice(0, 10),
    },
  );
}

async function executeExtractionRevalidationTask({ runtime, repository, entry, options }) {
  const result = runExtractionRevalidation({
    db: runtime.db,
    repository,
    mode: options.extractionRevalidation.mode === "apply" ? "apply" : "shadow",
    maxItems: options.extractionRevalidation.maxItems,
    includeGlobal: options.extractionRevalidation.includeGlobal,
    actor: "maintenance_scheduler",
    reason: "scheduled_extraction_revalidation",
  });
  return buildTaskExecution(
    "completed",
    entry,
    {
      ...result,
      items: result.items.slice(0, 10),
    },
  );
}

async function executeValidationCorpusTask({ runtime, entry, options }) {
  const preview = entry.preview ?? buildCaseWindow(VALIDATION_CASES, currentTaskCursor(entry), options.validationCaseLimit);
  const result = await runValidationSet({
    runtime,
    caseIds: preview.caseIds,
  });
  return {
    status: result.failed > 0 ? "needs_attention" : "completed",
    cursor: preview.nextCursor,
    summary: {
      caseIds: preview.caseIds,
      total: result.total,
      passed: result.passed,
      failed: result.failed,
      improvementArtifacts: summarizeArtifacts(result.improvementArtifacts),
      generatedAt: result.generatedAt,
    },
  };
}

async function executeReplayCorpusTask({ runtime, entry, options }) {
  const preview = entry.preview ?? buildCaseWindow(REPLAY_CASES, currentTaskCursor(entry), options.replayCaseLimit);
  const result = await runReplayCorpus({
    runtime,
    caseIds: preview.caseIds,
  });
  const needsAttention = result.mustPassFailed > 0
    || result.rankingTargetMissing > 0
    || result.rankingTargetPartial > 0;
  return {
    status: needsAttention ? "needs_attention" : "completed",
    cursor: preview.nextCursor,
    summary: {
      caseIds: preview.caseIds,
      total: result.total,
      mustPassFailed: result.mustPassFailed,
      rankingTargetPartial: result.rankingTargetPartial,
      rankingTargetMissing: result.rankingTargetMissing,
      improvementArtifacts: summarizeArtifacts(result.improvementArtifacts),
      generatedAt: result.generatedAt,
    },
  };
}

function buildBacklogReviewSummary(staleArtifacts, stats, proposalGeneration, integrity, options) {
  return {
    reviewLimit: options.backlogReviewLimit,
    staleAfterHours: options.backlogStaleAfterHours,
    activeCount: stats.improvementActiveCount ?? 0,
    proposalCount: stats.improvementProposalCount ?? 0,
    staleCount: staleArtifacts.length,
    staleArtifacts: staleArtifacts.map((artifact) => ({
      id: artifact.id,
      title: artifact.title,
      sourceKind: artifact.source_kind,
      updatedAt: artifact.updated_at,
    })),
    proposalGeneration,
    integrity,
  };
}

async function executeBacklogReviewTask({ runtime, entry, options }) {
  const staleBefore = new Date(Date.now() - (options.backlogStaleAfterHours * 60 * 60 * 1000)).toISOString();
  const staleArtifacts = runtime.db.listImprovementArtifacts({
    status: "active",
    updatedBefore: staleBefore,
    sort: "updated_asc",
    limit: options.backlogReviewLimit,
  });
  const stats = runtime.db.getStats();
  const proposalGeneration = await generateProposalArtifacts({
    runtime,
    ids: staleArtifacts.map((artifact) => artifact.id),
    limit: options.backlogReviewLimit,
    updatedBefore: staleBefore,
  });
  const integrity = await verifyProposalArtifacts({
    runtime,
    limit: options.backlogReviewLimit,
    repair: false,
  });
  const needsAttention = staleArtifacts.length > 0
    || proposalGeneration.generatedCount > 0
    || integrity.issueCount > 0;
  return buildTaskExecution(
    needsAttention ? "needs_attention" : "completed",
    entry,
    buildBacklogReviewSummary(staleArtifacts, stats, proposalGeneration, integrity, options),
  );
}

function executeTraceCompactionTask({ runtime, entry }) {
  const result = runtime.traceRecorder?.compact?.();
  return buildTaskExecution(
    "completed",
    entry,
    result ?? {
      storedBefore: 0,
      storedAfter: 0,
      expiredRemoved: 0,
      totalRecorded: 0,
    },
  );
}

async function executeIndexUpkeepTask({ runtime, entry }) {
  const schemaResult = runtime.db.runIndexUpkeep?.() ?? null;
  const result = await indexMemoryEmbeddings({
    db: runtime.db,
    config: runtime.config,
    fetchImpl: runtime.localInferenceFetch,
    cursor: currentTaskCursor(entry),
    maxMemories: 24,
  });
  return {
    ...buildTaskExecution(
    "completed",
    entry,
    { ...result, schema: schemaResult },
    ),
    cursor: result.cursor ?? currentTaskCursor(entry),
    status: result.failed > 0 ? "needs_attention" : "completed",
  };
}

function executeDoctorSnapshotTask({ runtime, repository, entry }) {
  const result = runDoctorObservation({
    runtime,
    repository,
    dryRun: false,
    trajectoryLimit: 20,
  });
  return buildTaskExecution(
    result.incidentCount > 0 ? "needs_attention" : "completed",
    entry,
    {
      incidentCount: result.incidentCount,
      criticalCount: result.criticalCount,
      warningCount: result.warningCount,
      infoCount: result.infoCount,
      recordedArtifactId: result.recordedArtifactId,
      generatedAt: result.generatedAt,
    },
  );
}

const TASK_EXECUTORS = Object.freeze({
  memoryHygiene: executeMemoryHygieneTask,
  deferredExtraction: executeDeferredExtractionTask,
  validationCorpus: executeValidationCorpusTask,
  replayCorpus: executeReplayCorpusTask,
  backlogReview: executeBacklogReviewTask,
  traceCompaction: executeTraceCompactionTask,
  indexUpkeep: executeIndexUpkeepTask,
  doctorSnapshot: executeDoctorSnapshotTask,
  extractionRevalidation: executeExtractionRevalidationTask,
});

async function executeTask({
  taskName,
  runtime,
  repository,
  entry,
  options,
}) {
  const executor = TASK_EXECUTORS[taskName];
  if (!executor) {
    throw new Error(`unsupported maintenance task: ${taskName}`);
  }
  return executor({
    taskName,
    runtime,
    repository,
    entry,
    options,
  });
}

function summarizeRunStatus({ completedCount, needsAttentionCount, failedCount, skippedCount }) {
  if (failedCount > 0) {
    return "failed";
  }
  if (needsAttentionCount > 0) {
    return "needs_attention";
  }
  if (completedCount > 0) {
    return "completed";
  }
  return skippedCount > 0 ? "skipped" : "idle";
}

function buildDisabledSweepResult(plan, trigger, dryRun) {
  return {
    generatedAt: plan.generatedAt,
    repository: plan.repository,
    trigger,
    dryRun,
    status: "disabled",
    taskCount: 0,
    completedCount: 0,
    needsAttentionCount: 0,
    failedCount: 0,
    skippedCount: plan.tasks.filter((task) => task.selected).length,
    tasks: [],
    plan,
  };
}

function buildPlannedSweepResult(plan, trigger, dryRun) {
  return {
    generatedAt: plan.generatedAt,
    repository: plan.repository,
    trigger,
    dryRun,
    status: plan.selectedTasks.length === 0 ? "skipped" : "planned",
    taskCount: plan.selectedTasks.length,
    completedCount: 0,
    needsAttentionCount: 0,
    failedCount: 0,
    skippedCount: Math.max(0, plan.tasks.filter((task) => task.selected).length - plan.selectedTasks.length),
    tasks: plan.selectedTasks.map((task) => ({
      taskName: task.taskName,
      label: task.label,
      status: "planned",
      durationMs: 0,
      summary: task.preview ? { caseIds: task.preview.caseIds } : null,
    })),
    plan,
  };
}

async function executeAndRecordTask({ entry, runtime, repository, trigger, options }) {
  const startedAt = nowIso();
  runtime.db.recordMaintenanceTaskStart({
    taskName: entry.taskName,
    trigger,
    repository,
    startedAt,
  });
  const durationStartedAt = Date.now();
  try {
    const execution = await executeTask({ taskName: entry.taskName, runtime, repository, entry, options });
    const durationMs = Date.now() - durationStartedAt;
    runtime.db.recordMaintenanceTaskResult({
      taskName: entry.taskName,
      status: execution.status,
      trigger,
      repository,
      startedAt,
      completedAt: nowIso(),
      durationMs,
      cursor: execution.cursor,
      summary: execution.summary,
    });
    return { entry, status: execution.status, durationMs, summary: execution.summary };
  } catch (error) {
    const durationMs = Date.now() - durationStartedAt;
    const message = error instanceof Error ? error.message : String(error);
    runtime.db.recordMaintenanceTaskResult({
      taskName: entry.taskName,
      status: "failed",
      trigger,
      repository,
      startedAt,
      completedAt: nowIso(),
      durationMs,
      cursor: entry.state?.cursor ?? 0,
      summary: { error: message },
    });
    return { entry, status: "failed", durationMs, summary: { error: message } };
  }
}

export async function runMaintenanceSweep({
  runtime,
  repository,
  trigger = "manual",
  requestedTasks = [],
  force = false,
  dryRun = false,
} = {}) {
  const plan = buildMaintenancePlan({
    runtime,
    repository,
    trigger,
    requestedTasks,
    force,
    recoverStaleWork: dryRun !== true,
  });

  if (!plan.enabled) {
    return buildDisabledSweepResult(plan, trigger, dryRun);
  }

  if (dryRun || plan.selectedTasks.length === 0) {
    return buildPlannedSweepResult(plan, trigger, dryRun);
  }

  const runId = runtime.db.createMaintenanceRun({
    trigger,
    repository,
    dryRun: false,
    plannedTasks: plan.selectedTasks.map((task) => task.taskName),
  });

  const options = buildMaintenanceOptions(runtime.config);
  const tasks = [];
  let completedCount = 0;
  let needsAttentionCount = 0;
  let failedCount = 0;
  let skippedCount = 0;

  for (const entry of plan.selectedTasks) {
    const result = await executeAndRecordTask({ entry, runtime, repository, trigger, options });
    if (result.status === "needs_attention") {
      needsAttentionCount += 1;
    } else if (result.status === "skipped") {
      skippedCount += 1;
    } else if (result.status === "failed") {
      failedCount += 1;
    } else {
      completedCount += 1;
    }
    tasks.push({ taskName: entry.taskName, label: entry.label, status: result.status, durationMs: result.durationMs, summary: result.summary });
  }

  const status = summarizeRunStatus({ completedCount, needsAttentionCount, failedCount, skippedCount });
  const completedAt = nowIso();
  runtime.db.completeMaintenanceRun({
    runId,
    status,
    repository,
    completedAt,
    completedCount,
    needsAttentionCount,
    failedCount,
    skippedCount,
    summary: { repository, trigger, tasks },
  });

  return {
    generatedAt: plan.generatedAt,
    repository: plan.repository,
    trigger,
    dryRun: false,
    runId,
    status,
    taskCount: tasks.length,
    completedCount,
    needsAttentionCount,
    failedCount,
    skippedCount,
    tasks,
    completedAt,
    plan,
  };
}

// Every host that cannot run maintenance inline on its own long-lived process
// (native CLI hook children, the Pi worker, cron/launchd) funnels through
// here rather than calling runMaintenanceSweep directly. It reuses the exact
// same session-start-bound planning logic Copilot's extension relies on
// (memoryHygiene + deferredExtraction only) and adds a cross-process mutex so
// two sweeps that start at nearly the same moment do not duplicate work.
const BACKGROUND_MAINTENANCE_LOCK_SCOPE = "background";
const BACKGROUND_MAINTENANCE_LEASE_MS = 5 * 60 * 1000;

export async function runBackgroundMaintenanceSweep({
  runtime,
  repository,
  trigger = "session_start",
  ownerToken = crypto.randomUUID(),
  leaseDurationMs = BACKGROUND_MAINTENANCE_LEASE_MS,
} = {}) {
  const plan = buildMaintenancePlan({ runtime, repository, trigger });
  if (!plan.enabled || !plan.autoRunOnSessionStart || plan.selectedTasks.length === 0) {
    return { attempted: false, locked: false, plan, result: null };
  }
  const acquired = runtime.db.acquireMaintenanceLock({
    scope: BACKGROUND_MAINTENANCE_LOCK_SCOPE,
    ownerToken,
    leaseDurationMs,
  });
  if (!acquired) {
    return { attempted: false, locked: true, plan, result: null };
  }
  try {
    const result = await runMaintenanceSweep({ runtime, repository, trigger });
    return { attempted: true, locked: false, plan, result };
  } finally {
    runtime.db.releaseMaintenanceLock({
      scope: BACKGROUND_MAINTENANCE_LOCK_SCOPE,
      ownerToken,
    });
  }
}

export function getMaintenanceStatus({
  runtime,
  repository,
} = {}) {
  const plan = buildMaintenancePlan({
    runtime,
    repository,
    trigger: "status",
    requestedTasks: [],
    force: false,
  });
  return {
    generatedAt: plan.generatedAt,
    repository: plan.repository,
    enabled: plan.enabled,
    autoRunOnSessionStart: plan.autoRunOnSessionStart,
    maxTasksPerRun: plan.maxTasksPerRun,
    dueTasks: plan.dueTasks,
    selectedTasks: plan.selectedTasks,
    skippedDueToCap: plan.skippedDueToCap,
    recentRuns: plan.recentRuns,
    tasks: plan.tasks,
  };
}
