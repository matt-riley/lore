...
export function restoreRecoverySnapshot({ derivedStorePath, snapshotPath, write = false, clientsStopped = false, preserveCurrentSuppressions = true, detectActiveUsers = defaultActiveDetector, fsOps = {} } = {}) {
  ...
  try {
    ...
    // Prove no other process has target open (mid-transaction or even just
    // holding a read snapshot) before swapping files underneath it. This is
    // acquired as late as possible — right before the renames — so the
    // exclusive lock and sidecar lock file (which lib/db/db.mjs's
    // openDatabase()/openReadOnly() back off for) are held for as short a
    // window as possible, not for the whole prepareSnapshotStage() above.
