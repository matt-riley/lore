...
import { acquireRestoreGuard } from "../db/db-restore-guard.mjs";
...
function defaultActiveDetector(dbPath) {
  const availability = spawnSync("lsof", ["-v"], { encoding: "utf8" });
  if (availability.error || availability.status !== 0 || String(availability.stderr || "").trim()) return null;
  const paths = [dbPath, `${dbPath}-wal`, `${dbPath}-shm`].filter(existsSync);
  if (paths.length === 0) return [];
  const result = spawnSync("lsof", ["-Fpn", "--", ...paths], { encoding: "utf8" });
  // lsof uses status 1 with no output when none of these existing files are open.
  if (result.error || (result.status !== 0 && result.status !== 1) || String(result.stderr || "").trim()) return null;
  if (result.status === 1 && !String(result.stdout || "").trim()) return [];
  const users = new Set();
  for (const line of String(result.stdout || "").split("\n")) {
    if (line.startsWith("p")) users.add(line.slice(1));
  }
  return [...users];
}
...
export function restoreRecoverySnapshot({ derivedStorePath, snapshotPath, write = false, clientsStopped = false, preserveCurrentSuppressions = true, detectActiveUsers = defaultActiveDetector, fsOps = {} } = {}) {
  ...
  try {
    ...
    // Check for open handles while the sidecar lock blocks new Lore opens,
    // then let the SQLite probe catch any active transaction or read snapshot.
    // The detector runs before the probe opens its own connection, so it also
    // catches idle handles in this process. Hold both locks only for the
    // final swap, not the prepareSnapshotStage() work above.
    const guard = acquireRestoreGuard(target, {
      beforeExclusive: (dbPath) => {
        const active = detectActiveUsers(dbPath);
        if (active === null) throw new Error("cannot safely restore while open database handle detection is unavailable; install lsof and retry");
