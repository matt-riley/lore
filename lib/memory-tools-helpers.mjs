...
export function formatProposalRows(rows) {
  return formatRows(rows, (row) => [
    `- [${row.id}] ${row.title}`,
...
export function normalizeImprovementEvidenceTag(value) {
  const normalized = String(value ?? "").trim().toLowerCase();
  return normalized || null;
}

export function deriveReplayImprovementTheme(evidence) {
...
export function deriveValidationImprovementTheme(evidence) {
...
export function deriveImprovementTheme(row) {
...
export function formatIntegrityIssues(issues) {
...
export function formatProposalGenerationReport(result) {
  return [
...
export function ensureLimit(value, fallback) {
...
export const PORTABLE_BUNDLE_EXPORT_ACTION = "export";
export const CAPABILITY_INVENTORY_ACTIONS = new Set(["summary", "recommend", "route", "evaluate", "json"]);

export function normalizePortableBundleAction(value) {
...
export function validateScopeOverrideRequest({ action, dryRun, reason, scope }) {
...
export function buildOnboardingProfileArgs(args) {
  return {
...
export function getReflectionDomainKey(args) {
  return typeof args.domainKey === "string" && args.domainKey.trim().length > 0
    ? args.domainKey.trim().toLowerCase()
    : null;
}

export function getObservationPersistSkipReason(runtime, domainKey) {
...
export function buildObservationUpsertInput({ runtime, reflection, request, args, domainKey }) {
  return {
...
export function summarizeImprovementClusters(rows, { minClusterSize = 2, maxClusters = 5 } = {}) {
...
export const PORTABLE_BUNDLE_TYPE = "lore-portable-improvement";

export function formatPortableBundleReport({
  bundleId,
  signature,
  bundlePath,
  repository,
...
export function resolveBundlePath(rawPath) {
