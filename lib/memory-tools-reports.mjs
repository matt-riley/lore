...
import { ensureArray, ensureLimit, formatRows } from "./memory-tools-helpers.mjs";

export function formatRecallSummary(trace) {
...
export function formatRecallReport(result, { includeTrace = false } = {}) {
  const lines = [
...
export function formatRecallSupportingFactsLines(envelope) {
  return [
    "",
    "## Supporting Facts",
    "",
...
export function formatRecallLookupLines(lookup, detailLevel) {
  const lines = [
...
export function formatRecallEvidenceLines(envelope, detailLevel) {
...
export function humanizeFocus(value) {
  return String(value || "summary")
    .replace(/_/g, " ")
    .replace(/\b\w/g, (match) => match.toUpperCase());
}

export function humanizeLookupLabel(value) {
...
export function buildReflectionHeaderLines(result) {
  return [
    `repository: ${result.repository ?? "global-only"}`,
...
export function buildReflectionInsightLines(result) {
...
export function buildReflectionEvidenceLines(result) {
  return [
    "",
    "## Supporting Evidence",
    "",
...
export function formatReflectionFactLines(facts) {
...
export function formatReflectionSourceAccountingLines(sourceAccounting) {
...
export function buildReflectionLookupCoverageLines(result) {
...
export function formatReflectionLookupLines(lookup) {
  return [
...
export function formatReflectionLookupSamples(lookup) {
...
export function formatBackfillRunRows(rows) {
  return formatRows(rows, (row) => [
...
export function formatBackfillItems(rows) {
  return formatRows(rows, (row) => [
    `- ${row.session_id}`,
...
export function formatBackfillProgressKeyValues(progress) {
  return [
...
export function formatControlledBackfillPreview(preview) {
...
export function determineMaintenancePhase(result) {
...
export function summarizeMaintenanceProgress(result) {
...
export function formatControlledBackfillRun(run, items = []) {
...
export function runControlledBackfillPreview(runtime, request) {
...
export function runControlledBackfillStart(runtime, request) {
...
export function runControlledBackfillResume(runtime, request, args) {
...
export function runControlledBackfillRestore(runtime, args) {
  const runId = ensureString(args.runId, "runId");
...
export function formatBackfillStatus(runtime, args, batchSize) {
...
export function compareOverlayState(left, right) {
...
export function formatLatencyMetricLine(prefix, suffix, value) {
  return `${prefix}${suffix}: ${value}`;
}

export function normalizeLatencyMetric(metric) {
  const {
    p50Ms = 0,
    p95Ms = 0,
    averageMs = 0,
...
export function formatTraceRecorderRoutes(routes) {
  return ensureArray(routes).length > 0
    ? ensureArray(routes).map((entry) => `${entry.route} x${entry.count}`).join(", ")
    : "none";
}

export function formatTraceRecorderLookups(lookups) {
...
export function formatTraceRecorderPatterns(patterns) {
...
export function formatIntentJournalRows(rows) {
  return formatRows(rows, (row) => {
    const contextKeys = Object.keys(row.context ?? {});
    return [
...
export const MEMORY_STATUS_IDENTITY_LINE_READERS = Object.freeze([
...
export const MEMORY_STATUS_IMPROVEMENT_FIELDS = [
  ["improvementCount", 0],
...
export const MEMORY_STATUS_TRACE_ARTIFACT_FIELDS = [
...
export function formatStatusFieldLines(source, fields) {
...
export function formatTraceRecorderHooks(hooks) {
...
export function formatTraceLookupSamples(rows) {
...
export function formatTracePromptNeedLines(promptNeed) {
  if (!promptNeed) {
    return [];
  }
  return [
...
export function formatRecentTraceLookupLines(lookups) {
...
export function buildRecentTraceHeaderLines(record) {
  return [
    `### ${record.id}`,
    `- hook: ${record.hook}`,
...
export function formatRecentTraceList(values) {
  return ensureArray(values).join(", ") || "none";
}

export function formatRecentTraceValue(value, fallback = "none") {
  return value ?? fallback;
}

export function buildRecentTraceRoutingLines(record) {
  return [
...
export function formatRecentTraceOptionalLines(record) {
  return [
...
export function formatRecentTraceRecord(record) {
  return [
...
export function formatRecentTraceRecords(records) {
...
export function formatMaintenanceTaskState(task) {
  return [
    `- ${task.label}`,
...
export function formatMaintenanceRunRows(runs) {
...
export const MAINTENANCE_TASK_SUMMARY_FIELDS = Object.freeze([
  ["processed", "processed"],
  ["failed", "failed"],
  ["staleCount", "stale"],
...
export function buildMaintenanceTaskSummaryFields(summary) {
...
export function formatMaintenanceTaskResult(task) {
