...
function formatRecentTraceRecord(record) {
  const lines = [
    ...
    `- route: ${record.routerDecision?.route ?? "unknown"}`,
    `- routeReason: ${record.routerDecision?.reason ?? "none"}`,
    `- contextInjected: ${record.output?.contextInjected === true}`,
    `- sectionTitles: ${ensureArray(record.output?.sectionTitles).join(", ") || "none"}`,
    `- eligibility.local: ${ensureArray(record.eligibility?.local).join(", ") || "none"}`,
    `- eligibility.crossRepo: ${ensureArray(record.eligibility?.crossRepo).join(", ") || "none"}`,
