...
function buildPortableBundleRequest(args, runtime) {
...
async function writePortableBundle(bundlePath, portableBundle) {
  if (!bundlePath) {
    return;
  }
...
function formatPortableBundleResult({ portableBundle, bundlePath, repository }) {
...
function normalizeCapabilityInventoryAction(value) {
  return CAPABILITY_INVENTORY_ACTIONS.has(value) ? value : "summary";
}

async function renderCapabilityInventoryAction(args, limit, action) {
  if (action === "evaluate") {
