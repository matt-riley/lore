...
function summarizeBackfillPreviewProgress(preview) {
...
function summarizeBackfillRunProgress(run) {
...
function summarizeMaintenanceProgress(result) {
  ...
  const progressPercent = totalCount > 0
    ...
    : 100;
  const currentPhase = result.dryRun === true
    ? "planning"
    ...
    toolDef("memory_status", {
      ...
      handler: async (args, invocation) => {
        ...
        const lines = [
          ...
          `backfillCurrentPhase: ${backfillCurrentPhase}`,
          ...
          `deferredActionableCount: ${deferredActionableCount}`,
          `deferredCurrentPhase: ${deferredCurrentPhase}`,
    ...
    toolDef("lore_onboard", {
      ...
      handler: async (args, invocation) => {
        ...
        const built = resolveOnboardingInput({
          ...
          userName: typeof args.userName === "string" && args.userName.trim().length > 0
            ? ensureString(args.userName, "userName")
            : undefined,
          assistantName: typeof args.assistantName === "string" && args.assistantName.trim().length > 0
            ? args.assistantName
            : undefined,
          profile: {
            ...
            useNameNaturally: typeof args.useNameNaturally === "boolean"
              ? args.useNameNaturally
              : undefined,
          },
        ...
        for (const memory of built.memories) {
          runtime.db.insertSemanticMemory(memory);
        }

        const autoNamed = typeof args.assistantName !== "string" || args.assistantName.trim().length === 0;
        return [
    ...
    toolDef("lore_retain", {
      ...
      handler: async (args, invocation) => {
        ...
        const repository = typeof args.repository === "string" && args.repository.trim().length > 0
          ? args.repository.trim()
          : runtime.repository;
        const domainKey = typeof args.domainKey === "string" && args.domainKey.trim().length > 0
    ...
    toolDef("lore_reflect", {
      ...
      handler: async (args, invocation) => {
        ...
        if (args.persistObservation === true) {
          ...
          observationLine = `Saved observation ${observationKey}.`;
        }
        ...
        return [
          observationLine,
          formatReflectionReport(reflection, { detailLevel }),
        ].filter(Boolean).join("\n\n");
      },
    }),
