...
export function buildRepairPlan(owner, request, rows) {
  const candidates = [], unresolved = [], sources = [];
  if (request.repositoryMappings.length) return { candidates, unresolved, sources };
    ...
    try {
      ...
      for (const end of ends) {
        ...
        for (const memory of extraction.semanticMemories ?? []) {
          ...
          if (!owner.findActiveSuppression(owner.buildSemanticMemoryWriteContext(memory, "2026-01-01T00:00:00.000Z"))) proposed.set(evidence.key, memory);
        }
      }
      ...
      const action = { sessionId, repository, memories, retireMemoryIds, retiredLinks, retiredEvidenceKeys: [...retiredEvidenceKeys].sort() };
  ...
  return { candidates, unresolved, sources };
}
...
export function applyRepairPlan(owner, candidates, selected) {
  for (const candidate of candidates) {
    ...
    owner.reconcileGeneratedMemories({ sessionId: candidate.sessionId, repository: candidate.repository, memories: candidate.memories, retiredEvidenceKeys: candidate.retiredEvidenceKeys });
  }
}
