...
const REPOSITORY_TABLES = ["semantic_memory", "episode_digest", "day_summary", "memory_domain", "refreshable_observation", "deferred_extraction", "improvement_backlog", "trajectory_artifact", "intent_journal", "backfill_run", "retrieval_trace_sample", "lore_activity_state", "session_evidence", "ingestion_checkpoint"];
const MAPPING_TABLES = [...REPOSITORY_TABLES, "memory_suppression"];
...
function candidateId(table, key) { return `aggregate:${table}:${JSON.stringify(key)}`; }
...
export function buildRowPlan(db, request) {
  ...
  // Explicit selectors decide the target set; repository is a validation
  // constraint on repair, never a broadener of a session or memory selector.
  ...
  // A memory/session selector already determines the source sessions to
  // repair. Keep the optional repository as a validation scope instead of
  // appending every episode and checkpoint in that repository.
  if (request.operation === "repair" && request.repository && !request.memoryIds.length && !request.sessionIds.length) {
  ...
  if (request.operation === "repair" && request.repository && (request.sessionIds.length || request.memoryIds.length)) {
    for (const table of ["episode_digest", "ingestion_checkpoint"]) {
      if ((tables[table] ?? []).some((row) => row.repository !== request.repository)) unresolved.push({ code: "FOREIGN_TARGET", table });
    }
  }
  ...
  if (request.operation === "purge") {
    ...
    for (const table of REPOSITORY_TABLES.filter((name) => name !== "semantic_memory")) {
  ...
  if ((request.operation === "purge" || request.operation === "repair" && request.repository) && Object.values(tables).every((items) => items.length === 0)) unresolved.push({ code: "TARGET_NOT_FOUND", selector: request.repository ?? request.scope });
