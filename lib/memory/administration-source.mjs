...
export function parseAdministrationTranscript(bytes, { client, sessionId, repository, cwd, timestamp }) {
  ...
  if (client === "codex" && records.some(({ value }) => value.type === "session_meta" && typeof value.payload?.id === "string" && value.payload.id !== nativeId)) throw new Error("SOURCE_SESSION_MISMATCH");
  if (client === "claude" && records.some(({ value }) => typeof value.sessionId === "string" && value.sessionId !== nativeId)) throw new Error("SOURCE_SESSION_MISMATCH");
  if (client === "pi") {
    const header = records.find(({ value }) => value.type === "session")?.value;
    if (header?.id && header.id !== nativeId) throw new Error("SOURCE_SESSION_MISMATCH");
  }
  let active = records;
  if (client === "claude") {
