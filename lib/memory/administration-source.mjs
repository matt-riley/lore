...
export function parseAdministrationTranscript(bytes, { client, sessionId, repository, cwd, timestamp }) {
  ...
  if (client === "pi") {
    ...
    if (header?.id && header.id !== sessionId) throw new Error("SOURCE_SESSION_MISMATCH");
  }
  let active = records;
  if (client === "claude") {
    ...
    active = [...nodes.values()].filter(({ value }) => ids.has(value.uuid)).sort((left, right) => Number(left.offset) - Number(right.offset));
  }
