...
function makePlan(owner, input) {
  ...
  const destination = replacement ? owner.db.prepare("SELECT * FROM semantic_memory WHERE type=? AND scope=? AND repository IS ? AND (canonical_key IS ? OR content=?) ORDER BY id LIMIT ?").all(replacement.type, replacement.scope, replacement.repository, replacementKey, replacement.content, request.limit + 1) : [];
  const repositories = [...new Set([...rows.memories.map((row) => row.repository), ...rows.mappings.flatMap((row) => [row.legacy, row.canonical]), ...repair.candidates.flatMap((row) => row.memories.map((memory) => memory.repository ?? null)), ...(replacement ? [replacement.repository] : [])])];
  const suppression = repositories.length ? owner.db.prepare(`SELECT * FROM memory_suppression WHERE ${repositories.map(() => "repository IS ?").join(" OR ")} ORDER BY suppression_key LIMIT ?`).all(...repositories, request.limit + 1) : [];
...
export function applyMemoryAdministration(owner, input, planFingerprint = input?.planFingerprint) {
  ...
  owner.withSemanticMemoryTransaction(() => {
    ...
    if (request.operation === "repair") {
      applyRepairPlan(owner, plan.repair.candidates, selected);
