...
export function normalizeAdministrationRequest(input = {}) {
  ...
  const globalSelection = input.scope === MEMORY_SCOPE.GLOBAL && operation === "purge";
  ...
  return Object.freeze({
    ...
    repository,
    repositoryMappings,
    scope: input.scope === undefined || input.scope === null ? null : normalizeText(input.scope).toLowerCase(),
    content,
...
function selectedMemories(db, request) {
  ...
  if (request.operation === "purge" && request.scope === MEMORY_SCOPE.GLOBAL) {
    return db.prepare("SELECT * FROM semantic_memory WHERE scope = 'global' AND superseded_by IS NULL ORDER BY id LIMIT ?").all(request.limit);
  }
  return [];
}
...
function fingerprintState(db, request, dependencies) {
  const state = {
    selectors: request,
    ...
    dependent: Object.fromEntries(Object.entries(dependencies.dependent).map(([key, rows]) => [key, rows.map((row) => row.id ?? row.domain_key ?? row.observation_key ?? `${row.run_id}:${row.session_id}`)])),
  ...
  if (request.repository) state.repositoryDependencies = Object.fromEntries(Object.entries(collectRepositoryDependencies(db, request.repository, request.limit)).map(([key, rows]) => [key, rows.map((row) => row.id ?? row.session_id ?? row.observation_key ?? row.domain_key ?? row.task_name ?? row.scope_key ?? row.repository ?? row)]));
  return hash(state);
}
...
function unresolvedFor(request, dependencies) {
  ...
  if (request.operation === "repair") {
    ...
    if (request.repositoryMappings.length > 0) {
      for (const mapping of request.repositoryMappings) {
        ...
        if (Object.values(rows).some((items) => items.length > 0) && Object.values(canonicalRows).some((items) => items.length > 0)) {
  ...
  if (request.operation === "purge" && request.repository && dependencies.memories.length >= request.limit) unresolved.push({ code: "BOUND_REACHED", selector: request.repository, limit: request.limit });
...
function makeReport(db, request, dependencies) {
  dependencies._db = db;
  const unresolved = unresolvedFor(request, dependencies);
  const affected = {
  ...
  if (request.repository) {
    const repositoryRows = collectRepositoryDependencies(db, request.repository, request.limit);
  ...
  const stateFingerprint = fingerprintState(db, request, dependencies);
  const report = {
    ...
    planFingerprint: hash({ request: { ...request, action: "preview" }, stateFingerprint, affected }),
    ...
    candidateIds: [...new Set(Object.values(affected).flat())],
  };
...
export function previewMemoryAdministration(db, requestInput) {
  const request = requestInput?.operation ? requestInput : normalizeAdministrationRequest(requestInput);
...
function insertCorrection(db, old, request) {
  ...
  `).run(replacementId, type, request.content ?? old.content, old.confidence, old.source_session_id, old.source_turn_index, scope, repository, old.domain_key, `${old.tags ?? ""} manual correction`.trim(), new Date().toISOString(), new Date().toISOString(), new Date().toISOString(), old.expires_at ?? null, JSON.stringify(metadata));
  db.db.prepare("UPDATE semantic_memory SET superseded_by = ?, updated_at = ? WHERE id = ? AND superseded_by IS NULL").run(replacementId, new Date().toISOString(), old.id);
  if (tableExists(db.db, "memory_suppression")) {
    db.db.prepare(`INSERT OR IGNORE INTO memory_suppression (suppression_key, memory_id, canonical_fingerprint, scope, repository, evidence_fingerprint, actor, reason, created_at, superseded_at, repair_candidate, legacy_marker_fingerprint) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 0, NULL)`).run(
      `correction:${old.id}:${replacementId}`, old.id, hash({ type: old.type, canonicalKey: old.canonical_key, scope: old.scope, repository: old.repository }), old.scope, old.repository, hash({ type: old.type, content: old.content }), "memory_correct", request.reason, new Date().toISOString(),
    );
  }
...
function purgeRows(db, request, report) {
  ...
  if (ids.length > 0) {
    ...
    if (tableExists(db.db, "memory_suppression")) {
      const insert = db.db.prepare("INSERT OR IGNORE INTO memory_suppression (suppression_key, memory_id, canonical_fingerprint, scope, repository, evidence_fingerprint, actor, reason, created_at, superseded_at, repair_candidate, legacy_marker_fingerprint) VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, NULL, 0, NULL)");
      for (const row of rows) insert.run(`purge:${row.id}:${hash(row.content)}`, hash({ type: row.type, canonicalKey: row.canonical_key, scope: row.scope, repository: row.repository }), row.scope, row.repository, hash({ type: row.type, content: row.content }), "memory_purge", "explicit purge", new Date().toISOString());
    }
    ...
    deleteByIds("memory_domain", "domain_key", report.affected.domains ?? []);
    deleteByIds("refreshable_observation", "observation_key", report.affected.observations ?? []);
  ...
  if (sessions.length > 0 && tableExists(db.db, "episode_digest")) {
    ...
    db.db.prepare(`DELETE FROM episode_digest WHERE session_id IN (${placeholders})`).run(...sessions);
    if (tableExists(db.db, "deferred_extraction")) db.db.prepare(`DELETE FROM deferred_extraction WHERE session_id IN (${placeholders})`).run(...sessions);
    ...
    deleteByIds("intent_journal", "id", report.affected.intents ?? []);
    if (tableExists(db.db, "backfill_run_item") && (report.affected.backfillItems ?? []).length > 0) {
      db.db.prepare(`DELETE FROM backfill_run_item WHERE session_id IN (${sessions.map(() => "?").join(",")})`).run(...sessions);
    }
  }
  if (request.repository) {
    for (const [table, column] of DERIVED_REPOSITORY_TABLES) {
      if (["semantic_memory", "episode_digest", "session_evidence", "deferred_extraction"].includes(table)) continue;
      if (tableExists(db.db, table)) db.db.prepare(`DELETE FROM ${table} WHERE ${column} = ?`).run(request.repository);
    }
...
function repairMappings(db, mappings) {
...
export function applyMemoryAdministration(db, requestInput, planFingerprint) {
  ...
  db.withSemanticMemoryTransaction(() => {
    ...
    if (request.operation === "repair") repairMappings(db, request.repositoryMappings);
  });
