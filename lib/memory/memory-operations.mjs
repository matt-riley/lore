  ...
  const merged = {
    ...
    trace: {
      ...
      omissions: [
        ...
        ...semantic.rows.filter((row) => !included.some((entry) => entry.id === row.id)).map((row) => ({
          stage: "semantic_matches",
          reason: "budget_or_duplicate",
          id: row.id,
        })),
      ],
    },
