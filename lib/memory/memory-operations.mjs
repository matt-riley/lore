...
export function mergeSemanticRecallResult({ result, semantic, config = {} } = {}) {
  ...
  const existingIds = new Set(Object.values(result.trace?.lookups ?? {})
  ...
  const renderedSemantic = included.filter((row) => finalBudgeted.text.toLowerCase().includes(row.content.toLowerCase()));
