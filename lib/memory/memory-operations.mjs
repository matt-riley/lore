import { detectPromptContextNeed } from "../context/capsule-assembler.mjs";
import {
...
} from "../context/workstream-overlays.mjs";
import { buildOnboardingSection } from "./onboarding.mjs";
import { collectFilteredReasonSummaries } from "../utils/filtered-reason-summary.mjs";
import { normalizeText } from "../utils/prompt-text-normalizer.mjs";
import { estimateTokens } from "../utils/token-estimator.mjs";
import { enforceSectionBudget, filterTraceIncludedRows, sectionsFromRenderedText } from "../context/output-budget.mjs";

const REQUIRED_RECALL_TITLES = [
  /^Lore Onboarding$/i,
...
/** Merge vector candidates into the already budgeted recall result. */
export function mergeSemanticRecallResult({ result, semantic, config = {} } = {}) {
  if (!result || !semantic || !Array.isArray(semantic.rows)) return result;
  const totalBudget = Number.isFinite(Number(config?.budgets?.total))
    ? Number(config.budgets.total)
    : Number.POSITIVE_INFINITY;
  const baseBudgeted = enforceSectionBudget({
  ...
  const baseText = baseBudgeted.text;
  const existingText = baseText.toLowerCase();
  const baseTrace = filterTraceIncludedRows(result.trace, baseText);
  const existingIds = new Set(Object.values(baseTrace?.lookups ?? {})
    .flatMap((lookup) => lookup?.includedRows ?? [])
    .map((row) => row?.id ?? row?.memoryId).filter(Boolean));
  const seenIds = new Set(existingIds);
  const seenContent = new Set();
  const included = [];
  const sectionPrefix = "## Semantic Matches\n\n";
  const omissions = [];
  for (const row of semantic.rows) {
    const content = normalizeText(row.content);
    if (!content) {
      omissions.push({ stage: "semantic_matches", reason: "empty_candidate", id: row.id ?? null });
      continue;
    }
    if (row.id && seenIds.has(row.id)) {
      omissions.push({ stage: "semantic_matches", reason: "duplicate_id", id: row.id });
      continue;
    }
    if (existingText.includes(content.toLowerCase()) || seenContent.has(content.toLowerCase())) {
      omissions.push({ stage: "semantic_matches", reason: "duplicate_content", id: row.id ?? null });
      continue;
    }
    const line = `- [${row.type}] ${content} (${Math.round(Number(row.score ?? 0) * 1000) / 1000})`;
    const candidateText = `${baseText}${baseText ? "\n\n" : ""}${sectionPrefix}${[...included.map((entry) => `- [${entry.type}] ${entry.content} (${Math.round(Number(entry.score ?? 0) * 1000) / 1000})`), line].join("\n")}`.trim();
    if (estimateTokens(candidateText) > totalBudget) {
      omissions.push({ stage: "semantic_matches", reason: "budget", id: row.id ?? null });
      continue;
    }
    included.push({ ...row, content });
    if (row.id) seenIds.add(row.id);
    seenContent.add(content.toLowerCase());
  }
  const text = included.length > 0
    ? `${baseText}${baseText ? "\n\n" : ""}${sectionPrefix}${included.map((row) => `- [${row.type}] ${row.content} (${Math.round(Number(row.score ?? 0) * 1000) / 1000})`).join("\n")}`.trim()
    : baseText;
  const output = result.trace?.output ?? {};
  const sectionTitles = Array.isArray(output.sectionTitles) ? [...output.sectionTitles] : [];
  const sectionDetails = Array.isArray(output.sectionDetails) ? [...output.sectionDetails] : [];
  if (included.length > 0) {
    sectionTitles.push("Semantic Matches");
    sectionDetails.push({
      title: "Semantic Matches",
      source: "semantic_matches",
      budget: totalBudget,
      usedTokens: estimateTokens(`${sectionPrefix}${included.map((row) => `- [${row.type}] ${row.content} (${Math.round(Number(row.score ?? 0) * 1000) / 1000})`).join("\n")}`),
  ...
  const merged = {
    ...result,
    text,
    semanticMatches: included,
    semanticDiagnostics: semantic.diagnostics ?? null,
    estimatedTokens: estimateTokens(text),
    semanticMatchesRendered: included.length > 0,
    trace: {
      ...result.trace,
      lookups: {
        ...result.trace?.lookups,
        semantic: {
          enabled: semantic.enabled !== false,
          rows: semantic.rows,
          includedRows: included,
          diagnostics: semantic.diagnostics ?? null,
          reason: included.length > 0 ? null : "budget_or_no_valid_vectors",
        },
      },
      output: {
        ...output,
      ...
      omissions: [
        ...(result.trace?.omissions ?? []),
        ...omissions,
      ],
    },
  };
  const finalBudgeted = enforceSectionBudget({
    sections: sectionsFromRenderedText(merged.text, merged.trace?.output?.sectionTitles, merged.trace?.output?.sectionDetails),
    totalBudget,
    requiredTitles: REQUIRED_RECALL_TITLES,
  });
  const renderedSemantic = finalBudgeted.sections.some((section) => section.source === "semantic_matches")
    ? included
    : [];
  const finalTrace = filterTraceIncludedRows(merged.trace, finalBudgeted.text);
  finalTrace.lookups.semantic = {
    ...finalTrace.lookups.semantic,
    includedRows: renderedSemantic,
  };
  finalTrace.output.sectionTitles = finalBudgeted.sections.map((section) => section.title);
  finalTrace.output.sectionDetails = finalBudgeted.sections.map((section) => ({
    title: section.title,
  ...
  finalTrace.output.estimatedTokens = finalBudgeted.estimatedTokens;
  return {
    ...merged,
    text: finalBudgeted.text,
    sections: finalBudgeted.sections.map((section) => section.text),
    sectionDetails: finalBudgeted.sections,
    semanticMatches: renderedSemantic,
    semanticMatchesRendered: renderedSemantic.length > 0,
    estimatedTokens: finalBudgeted.estimatedTokens,
    trace: finalTrace,
  };
}
...
function resolveRecentSessionDetail(session) {
  const checkpointDetail = normalizeText(session?.evidenceSummary || "");
  if (checkpointDetail) {
    return truncateText(checkpointDetail, 600);
  }
  return truncateText(normalizeText(session?.summary || session?.workspaceSummary || ""), 160);
}

function buildRecentSessionEvidenceText(session) {
  const repoLabel = session?.repository ? `${session.repository}: ` : "";
  const detail = resolveRecentSessionDetail(session);
  if (detail) {
...
export function recallMemory({
  ...
  const bounded = enforceSectionBudget({
    sections: sectionsFromRenderedText(rawText, sectionTitles, base.trace?.output?.sectionDetails),
    totalBudget: Number(db.config?.budgets?.total),
    requiredTitles: REQUIRED_RECALL_TITLES,
  });
  const boundedTrace = filterTraceIncludedRows({
    ...base.trace,
    lookups: {
...
function fetchRecentSessionsForLookback({
  ...
  const recentSessions = sessionStore.findSessionsSince({
    ...
    repository,
    includeOtherRepositories,
    includeCheckpointEvidence: Number.isFinite(candidateLimit),
    limit: Number.isFinite(candidateLimit)
      ? Math.max(3, Math.min(Math.trunc(Number(candidateLimit)), 40))
      : Math.max(3, Math.min(limit * 2, 20)),
  });
...
export function reflectMemory({
  ...
  focus = null,
  lookbackHours = null,
  recentSessionCandidateLimit = null,
}) {
  ...
  const {
    ...
    lookbackHours,
    limit,
    candidateLimit: recentSessionCandidateLimit,
  });
