...
const RULES = Object.freeze([
  ...
  { reason: "basic_auth", pattern: /\bAuthorization\s*:\s*Basic\s+[A-Za-z0-9+/=]{16,}/i },
  { reason: "azure_account_key", pattern: /\bAccountKey\s*=\s*[A-Za-z0-9+/=]{20,}/i },
  { reason: "connection_string", pattern: /\b(?:postgres|postgresql|mysql|mongodb(?:\+srv)?|redis|amqp):\/\/[^\s:@/]+:[^\s@/]+@/i },
  {
    reason: "secret_assignment",
    // Quoted keys (`"api_key": "…"`) and values containing punctuation are
    // the shapes real configs use, so the value class is broad on purpose:
    // this is a withhold-only gate, and over-matching costs one un-reranked
    // memory while under-matching leaks a credential.
    pattern: /(?:password|passwd|secret|token|api[_-]?key)["']?\s*[:=]\s*["']?[^\s"',;]{16,}/i,
  },
]);
