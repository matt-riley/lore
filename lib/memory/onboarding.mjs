...
import { normalizeEnum } from "../context/style-addressing.mjs";
...
const DEFAULT_INTERACTION_STYLE_PROFILE = Object.freeze({
  voice: "colleague",
  warmth: "warm",
  humor: "light",
...
function normalizeInteractionStyleProfile(overrides = {}, baseProfile = null) {
  // Use baseProfile as fallback, then DEFAULT
  const base = baseProfile ?? DEFAULT_INTERACTION_STYLE_PROFILE;

  const voice = normalizeEnum(overrides.voice, ALLOWED_VOICES, base.voice);
  const warmth = normalizeEnum(overrides.warmth, ALLOWED_WARMTH, base.warmth);
  const humor = normalizeEnum(overrides.humor, ALLOWED_HUMOR, base.humor);
  ...
  return {
    ...
    useNameNaturally: typeof overrides.useNameNaturally === "boolean"
      ...
      : base.useNameNaturally,
  };
}

function extractAssistantName(memory) {
...
export function readOnboardingState({ db }) {
  ...
  // Extract the profile from the active interaction_style row's metadata
  const storedProfile = styleRows.length > 0 && styleRows[0]?.metadata?.profile
    ? styleRows[0].metadata.profile
    : null;
...
export function buildOnboardingMemories({
  ...
  const resolvedProfile = normalizeInteractionStyleProfile(profile, options.originalProfile);
  ...
  return {
    ...
    profile: resolvedProfile,
    memories,
    originalProfile: options.originalProfile ?? null,
    profileChanged: !profilesAreEqual(options.originalProfile, resolvedProfile),
  };
}

function profilesAreEqual(profile1, profile2) {
  if (!profile1 || !profile2) {
    return profile1 === profile2;
  }
  return (
    profile1.voice === profile2.voice &&
    profile1.warmth === profile2.warmth &&
    profile1.humor === profile2.humor &&
    profile1.humorFrequency === profile2.humorFrequency &&
    profile1.collaborative === profile2.collaborative &&
    profile1.useNameNaturally === profile2.useNameNaturally
  );
}
...
export function resolveOnboardingInput({
  ...
  // Merge incoming profile with existing profile
  const baseProfile = existingState?.storedProfile;
  const resolvedProfile = normalizeInteractionStyleProfile(profile, baseProfile);
...
function buildOnboardingTraceDisabled({ state, reason, missing }) {
  return {
...
function appendOnboardingIdentityGuidance(lines, state) {
...
export function buildOnboardingSection({ db, promptNeed }) {
  ...
  if (state.complete) {
    return buildOnboardingTraceDisabled({
      state,
      reason: "complete",
      missing: [],
    });
  ...
  const lines = buildOnboardingSectionLines(state);
