...
function buildGlobalRulesSection(globalRules) {
...
function buildScopedRulesSection(scopedRules, prependSeparator) {
  if (scopedRules.length === 0) return [];
  const lines = prependSeparator ? [""] : [];
...
export async function buildProceduralProfile({ prompt, relevantInstructionFiles, config }) {
  ...
  const globalSection = buildGlobalRulesSection(globalRules);
  const scopedSection = buildScopedRulesSection(scopedRules, globalSection.length > 0);
  const lines = [...globalSection, ...scopedSection];
