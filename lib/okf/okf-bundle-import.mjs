...
export function formatOkfImportResult({
  ...
  return [
    ...
    `conceptsRead: ${readConceptCount}`,
    `importedCount: ${importedCount}`,
    `skippedCount: ${skippedCount}`,
    `truncated: ${truncated === true}`,
    ...(truncationReasons.length ? [`truncationReasons: ${truncationReasons.join(", ")}`] : []),
    ...(skippedFiles.length ? [`skippedFiles: ${skippedFiles.join(", ")}`] : []),
    "",
    "Notes:",
    ...
    "- retrieve imported concepts via memory_search(query=\"okf_import\", type=\"okf_concept\")",
    "- to revert a bad import: memory_search(query=\"okf_import\", type=\"okf_concept\") to list rows, then memory_forget(id=..., supersededBy=\"reverted okf import\") per row",
    ...(truncated === true ? ["- this import was bounded by the OKF read limits; split or shrink the bundle and re-import to retain the remaining concepts"] : []),
  ].filter(Boolean).join("\n");
}
