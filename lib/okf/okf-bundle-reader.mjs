 ...
 * the directory walk and reserved-filename filtering are cheap filename-only
 * checks and still cover every file, but file *content* reads/parsing stop
 * once the cap is hit. `totalConceptFileCount` on the result still reports
 * the true number of concept-eligible files in the bundle (from the cheap
 * filename listing) even when `concepts` itself was capped, so callers can
 * report accurate "N found, M imported" counts without paying to read/parse
 * every file. Callers like memory_portable_bundle's action=import use this
 * so one large bundle directory can't force unbounded disk I/O/CPU/memory
 * even though the amount of memory actually retained is already capped.
 ...
 */
export async function readOkfBundle(bundleDir, { maxConcepts, includeGraph = true } = {}) {
  const files = await walkMarkdownFiles(bundleDir);
  const conceptFiles = files.filter((filePath) => !RESERVED_FILENAMES.has(path.basename(filePath)));
  ...
  return { bundleDir, concepts, edges, backlinksById, totalConceptFileCount: conceptFiles.length };
}
