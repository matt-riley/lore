...
function schemaInfo(db, dbPath) {
  for (const table of VERSION_TABLES) {
    ...
    return {
      tableName: table,
      version,
      requiredTables: ["semantic_memory", "episode_digest"].every((required) => Boolean(
        db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(required),
      )),
    };
  }
  return { tableName: null, version: 0, requiredTables: false };
}
...
export function restoreRecoverySnapshot({ derivedStorePath, snapshotPath, write = false, clientsStopped = false, detectActiveUsers = defaultActiveDetector, fsOps = {} } = {}) {
  ...
  try {
    ...
    return { written: true, target, snapshotPath: source, rescuePath: movedTarget ? rescue : null, validation: reopened, warnings, stagedValidation };
  } catch (error) {
    ...
    if (movedTarget) {
      ...
      for (const suffix of sidecars) if (existsSync(`${rescue}${suffix}`)) {
