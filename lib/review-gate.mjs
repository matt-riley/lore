...
function markMatchingSections(found, headingText) {
...
function markYamlSections(found, lower) {
  for (const section of PROPOSAL_REQUIRED_SECTIONS) {
...
export function checkProposalSections(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (m) markMatchingSections(found, m[1]);
  }
  markYamlSections(found, text.toLowerCase());
 ...
 * @param {object} options.runtime     - Active Lore runtime (db access)
