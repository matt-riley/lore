...
export function readMemoryDomainsEnabled(config) {
  return readMemoryOperationsEnabled(config)
    && readRolloutBoolean(config, "memoryDomains", false);
}

export function readRefreshableObservationsEnabled(config) {
  return readMemoryDomainsEnabled(config)
    && readRolloutBoolean(config, "refreshableObservations", false);
}
...
export function readPlanReviewOrchestratorEnabled(config) {
  // DEPRECATED: Replaced by explicit plan-review-loop skill
  // Gated separately - does not depend on memory operations
  // Disabled by default for safe staged rollout
  // return readRolloutBoolean(config, "planReviewOrchestrator", false);
  return false;
}
