...
function buildInteractionStyleProfile(signals) {
  return {
    voice: signals.hasColleagueSignal
      ? "colleague"
      : signals.hasCollaborativeSignal
        ? "collaborative"
        : "friendly",
    warmth: signals.hasFriendlySignal ? "warm" : "balanced",
    humor: signals.hasHumorNegativeSignal ? "none" : signals.hasHumorPositiveSignal ? "light" : "none",
    humorFrequency: signals.hasHumorNegativeSignal
      ? "never"
      : signals.hasHumorPositiveSignal
        ? signals.hasHumorFrequentSignal ? "frequent" : "occasional"
        : "never",
