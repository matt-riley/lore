...
import { dispatchOperation as dispatchRegisteredOperation, requireDispatchOutcome } from "./operation-dispatch.mjs";
import {
...
const LORE_CLIENTS = Object.freeze(["copilot", "pi", "codex", "claude", "antigravity"]);
const LORE_SURFACES = Object.freeze(["hook", "tool", "cli", "slash"]);
...
export async function createLoreSession({
  ...
  session.dispatchOperation = async function dispatchOperation(name, args = {}, extra = {}) {
    const invocationSurface = extra.surface ?? "tool";
    // Environment/current-workspace identity is retrieval context, not an
    // implicit replacement selector; administration callers pass explicit
    // selectors in args and this override only applies when requested.
    const previousRepository = session.repository;
    if (Object.hasOwn(extra, "repository")) {
      session.repository = extra.repository;
    }
    try {
      return await dispatchRegisteredOperation({
        tools: toolsForSurface(invocationSurface),
        name,
        args,
        invocation: {
        ...
        unavailable: !session.initialized || session.lastError
          ? unavailableMessage(session.lastError?.message ?? "not initialized")
          : null,
      });
    } finally {
  ...
  // String-returning compatibility wrapper: unavailable/unknown tools stay as
  // text, handler errors keep throwing so protocol transports report failures.
  session.dispatchTool = async function dispatchTool(name, args = {}, extra = {}) {
    return requireDispatchOutcome(await session.dispatchOperation(name, args, extra), { name });
  };
