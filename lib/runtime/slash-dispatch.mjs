...
export const LORE_SLASH_ADVERTISEMENT = "You can also run Lore via /lore <tool> (for example /lore doctor or /lore retain --type decision \"…\"). Extra capabilities run through /lore <verb> rather than the model tool list.";
...
const VERB_ALIASES = Object.freeze({
  save: "lore_retain",
  doctor: "lore_doctor",
  "audit-extractions": "lore_audit_extractions",
});
...
export async function dispatchSlashResult(argsText, dispatchTool, extra = {}) {
  const parsed = parseLoreArgv(argsText);
  if (parsed.error) {
    return { ok: false, text: parsed.error };
  }
  if (typeof dispatchTool !== "function") {
    return { ok: false, text: "lore unavailable: dispatchTool is required" };
  }
  try {
    return {
      ok: true,
      text: await dispatchTool(parsed.name, parsed.args, {
        ...extra,
...
// Chat and native-hook surfaces render dispatch failures inline and stay
// fail-open; the human CLI consumes dispatchSlashResult to exit nonzero.
export async function dispatchSlash(argsText, dispatchTool, extra = {}) {
  return (await dispatchSlashResult(argsText, dispatchTool, extra)).text;
}
