...
import { DEFAULT_MODEL_TOOL_NAMES } from "./tool-registry.mjs";

export const LORE_SLASH_PROMPT_PATTERN = /^\/lore\b/u;
...
export const LORE_SLASH_ADVERTISEMENT = "Lore extras are available as /lore <tool> (for example /lore doctor or /lore retain --type decision \"…\"). Extra verbs stay on /lore rather than the default model tool list.";

export const LORE_SLASH_USAGE = "usage: /lore <verb> [args] — status | recall | retain | forget | search | explain | validate | correct | onboard | doctor | …";
...
function extractJsonFlag(input) {
  if (Array.isArray(input)) {
    const tokens = input.map((part) => String(part));
    ...
    if (tokens[index].startsWith("--json=")) {
      ...
      tokens.splice(index, tokens[index + 1] === undefined ? 1 : 2);
    }
    return { tokens, jsonRaw: jsonRaw || undefined };
  }

  const trimmed = String(input ?? "").trim();
  const match = /(?:^|\s)--json(?:\s+|=)([\s\S]+)$/u.exec(trimmed);
  if (!match) {
    return { tokens: tokenizeLoreArgv(trimmed), jsonRaw: undefined };
  }
  const prefix = trimmed.slice(0, match.index).trim();
  return {
    tokens: tokenizeLoreArgv(prefix),
    jsonRaw: unwrapQuoted(match[1]),
  };
}
...
export async function dispatchSlash(argsText, dispatchTool, extra = {}) {
  ...
  return dispatchTool(parsed.name, parsed.args, {
    ...extra,
  ...
  return {
    ...
    handler: async ({ args, sessionId } = {}) => {
      const text = typeof dispatch === "function"
        ? await dispatch(String(args ?? ""), { sessionId, surface: "slash" })
...
export function defaultModelToolSummary() {
  return DEFAULT_MODEL_TOOL_NAMES.join(", ");
}
