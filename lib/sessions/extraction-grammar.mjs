...
export const EXPLICIT_PREFERENCE_PATTERNS = [
...
function maskedQuotedDirective(text) {
  return text.replace(/"[^"\n]*"|“[^”\n]*”|‘[^’\n]*’|(?<![\p{L}\p{N}])'[^'\n]*'/gu, "quoted object");
}
...
function isQuotedSentence(text) {
  ...
  const directive = directiveBody(maskedQuotedDirective(trimmed)).trim();
  if (/^(?:"[^"\n]*"|“[^”\n]*”|‘[^’\n]*’|'[^'\n]*')\s*[.!?]?$/u.test(trimmed)) return true;
  // Apply the same grammar used downstream after masking quoted objects, so
  // the quoted text cannot supply policy words of its own.
  const correctionLead = /^actually\s*,?\s*(?:that(?:'s| is)\s+wrong\s*:\s*)/i.test(trimmed)
    && /^use\s+.+$/i.test(directive);
  return !correctionLead && !isAcceptedDirectiveSyntax(directive);
}
...
function standingDirectiveSyntax(text) {
  const body = directiveBody(text);
...
function isAcceptedDirectiveSyntax(text) {
  return EXPLICIT_PREFERENCE_PATTERNS.some((pattern) => pattern.test(text))
    || EXPLICIT_REJECTION_PATTERNS.some((pattern) => pattern.test(text))
    || standingDirectiveSyntax(text) !== null;
}

export function standingDirectiveType(text) {
  if (isNonDirectiveSentence(text, { allowConditions: true })) return null;
  return standingDirectiveSyntax(text);
}

// A request can chain a standing-sounding clause with a one-off action via
// "and then" ("Use atomic commits for all the changes in this repo and then
// push them up to main"). The trailing clause is a one-off action, not
// policy, so it must never be folded into a retained standing directive.
// Conservative: only a recognized one-off action verb after "and then"
// triggers the split, so unrelated uses of "and then" are untouched.
const CHAINED_ONE_OFF_ACTION_PATTERN = /^(?:then\s+)?(?:please\s+)?(?:push|merge|deploy|open\s+(?:a\s+|an\s+)?(?:pr|pull\s+request)|create\s+(?:a\s+|an\s+)?(?:pr|pull\s+request)|submit\s+(?:a\s+|an\s+)?(?:pr|pull\s+request)|commit(?:\s+(?:it|that|this|them|the\s+changes?))?|run\s+(?:the\s+)?(?:tests?|build|ci|deploy(?:ment)?)|close\s+(?:the\s+)?(?:issue|ticket)|ship\s+it|release\s+it|tag\s+(?:the\s+)?release)\b/i;

export function splitChainedOneOffClause(sentence) {
  const text = String(sentence || "");
  const match = text.match(/^(.*?)[,]?\s+and\s+then\s+(.+)$/i);
  if (!match) return null;
  const [, leadRaw, trailingRaw] = match;
  const trailing = trailingRaw.replace(/\s+please\s*[.!?]*$/i, "").replace(/[.!?]+$/, "").trim();
  if (!CHAINED_ONE_OFF_ACTION_PATTERN.test(trailing)) return null;
