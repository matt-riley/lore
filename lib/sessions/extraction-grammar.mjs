...
function isQuotedSentence(text) {
  ...
  return !/^(?:(?:for|in)\s+[^,]+,\s*)?(?:please\s+)?(?:i|we)\s+(?:really\s+)?prefer\b|^(?:(?:for|in)\s+[^,]+,\s*)?(?:please\s+)?(?:always\s+)?(?:use|prefer|keep|choose|select|write|run|include|show|ask|check|preserve|make|never|avoid|stop)\b|^(?:(?:for|in)\s+[^,]+,\s*)?(?:please\s+)?(?:do\s+not|don't)\b/i.test(trimmed);
}
