...
function isQuotedSentence(text) {
  ...
  const reported = /\b(?:says?|said|quote[sd]?)\b|\b(?:example|guide)\s*:/i.test(text);
  ...
  return !/^(?:(?:for|in)\s+[^,]+,\s*)?(?:please\s+)?(?:i|we)\s+(?:really\s+)?prefer\b|^(?:(?:for|in)\s+[^,]+,\s*)?(?:please\s+)?(?:always\s+)?(?:use|prefer|keep|choose|select|write|run|include|show|ask|check|preserve|make)\b/i.test(trimmed);
}
