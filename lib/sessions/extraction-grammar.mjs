...
function isQuotedSentence(text) {
  // Quoted tool names and other atomic terms are often part of a direct
  // instruction (for example, `Always use "node:test"`). A quoted phrase
  // remains non-directive evidence because it may be a reported instruction
  // or an example; fenced code is filtered by the caller.
  const quotedSpans = [
    ...text.matchAll(/"([^"\n]*)"|“([^”\n]*)”|‘([^’\n]*)’|'([^'\n]*)’|(?<![\p{L}\p{N}])'([^'\n]*)'/gu),
  ];
  if (quotedSpans.length > 0) {
    return quotedSpans.some((match) => {
      const content = match.slice(1).find((part) => part !== undefined)?.trim() ?? "";
      return !/^[\p{L}\p{N}_./:@+#-]+$/u.test(content);
    });
  }
  return /["“”‘]|(?:^|[\s:(])'/.test(text)
