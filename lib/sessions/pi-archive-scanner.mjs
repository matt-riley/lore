...
/**
 * Resolve the Pi session archive root: explicit paths.piSessionDir first,
 * then the PI_CODING_AGENT_DIR client override, then the standard profile.
 * `~/` overrides expand against `home` like every other Lore path.
 */
export function resolvePiSessionDir({ paths = {}, env = process.env, home = os.homedir() } = {}) {
  const expandHome = (value) => (typeof value === "string" && value.startsWith("~/") ? path.join(home, value.slice(2)) : value);
  const configured = expandHome(paths?.piSessionDir);
  if (typeof configured === "string" && configured.trim().length > 0) {
    return path.resolve(configured);
  }
  const agentDir = expandHome(env?.PI_CODING_AGENT_DIR);
  ...
  async scan({ currentSessionId = null, maxCandidates = DEFAULTS.max } = {}) {
    ...
    while (scanned < this.#scanCap && candidates.length < limit && this.#stack.length > 0) {
      ...
      if (this.#resumeCursor) {
        ...
        this.#resumeCursor = null;
        // Revisit the cursor file itself. It may have been replaced since the
        // previous scan, in which case source identity must be checked again.
      }
      this.#lastPath = fullPath;
      if (entry.isDirectory()) {
        try {
      ...
      try {
        ...
        if (!sessionId || sessionId === currentSessionId || this.#isAlreadyExtracted(sessionId, source)) {
          continue;
        }
        candidates.push({
          path: fullPath,
