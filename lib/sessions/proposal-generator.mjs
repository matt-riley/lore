...
import { resolveLoreHomeFromConfig, resolveProposalRoot } from "../core/lore-paths.mjs";
...
function loreHomeFromConfig(config) {
  return resolveLoreHomeFromConfig(config);
}

function proposalDocsRoot(config) {
  return resolveProposalRoot(config);
}

function relativePathFromRepo(config, absolutePath) {
  return path.relative(loreHomeFromConfig(config), absolutePath).replaceAll(path.sep, "/");
}
...
async function ensureProposalDirectory(dirPath) {
  ...
  try {
    ...
    // Best-effort privacy on the proposals tree; never move configured paths.
  }
}

async function writePrivateProposalFile(filePath, content) {
  await ensureProposalDirectory(path.dirname(filePath));
  await writeFile(filePath, content, { encoding: "utf8", mode: 0o600 });
  try {
    await chmod(filePath, 0o600);
  } catch {
    // Best-effort privacy on generated proposal files.
  }
}
...
async function writeProposalIndex(runtime) {
  const rows = listProposalIndexRows(runtime);
  const indexPath = path.join(proposalDocsRoot(runtime.config), "PROPOSAL_INDEX.md");
  await writePrivateProposalFile(indexPath, renderProposalIndex(rows));
  return relativePathFromRepo(runtime.config, indexPath);
}

const PROPOSAL_INDEX_LIMIT = 200;

// Generation, verification, and repair must judge the index from the same
// complete, consistently ordered query. Callers' inspection limits apply to
// individual proposal files only.
function listProposalIndexRows(runtime) {
  return runtime.db.listImprovementArtifacts({
    hasProposal: true,
    limit: PROPOSAL_INDEX_LIMIT,
  });
}
...
export async function generateProposalArtifacts({
  ...
  for (const artifact of candidates) {
    ...
    if (!dryRun) {
      await writePrivateProposalFile(absolutePath, content);
...
function buildProposalVerificationState(row, config) {
  ...
  const absolutePath = row.proposal_path
    ? path.join(loreHomeFromConfig(config), row.proposal_path)
...
async function repairProposalArtifact({ runtime, row, state, type, dryRun, repair }) {
  ...
  await writePrivateProposalFile(state.absolutePath, state.content);
...
async function repairProposalIndex({ state, repair, dryRun, repaired, type }) {
  ...
  await writePrivateProposalFile(state.indexPath, state.expectedContent);
  repaired.push(buildProposalIssue("proposal-index", type, state.proposalPath));
}

async function verifyProposalIndexArtifacts({ runtime, repair, dryRun }) {
  const rows = listProposalIndexRows(runtime);
  const state = buildProposalIndexState(rows, runtime.config);
  const issues = [];
  const repaired = [];
  ...
  try {
    ...
    // The index is only written on the generation path, so on a store with no
    // proposals an absent file is the steady state rather than drift. There is
    // no derived content to materialize and nothing for the index to describe.
    if (rows.length > 0) {
