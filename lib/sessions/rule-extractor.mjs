...
// Bumped whenever the standing-directive grammar (extraction-grammar.mjs) or
// the classification it feeds (this module) changes materially. Stamped into
// every rule-generated semantic memory's metadata so a later revalidation
// pass (lib/sessions/extraction-revalidation.mjs) can tell a row extracted by
// an older grammar apart from one that already reflects the current rules.
export const EXTRACTOR_VERSION = "2026-09-23";
...
export function hasExplicitDirective(text, patterns) {
...
function buildDirectiveMemory({ sentence, scopeText = sentence, type, repository, sessionId, turn, sourceRole }) {
  const extractedScope = scopeForExtractedDirective(scopeText, repository);
  // An inferred directive with no repository provenance must remain outside
  // global scope. The semantic classifier intentionally preserves explicit
  // operator-written directive memories, so make the inferred boundary
  // explicit here before the memory is persisted.
  const scope = type === "directive" && !Object.hasOwn(extractedScope, "scope")
    ? { scope: MEMORY_SCOPE.REPO, repository: null }
    : extractedScope;
...
function extractDirectiveMemoriesFromTurn({ turn, text, repository, sessionId, sourceRole }) {
  const memories = [];
  ...
  for (const splitSentence of splitSentences(text, { splitSemicolons: false })) {
    // A standing-sounding clause chained to a one-off action via "and then"
    // ("...and then push them up to main") must drop the one-off tail before
    // classification, so the tail is never folded into a retained directive.
...
function extractDecisionMemoryFromTurn({ turn, text, repository, sessionId, sourceRole }) {
  const memories = [];
...
function attachEvidenceToSemanticMemories(memories, { sessionId, turns }) {
...
// A trigger match's capture group ("you keep <this>") can run past the
// natural clause boundary into an unrelated, appended remark joined by a
// dash, comma, or semicolon (for example "you keep doing this - can you stop
// stopping"). isNonDirectiveSentence only rejects a question at the *start*
// of the string, so a question clause stitched on after a separator slips
// through as if it were the mistake itself. Reject it explicitly instead of
// trying to salvage a clean clause out of a run-on capture.
const MISTAKE_EMBEDDED_QUESTION_PATTERN = /(?:^|[,;:-]\s*)(?:can|could|would|will|shall|do|does|did|why|how)\s+(?:you|i|we)\b/i;

// A well-formed mistake is a short, declarative clause: no leading/trailing
// punctuation (handled by cleanMistakeText), no embedded or trailing
// question, no bare quoted fragment, and enough distinct content words that
// it reads as a real clause rather than a stray sentence fragment. A run-on
// capture that swallowed an unrelated following sentence is rejected by the
// length cap rather than truncated, per the "prefer nothing unless the
// grammar can do it reliably" rule for this extractor.
const MISTAKE_MAX_LENGTH = 140;

export function isWellFormedMistakeClause(mistake) {
  if (!mistake || mistake.length < 12 || mistake.length > MISTAKE_MAX_LENGTH) return false;
  if (mistake.includes("?")) return false;
  if (isNonDirectiveSentence(mistake) || isHypotheticalDirectiveSentence(mistake)) return false;
  if (MISTAKE_EMBEDDED_QUESTION_PATTERN.test(mistake)) return false;
  const mistakeWords = mistake.toLowerCase().split(/\s+/).filter(Boolean);
  if (new Set(mistakeWords).size < 3) return false;
  if (/^(?:sad\s+times|bad\s+times|oh\s+well|never\s+mind|ugh|oops|yikes|haha|lol|sigh)\b/i.test(mistake)) return false;
...
function extractRecurringMistakeMemories(turns, repository, sessionId, config = null) {
  ...
  for (const turn of turns.slice(-20)) {
    for (const sentence of splitSentences(normalizeTurnText(turn.user_message, config))) {
      ...
      if (!isWellFormedMistakeClause(mistake)) continue;
...
function buildSessionSemanticMemories({
  ...
  // No implicit, failure-heuristic assistant_goal here: see the removed
  // inferAssistantGoalFromFailures for why (2026-09-24 review finding --
  // templated "Current assistant goal: stabilize ..." rows duplicated across
  // repo/transferable scopes and many repositories, reinforced dozens of
  // times, and injected into prompts as commitments that were never the
  // user's actual goal).
