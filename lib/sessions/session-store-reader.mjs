...
import { sanitizeFtsQuery, tokenizeText } from "../utils/query-normalizer.mjs";
import { normalizeText } from "../utils/text-normalizer.mjs";
import { SessionSource } from "../runtime/session-source.mjs";

export { buildDateSessionMatchRecord, buildSessionMatchRecord };

function buildSessionMatchRecord(row, hydrated, score) {
  return {
    session_id: row.session_id,
    repository: hydrated ? hydrated.repository : row.repository,
...
function buildHydratedWorkspaceFields(workspace, row) {
  return {
    cwd: pickPreferredWorkspaceValue(workspace.cwd, row.cwd),
...
function buildDateSessionMatchRecord(row, evidenceSummary = null) {
  return {
    session_id: row.id,
...
function buildCheckpointEvidenceSummary(row) {
  const title = normalizeText(row?.title);
  const overview = normalizeText(row?.overview);
  return [title, overview].filter(Boolean).join(" - ").slice(0, 1200);
}

export class SessionStoreReader extends SessionSource {
  constructor(config, options = {}) {
    ...
    this.identityResolverCacheVersion = typeof options === "object" && typeof options?.identityResolverCacheVersion === "function"
      ? options.identityResolverCacheVersion
      : null;
    this.lastIdentityResolverCacheVersion = undefined;
    this.identityResolver = typeof options === "function"
      ? options
      : options?.resolveRepositoryIdentity ?? null;
  }
  ...
  loadLatestCheckpointEvidence(sessionIds) {
    if (!Array.isArray(sessionIds) || sessionIds.length === 0) {
      return new Map();
    }
    const placeholders = sessionIds.map(() => "?").join(", ");
    const rows = this.db.prepare(`
      SELECT checkpoint.session_id, checkpoint.title, checkpoint.overview
      FROM checkpoints AS checkpoint
      INNER JOIN (
        SELECT session_id, MAX(checkpoint_number) AS checkpoint_number
        FROM checkpoints
        WHERE session_id IN (${placeholders})
        GROUP BY session_id
      ) AS latest
        ON latest.session_id = checkpoint.session_id
       AND latest.checkpoint_number = checkpoint.checkpoint_number
    `).all(...sessionIds);
    return new Map(rows.map((row) => [
      row.session_id,
      buildCheckpointEvidenceSummary(row),
    ]));
  }
  ...
  hydrateSessionRow(row) {
    ...
    const hydrated = workspace
      ...
      : baseRow;
    if (!this.identityResolver) return hydrated;
    const identity = {
      cwd: hydrated.cwd ?? row.cwd ?? null,
      legacy: workspace?.repository ?? row.repository ?? null,
    };
    if (this.identityResolverCacheVersion) {
      let version = null;
      try { version = this.identityResolverCacheVersion(); } catch { version = null; }
      if (version !== this.lastIdentityResolverCacheVersion) {
        this.identityCache.clear();
        this.lastIdentityResolverCacheVersion = version;
      }
    }
    const cacheKey = `${identity.cwd ?? ""}\u0000${identity.legacy ?? ""}`;
    let resolved;
    if (this.identityCache.has(cacheKey)) resolved = this.identityCache.get(cacheKey);
    else {
      resolved = this.identityResolver(identity);
      this.identityCache.set(cacheKey, resolved ?? null);
    }
    return { ...hydrated, repository: resolved ?? null };
  }
  ...
  findSessionsSince({
    ...
    const boundedRows = filtered.slice(0, boundedLimit);
    if (!includeCheckpointEvidence) {
      return boundedRows.map((row) => buildDateSessionMatchRecord(row));
    }
    const checkpointEvidence = this.loadLatestCheckpointEvidence(
      boundedRows.map((row) => row.id),
    );
    return boundedRows.map((row) => buildDateSessionMatchRecord(
      row,
      checkpointEvidence.get(row.id),
    ));
  }
  ...
  searchIndex({ query, repository, limit = 5 }) {
    ...
    const rows = this.db.prepare(`
      SELECT si.session_id, si.content, si.source_type, si.source_id, s.cwd, s.repository, s.branch
    ...
    const hydrated = rows.map((row) => {
      ...
      return {
        ...row,
        repository: session ? session.repository : row.repository,
  ...
  readRelevantSessionRows(sanitized, repository, limit) {
    return this.db.prepare(`
      SELECT si.session_id, si.content, si.source_type, si.source_id, s.cwd, s.repository, s.branch, s.updated_at
      FROM search_index si
      JOIN sessions s ON s.id = si.session_id
      WHERE search_index MATCH ?
      LIMIT ?
