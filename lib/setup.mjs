...
function hasOnlyRuntimeFiles(target, marker) {
  if (!marker || marker.version !== 1 || !Array.isArray(marker.files)) return false;
  const allowed = new Set([...marker.files, INSTALL_MARKER]);
  return readdirSync(target).every((entry) => allowed.has(entry))
    && (marker.fingerprint === undefined || marker.fingerprint === fingerprint(target));
}
...
function quarantine(target) {
  ...
  const destination = `${target}.lore-setup-quarantine-${randomUUID()}`;
    ...
    for (const [index, change] of plan.changes.entries()) {
      ...
      try {
        if (change.type === "json") {
          ...
          const current = targetStat(change.target);
          if (!current || !current.isDirectory() || current.isSymbolicLink()) throw new Error(`Installation changed during removal: ${change.target}`);
          ensureBackups();
...
function legacyHookCommands(value, client) {
  ...
  const visit = (item) => {
    ...
    if (typeof item.command === "string" && item.command.includes("lore-cli.mjs") && item.command.includes(` hook ${client} `)) found.push(item.command);
...
export function planRemove(ids, { env = process.env, home = os.homedir() } = {}) {
  ...
  for (const id of ids) {
    ...
    if (id === "copilot" || id === "pi") {
      ...
      changes.push({ type: "directory-remove", target, existed: true });
    } else {
      const { original, value } = readTarget(target);
