...
function readManifest(target) {
  ...
  try {
    const manifest = readTarget(target).value;
    const object = (value) => value !== null && typeof value === "object" && !Array.isArray(value);
    if (manifest.version !== 1 || !object(manifest.installs)) {
      throw new Error("expected version 1 and an installs object");
    }
    for (const [id, record] of Object.entries(manifest.installs)) {
      const directoryClient = id === "copilot" || id === "pi";
      if (!SETUP_CLIENTS.some(client => client.id === id) || !object(record)
        || record.kind !== (directoryClient ? "directory" : "hooks")
        || typeof record.target !== "string" || !record.target.trim()
        || (!directoryClient && (!Array.isArray(record.commands) || !record.commands.length
          || record.commands.some(command => typeof command !== "string" || !command.trim())))) {
        throw new Error(`invalid ownership record for ${id}`);
      }
    }
    return manifest;
  } catch (error) {
    throw new Error(`Invalid Lore installation manifest at ${target}: ${error.message}. Restore it from a known-good backup or repair its ownership records before retrying. No changes were made.`, { cause: error });
  }
}
