...
import { mergeSemanticRecallResult } from "../../memory/memory-operations.mjs";
...
export function buildLoreRecallTool(getRuntime, context) {
  ...
  return toolDef("lore_recall", {
    handler: async (args, invocation) => {
      ...
      const semanticAllowed = result.promptNeed?.hasTemporalSignal !== true;
      if (semanticAllowed && semanticSearchEnabled(runtime.config)) {
        const semantic = await semanticSearch({
          db: runtime.db,
          query: prompt,
          repository: runtime.repository,
          includeOtherRepositories: args.includeOtherRepositories === true,
          transferableFallback: args.includeOtherRepositories === true,
          limit: ensureLimit(args.limit, runtime.config.limits.promptContextLimit, 50),
        });
        if (semantic.enabled && semantic.rows.length > 0) {
          result = mergeSemanticRecallResult({
            result,
            semantic,
            config: runtime.config,
          });
        } else if (semantic.diagnostics || semantic.error) {
