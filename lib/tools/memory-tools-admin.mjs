...
export function persistOnboardingMemories(db, memories, changedTypes = null) {
  for (const memory of memories) {
    // If changedTypes is specified, only supersede types that actually changed
    if (changedTypes && !changedTypes.includes(memory.type)) {
      continue;
    }

    // Re-onboarding replaces any prior active row of the same onboarding type
    // (superseded rows are excluded from all reads) so profile updates don't
    // accumulate conflicting style/identity rows in prompt context.
    const prior = db.searchSemantic({
      query: "",
      repository: null,
    ...
    const retainedId = db.insertSemanticMemory(memory);
    for (const row of prior) {
      if (row.id === retainedId) {
        // Canonical-key upsert merged into the existing row — it already
        // carries the new profile; superseding it would erase the update.
        continue;
      }
      db.forgetMemory({
        id: row.id,
        supersededBy: retainedId ?? `onboarding:${row.id}`,
      });
    }
  }
}

export function hasProvidedOnboardingFields(args) {
  const userNameProvided = readOptionalTrimmedString(args.userName) ? true : false;
  const assistantNameProvided = readOptionalTrimmedString(args.assistantName) ? true : false;

  const profileArgs = buildOnboardingProfileArgs(args);
  const profileFieldsProvided = Object.values(profileArgs).some((val) => val !== undefined);

  return userNameProvided || assistantNameProvided || profileFieldsProvided;
}

export function determineChangedTypes(args, built, existingState) {
  const changedTypes = [];

  // Check if userName changed from what was already stored
  const userNameProvided = readOptionalTrimmedString(args.userName);
  if (userNameProvided || !existingState?.userName) {
    if (built.userName !== existingState?.userName) {
      changedTypes.push("user_identity");
    }
  }

  // Check if assistantName changed from what was already stored
  const assistantNameProvided = readOptionalTrimmedString(args.assistantName);
  if (assistantNameProvided || !existingState?.assistantName) {
    if (built.assistantName !== existingState?.assistantName) {
      changedTypes.push("assistant_identity");
    }
  }

  // Check if profile changed
  if (built.profileChanged) {
    changedTypes.push("interaction_style");
  }
...
export function formatOnboardingNoChange() {
  return "Lore onboarding unchanged; pass userName, assistantName, or profile fields to update.";
}
