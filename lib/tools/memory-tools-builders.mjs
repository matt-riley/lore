...
} from "../memory/extraction-revalidation.mjs";
import {
...
import { readOkfBundle } from "../okf/okf-bundle-reader.mjs";
import { buildOkfImportMemories, formatOkfImportResult } from "../okf/okf-bundle-import.mjs";
import {
  applyScopeOverride,
...
function toolDef(name, rest) {
  ...
  if (Object.hasOwn(rest, "parameters")) {
    throw new Error(`Builder for ${name} must not pass parameters; the manifest is SSoT`);
  }
  return {
    ...rest,
    name: spec.name,
...
function wrapCoreAliasTool(tool, alias, getRuntime) {
  if (alias === "memory_save") {
    return {
      ...tool,
      name: alias,
      handler: withAvailableRuntime(getRuntime, async ({ args, invocation, runtime }) => (
        saveSemanticMemoryFromAlias({ args, invocation, runtime, retainMemory })
      )),
    };
  }
...
function expandCoreAliasTools(tool, getRuntime) {
  const spec = getLoreCapabilitySpec(tool.name);
  return [
    tool,
    ...listCoreAliasToolNames(spec).map((alias) => wrapCoreAliasTool(tool, alias, getRuntime)),
  ];
}
...
import { buildLoreAuditExtractionsTool } from "./builders/memory-tool-builder-lore-audit-extractions.mjs";
...
async function importOkfPortableBundle({ runtime, invocation, request }) {
  ...
  const { concepts, totalConceptFileCount, readConceptFileCount, truncated, truncationReasons, skippedFiles } = await readOkfBundle(request.bundlePath, {
  ...
  return formatOkfImportResult({
    ...
    readConceptCount: readConceptFileCount,
    truncated,
    truncationReasons,
    skippedFiles,
  });
...
export function createMemoryTools({ getRuntime }) {
  return MEMORY_TOOL_BUILDERS.flatMap((buildTool) => (
    expandCoreAliasTools(buildTool(getRuntime, MEMORY_TOOL_BUILDER_CONTEXT), getRuntime)
  ));
}
