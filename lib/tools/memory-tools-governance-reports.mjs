...
function formatInstallHealthSignals(signals) {
  const parts = [];
  if (signals?.installHealth) parts.push(`installClientsChecked=${signals.installHealth.clientsChecked} duplicateInstalls=${signals.installHealth.duplicateInstalls}`);
  if (typeof signals?.ingestionCheckpointFailures === "number") parts.push(`ingestionCheckpointFailures=${signals.ingestionCheckpointFailures}`);
  if (signals?.lastMaintenanceRun) parts.push(`lastMaintenanceRun=${signals.lastMaintenanceRun.status}@${signals.lastMaintenanceRun.updatedAt}`);
...
function formatDoctorReport(result) {
  if (!result.incidentCount) {
    return [
      ...
      `signals: maintenanceTasks=${result.signals.maintenanceTaskCount} trajectoryScanned=${result.signals.trajectoryRecentCount}${formatInstallHealthSignals(result.signals)}`,
    ].join("\n");
  }
  const header = [
    `# Lore Doctor Report`,
    ...
    `signals: maintenanceTasks=${result.signals.maintenanceTaskCount} trajectoryScanned=${result.signals.trajectoryRecentCount} improvementActive=${result.signals.improvementActiveCount}${formatInstallHealthSignals(result.signals)}`,
  ...
  const incidentLines = result.incidents.map((inc) => {
    const contextPairs = Object.entries(inc.context ?? {})
      .filter(([, v]) => v != null && v !== "")
      // Structured detail (arrays of records) stays in the JSON result; the
      // text report shows only scalar context so it never prints [object Object].
      .filter(([, v]) => (Array.isArray(v) ? v.every((item) => typeof item !== "object" || item === null) : typeof v !== "object"))
