import { buildRecallEnvelope } from "../memory/memory-operations.mjs";
...
function formatSemanticMatchesLines(result) {
  if (result.semanticMatchesRendered === true
    || !Array.isArray(result.semanticMatches) || result.semanticMatches.length === 0) {
    return [];
  }
  return [
    "",
    "## Semantic Matches",
    "",
    ...result.semanticMatches.map((row) => (
      `- [${row.type}] ${row.content} (${Math.round(row.score * 1000) / 1000})`
    )),
  ];
}
...
function formatRecallReport(result, { includeTrace = false } = {}) {
  const lines = [
    ...
    ...formatSemanticMatchesLines(result),
  ];
  if (includeTrace) {
    lines.push(
      "",
