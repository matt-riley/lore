...
function buildTraceRecordDetails(trace, options, promptNeed, contextText, fallbackDecision) {
  return {
    promptNeed,
    eligibility: buildTraceEligibility(trace),
    routerDecision: compactRouterDecision(trace?.routerDecision, fallbackDecision),
    lookups: buildTraceLookups(trace, options),
    omissions: compactOmissions(trace?.omissions),
    output: buildTraceOutput(trace, contextText),
  };
}
...
function buildTraceRecord(event, options, index) {
  ...
  const fallbackDecision = buildFallbackTraceDecision(event, trace, promptNeed, contextText);

  return {
    ...buildTraceRecordIdentity(event, trace, options, index),
...
function normalizeRecorderLimits(traceRecorderConfig) {
  return {
    maxRecords: clampInteger(traceRecorderConfig?.maxRecords, 40, { min: 1, max: 500 }),
    maxAgeMs: clampInteger(traceRecorderConfig?.maxAgeMs, 30 * 60 * 1000, { min: 60 * 1000, max: 24 * 60 * 60 * 1000 }),
    maxRowsPerLookup: clampInteger(traceRecorderConfig?.maxRowsPerLookup, 3, { min: 1, max: 10 }),
    maxFilteredRowsPerLookup: clampInteger(traceRecorderConfig?.maxFilteredRowsPerLookup, 3, { min: 1, max: 10 }),
    maxPromptChars: clampInteger(traceRecorderConfig?.maxPromptChars, 160, { min: 32, max: 500 }),
    maxRowChars: clampInteger(traceRecorderConfig?.maxRowChars, 160, { min: 32, max: 500 }),
    maxContextChars: clampInteger(traceRecorderConfig?.maxContextChars, 600, { min: 64, max: 4000 }),
  };
}
...
function normalizeDurableRecorderOptions(traceRecorderConfig) {
  return {
    persistDurableSample: traceRecorderConfig?.persistDurableSample !== false,
    durableSampleRate: normalizeSampleRate(traceRecorderConfig?.durableSampleRate),
    durableMaxRowsPerRepository: clampInteger(traceRecorderConfig?.durableMaxRowsPerRepository, 120, { min: 20, max: 5000 }),
    durableMaxRowsGlobal: clampInteger(traceRecorderConfig?.durableMaxRowsGlobal, 240, { min: 20, max: 10000 }),
    durableMaxAgeMs: clampInteger(traceRecorderConfig?.durableMaxAgeMs, 14 * 24 * 60 * 60 * 1000, { min: 60 * 60 * 1000, max: 365 * 24 * 60 * 60 * 1000 }),
  };
}
