...
function renderHelp() {
  return [
    ...
    "  LORE_COPILOT_HOME     Override Copilot inputs (existing legacy Lore stores remain supported).",
    "  LORE_HOME             Override Lore storage directory (default: ~/.config/lore).",
    "  XDG_CONFIG_HOME       Override the config root; Lore uses its lore/ subdirectory.",
    "  LORE_CONFIG           Override lore.json path directly.",
  ].join("\n");
}
...
function renderRecommendedSchedule(config) {
  ...
  return [
    ...
    "- On session start, Lore only auto-selects memoryHygiene and deferredExtraction.",
    "- All other tasks are for external or manual sweeps via this script.",
    "",
    "Maintenance modes:",
    "  automatic       memoryHygiene and deferredExtraction run at session start when enabled and due.",
    ...
    "",
    "Suggested cadences:",
    `- memoryHygiene: every ${cadenceFor("memoryHygiene", 0)} minutes (session-start eligible; mode must be shadow or apply)`,
    ...
    "",
    "# non-standard Lore storage (Copilot inputs retain their own home)",
    "0 */6 * * * LORE_HOME=/path/to/lore-home node /path/to/lore/scripts/run-maintenance.mjs --tasks validationCorpus,backlogReview >> /path/to/lore-maintenance.log 2>&1",
    "",
