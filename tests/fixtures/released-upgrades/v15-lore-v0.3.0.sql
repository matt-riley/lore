-- Released Lore schema fixture: lore-v0.3.0

-- Source commit: 08faff854fba4ef0df6b069acfcaa90717c601f2

-- Extracted from lore-v0.3.0:lib/schema.mjs (SCHEMA_VERSION 15)
...
CREATE INDEX IF NOT EXISTS idx_memory_domain_scope_status
      ON memory_domain(scope, status, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_memory_domain_repository_status
      ON memory_domain(repository, status, updated_at DESC);

CREATE TABLE IF NOT EXISTS refreshable_observation (
      observation_key TEXT PRIMARY KEY,
      domain_key TEXT,
      title TEXT NOT NULL,
      prompt TEXT NOT NULL,
      focus TEXT NOT NULL DEFAULT 'summary',
      summary TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_refreshable_observation_domain_status
      ON refreshable_observation(domain_key, status, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_refreshable_observation_repository_status
      ON refreshable_observation(repository, status, updated_at DESC);
