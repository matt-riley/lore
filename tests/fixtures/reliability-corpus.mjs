...
const INDEPENDENT_SEEDS = Object.freeze([
  ...
  ["token-redaction", "acme/gateway", "Redact bearer tokens before gateway requests enter debug logs, including failed requests.", "Gateway diagnostics will redact bearer tokens before logging success or failure.", "What must gateway debug logs redact?", "rejected_approach", "repo", "redact bearer tokens gateway debug logs"],
  ...
  ["no-secret-fixtures", "acme/tests", "Never commit production secrets to fixtures; generated credentials must be visibly fake and scoped to the test.", "Fixtures will use visibly fake credentials with test-only scope.", "What credential rule applies to test fixtures?", "rejected_approach", "repo", "never production secrets fixtures fake credentials test"],
  ...
  ["cron-timezone", "acme/schedules", "Schedule billing jobs in UTC; local daylight changes should never shift an invoice run.", "Billing schedules will use UTC to avoid daylight-driven shifts.", "Which timezone governs billing jobs?", "user_preference", "repo", "schedule billing jobs UTC daylight invoice"],
  ...
  ["rate-limit-key", "acme/limits", "Key rate limits by account rather than IP because mobile clients can share an address.", "The limiter will use account identity as its key and retain the mobile-client rationale.", "What identifies a rate-limit bucket?", "decision", "repo", "rate limits account IP mobile clients"],
  ...
  ["job-idempotency", "acme/settlement", "Require an idempotency key on settlement commands; a client retry must not create a second payout.", "Settlement commands will reject missing keys and deduplicate retries.", "What prevents duplicate settlement payouts?", "rejected_approach", "repo", "idempotency key settlement commands retry second payout"],
  ["tracing-sampling", "acme/trace", "Sample successful requests at five percent but keep every failed trace so debugging retains the causal path.", "Tracing will use five percent success sampling and complete failure capture.", "What sampling policy did we set for request traces?", "decision", "repo", "sample successful requests five percent every failed trace"],
  ["api-version", "acme/public-api", "Expose the new report fields under API version v3; existing v2 clients must keep their response shape.", "The report fields will be added to v3 while v2 remains stable.", "Which API version receives the report fields?", "decision", "repo", "report fields API version v3 v2 stable"],
  ...
  ["decimal-money", "acme/money", "Represent money as integer minor units; binary floating point is not acceptable for totals.", "Money calculations will use integer minor units and avoid binary floating point.", "How should monetary totals be represented?", "rejected_approach", "repo", "money integer minor units binary floating point"],
  ...
  ["archive-format", "acme/archive", "Use zstd for cold archives because decompression speed matters during incident review.", "Cold archives will use zstd with incident review speed as the rationale.", "Which compression format is used for cold archives?", "decision", "repo", "use zstd cold archives decompression incident review"],
  ["locale-fallback", "acme/i18n", "Fall back to en-GB when a translation is missing so legal wording remains explicit.", "Missing translations will fall back to en-GB and emit a diagnostic.", "What locale is the translation fallback?", "user_preference", "repo", "fallback en-GB translation missing legal wording"],
  ["no-debug-pii", "acme/support", "Never log full support messages in debug mode; redact customer details before retaining diagnostic context.", "Support diagnostics will redact customer details before any debug retention.", "What must support debug logs exclude?", "rejected_approach", "repo", "never log full support messages redact customer details"],
  ...
  ["db-readonly", "acme/reporting", "Run reporting queries through a read-only database role; dashboards must not mutate operational data.", "Reporting will use a read-only role and reject mutation privileges.", "Which database role do dashboards use?", "rejected_approach", "repo", "reporting queries read-only role dashboards mutate"],
  ...
  ["snapshot-naming", "acme/snapshots", "Name snapshots with the repository and UTC timestamp so operators can identify their source without opening them.", "Snapshots will use repository and UTC timestamp components in their names.", "How should snapshots be named?", "user_preference", "repo", "snapshots repository UTC timestamp names"],
  ["data-classification", "acme/data", "Classify imported records before indexing; restricted fields must never enter the general search index.", "The importer will classify records and route restricted fields away from general search.", "What happens before imported records are indexed?", "rejected_approach", "repo", "classify imported records restricted fields search index"],
  ["global-tone", "acme/voice", "Use a calm, direct tone for difficult status updates in every repository; avoid theatrical certainty.", "I will keep difficult updates calm and direct while naming uncertainty.", "What tone should difficult status updates use?", "user_preference", "global", "calm direct tone difficult status updates uncertainty"],
  ["lock-timeout", "acme/locks", "Fail a lock acquisition after eight seconds and return a retryable error instead of waiting indefinitely.", "Lock acquisition will time out at eight seconds with a retryable error.", "What is the lock acquisition timeout?", "decision", "repo", "lock acquisition eight seconds retryable error"],
  ...
  ["blob-encryption", "acme/blobs", "Encrypt stored blobs with the service key before replication; transport TLS alone is not storage protection.", "Blob encryption will occur before replication and use the service key.", "What protects replicated blobs at rest?", "rejected_approach", "repo", "encrypt stored blobs service key replication TLS"],
  ["release-channel", "acme/release", "Publish candidate builds to the beta channel for one day before promoting them to stable.", "Candidate builds will spend one day in beta before stable promotion.", "What release channel sequence did we choose?", "decision", "repo", "candidate builds beta channel one day stable"],
  ["global-copy", "acme/copy", "Prefer plain language in user-facing copy across projects; explain a technical term on first use.", "User-facing copy will use plain language and define technical terms once.", "What copy style should apply to user-facing text?", "user_preference", "global", "prefer plain language user-facing copy technical term"],
  ...
  ["worker-pool", "acme/pool", "Cap the image worker pool at twelve concurrent jobs so memory pressure stays predictable.", "The image worker pool will allow at most twelve concurrent jobs.", "What is the image worker concurrency limit?", "decision", "repo", "image worker pool twelve concurrent jobs memory"],
  ["no-raw-sql", "acme/query", "Do not concatenate user input into SQL; parameterized queries are mandatory at every repository boundary.", "Repository queries will use parameters and reject concatenated user input.", "What SQL construction rule is mandatory?", "rejected_approach", "repo", "do not concatenate user input SQL parameterized queries"],
  ...
  ["redaction-order", "acme/logging", "Redact sensitive headers before serializing the request for logs; redacting the final string leaves nested values exposed.", "Request logging will redact headers before serialization.", "When does request logging redact sensitive headers?", "rejected_approach", "repo", "redact sensitive headers before serializing request logs"],
  ...
  ["dead-letter", "acme/dlq", "Move messages to a dead-letter queue after three processing failures and preserve the original error.", "The consumer will dead-letter after three failures with the original error attached.", "When does a message enter the dead-letter queue?", "decision", "repo", "dead-letter queue three processing failures original error"],
  ["no-silent-truncate", "acme/text", "Do not silently truncate user text; return a visible limit error and record the accepted maximum.", "Text input will return a visible limit error instead of silently truncating.", "What happens when user text exceeds the limit?", "rejected_approach", "repo", "do not silently truncate user text visible limit error"],
  ...
  ["decision-tls", "acme/tls", "Use TLS 1.3 for internal service calls because the platform supports it and handshake latency is acceptable.", "Internal calls will use TLS 1.3 with the measured latency tradeoff.", "Which TLS version did we choose internally?", "decision", "repo", "use TLS 1.3 internal service calls handshake latency"],
  ...
  ["redelivery-count", "acme/delivery", "Stop redelivering a message after five attempts and retain the delivery history for operators.", "Delivery will stop after five attempts with history retained.", "After how many attempts does delivery stop?", "decision", "repo", "stop redelivering message five attempts delivery history"],
  ...
  ["payload-limit", "acme/payload", "Limit webhook payloads to two megabytes and return a typed error when the limit is exceeded.", "Webhook intake will enforce a two-megabyte payload limit.", "What is the webhook payload limit?", "decision", "repo", "webhook payloads two megabytes typed error"],
  ...
  ["worker-shutdown", "acme/shutdown", "Stop accepting new work before closing database connections so in-flight jobs can finish cleanly.", "Shutdown will drain intake before closing the database.", "What order should worker shutdown follow?", "user_preference", "repo", "stop accepting work before closing database connections"],
  ["decision-oidc", "acme/identity", "Use OIDC for service identity because short-lived tokens reduce credential rotation burden.", "Service identity will use OIDC and short-lived tokens.", "Why did we choose OIDC for service identity?", "decision", "repo", "use OIDC service identity short-lived tokens rotation"],
  ...
  ["decision-index", "acme/index", "Use an inverted index for exact identifiers and semantic search only as a supplemental ranking signal.", "Exact identifiers will use the inverted index, with semantic ranking supplemental.", "Which search path handles exact identifiers?", "decision", "repo", "inverted index exact identifiers semantic supplemental"],
  ...
  ["request-budget", "acme/budget", "Cap each request's downstream fan-out at four services so latency remains explainable.", "Requests will allow at most four downstream calls and record the cap.", "What limits downstream request fan-out?", "decision", "repo", "request downstream fan-out four services latency"],
  ...
  ["decision-retention-window", "acme/window", "Retain the last thirty days of operational events online and archive older events for audit retrieval.", "Operational storage will keep thirty days online and archive older events.", "What is the operational event retention split?", "decision", "repo", "retain thirty days operational events archive older audit"],
  ...
  ["header-size", "acme/http", "Reject HTTP headers larger than sixteen kilobytes before routing the request.", "The edge will reject headers over sixteen kilobytes before routing.", "What header-size limit applies before routing?", "decision", "repo", "reject HTTP headers sixteen kilobytes routing"],
  ...
  ["safe-parser", "acme/safe-parser", "Parse untrusted YAML with aliases disabled because recursive aliases can exhaust memory.", "The YAML parser will disable aliases before reading untrusted input.", "Which YAML parser safety option is required?", "rejected_approach", "repo", "YAML aliases disabled recursive aliases memory"],
  ["global-questions", "acme/questions", "When a prompt is only a question, do not store it as a preference until a decision is explicitly made.", "Questions will remain exploratory until an explicit decision appears.", "How should question-only prompts be treated?", "user_preference", "global", "question only do not store preference explicit decision"],
  ["decision-restore", "acme/restore-v2", "Restore snapshots into a staging file and validate them before replacing the active database.", "Snapshot restore will stage and validate before active replacement.", "How should database snapshots be restored?", "decision", "repo", "restore snapshots staging file validate replacing active database"],
  ...
  ["concurrency-limit", "acme/concurrency", "Limit concurrent report generation to three jobs because each job holds a large working set.", "Report generation will run at most three jobs concurrently.", "What limits concurrent report generation?", "decision", "repo", "limit concurrent report generation three jobs working set"],
]);
