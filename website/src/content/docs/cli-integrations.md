...
Time-budgeted session-start maintenance (`memoryHygiene`, `deferredExtraction`) runs on these adapters too: on `SessionStart` (Antigravity: the first `PreInvocation`), the short-lived hook process spawns a detached `scripts/run-maintenance.mjs --background` and returns immediately without waiting on it. The child has a 60-second time budget; if it expires, the child waits for in-flight work to settle before closing the database and exits with status 1. Raw archive backfill, host hook metrics, and subagent-specific scope tracking are not wired into these adapters. Use the separate [maintenance script](/guides/maintenance/) for the other tasks (validation/replay corpora, backlog review, trace compaction, index upkeep, doctor snapshots, and opt-in extraction revalidation) or for wall-clock-driven scheduling. [Troubleshooting](/guides/troubleshooting/#native-cli-hooks-do-not-recall-or-capture) covers trust, paths, and missing transcripts.
